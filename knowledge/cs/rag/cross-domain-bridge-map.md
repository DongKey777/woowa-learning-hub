...
> retrieval-anchor-keywords: browser controller save sql route, request entry to sql route, network spring database beginner route, controller 다음 save sql route, browser to controller to database route
...
- `브라우저 요청이 controller를 지나 DB까지 어떻게 와요?`, `controller 다음에 save()와 SQL은 어디서 봐요?`, `왜 네트워크나 Spring을 안 보고 DB 문서부터 보면 헷갈려요?`처럼 request entry와 SQL handoff가 한 질문에 섞이면 [Browser -> Controller -> `save()` -> SQL Beginner Handoff](#bridge-request-entry-to-sql-cluster) route를 먼저 탄다.
...
<a id="bridge-request-entry-to-sql-cluster"></a>
### Browser -> Controller -> `save()` -> SQL Beginner Handoff (`network ↔ spring ↔ database`)

대표 symptom alias: `브라우저 요청이 controller를 지나 DB까지 어떻게 와요?`, `controller 다음에 save()와 SQL은 어디서 봐요?`, `왜 네트워크나 Spring을 안 보고 DB 문서부터 보면 헷갈려요?`, `request -> controller -> service -> repository 흐름이 먼저예요`

카테고리 entrypoint: [Network README: Network -> Spring handoff](../contents/network/README.md#network---spring-handoff), [Spring README: 빠른 탐색](../contents/spring/README.md#빠른-탐색), [Database README: Request -> Controller -> `save()` -> SQL handoff](../contents/database/README.md#database-bridge-request-entry-to-sql)

beginner-safe ladder:

1. [HTTP 요청-응답 기본 흐름](../contents/network/http-request-response-basics-url-dns-tcp-tls-keepalive.md)
2. [Spring 요청 파이프라인과 Bean Container 기초](../contents/spring/spring-request-pipeline-bean-container-foundations-primer.md)
3. [Database First-Step Bridge](../contents/database/database-first-step-bridge.md)
4. [JDBC · JPA · MyBatis 기초](../contents/database/jdbc-jpa-mybatis-basics.md)

이 route의 목표는 "`요청이 어디로 들어오나`", "`controller/service는 누가 연결했나`", "`DB 작업을 어디까지 묶나`", "`save() 뒤 SQL은 어느 기술이 만들까`"를 먼저 분리하는 것이다. `404`, `400`, bean wiring, `@Transactional`이 한 장면에 같이 보이면 이 route 안에서 질문 축만 다시 고르고, `deadlock`, `40001`, `failover`, `cdc replay`는 뒤 follow-up으로 미룬다.

safe next step rule:

- URL 길찾기나 `DispatcherServlet`이 먼저 흐리면 [Spring MVC 요청 생명주기 기초](../contents/spring/spring-mvc-request-lifecycle-basics.md)로 한 칸만 내려간다.
- Bean 등록, `controller` 안 `service` 주입, component scan이 먼저 흐리면 [Spring Bean과 DI 기초](../contents/spring/spring-bean-di-basics.md)로 한 칸만 내려간다.
- `@Transactional` 경계가 새로 막히면 [Database README: 트랜잭션 경계 / 애플리케이션 브리지](../contents/database/README.md#database-bridge-transaction-app)나 [Transaction Isolation / `@Transactional` / Rollback Debugging](#bridge-database-spring-transaction-cluster) route로 이동한다.
- `save()` 뒤 SQL 위치는 보이는데 예외 이름이나 retry가 새로 붙으면 [Database README: Retry / Idempotency / Replay 브리지](../contents/database/README.md#database-bridge-retry-replay)로만 한 칸 더 내려간다.
