...
`survey -> primer -> primer bridge -> catalog/navigator -> deep dive -> playbook/runbook/recovery`
...
9. beginner/junior 독자라면 첫 클릭 후보를 `survey -> primer -> primer bridge -> catalog` 순서에서 먼저 고른다.
...
| `이 증상을 어느 문서로 넘기죠`, `다음 문서가 뭐죠` | `primer bridge` 또는 `catalog` | advanced incident catalog |
...
- `catalog / navigator`: 다음에 읽을 문서를 고른다.
