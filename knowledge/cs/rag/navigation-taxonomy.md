...
> 한 줄 요약: README와 navigator 문서에서 `survey`, `primer` / `primer bridge`, `catalog/navigator`, `deep dive`, `playbook` / `recovery`, `question bank`를 역할 기준으로 구분해 retrieval routing의 혼선을 줄이고 advanced incident 문서를 beginner entrypoint로 잘못 고르는 일을 줄인다.
...
- [`routing helper`] [RAG Design](./README.md)
- [`routing helper`] [Topic Map](./topic-map.md)
- [`routing helper`] [Query Playbook](./query-playbook.md)
- [`cross-category bridge`] [Cross-Domain Bridge Map](./cross-domain-bridge-map.md)
- [`metadata helper`] [Retrieval Anchor Keywords](./retrieval-anchor-keywords.md)
- [`style guide`] [Document Naming and Linking Guidelines](./document-naming-linking-guidelines.md)
- [`synthesis`, advanced follow-up] [Master Notes](../master-notes/README.md)
- [`category navigator`] [Database README](../contents/database/README.md)
- [`category navigator`] [Network README](../contents/network/README.md)
- [`category navigator`] [Security README](../contents/security/README.md)

retrieval-anchor-keywords: navigation taxonomy, document role, survey, primer, primer bridge, catalog, deep dive, playbook, recovery, entry shelf, helper shelf, follow-up shelf, primer vs playbook, 처음, 헷갈려요

## 왜 필요한가

같은 `README.md` 안에 학습 순서, 입문 개념, 주제별 문서 목록, 실전 장애 문서가 함께 들어가면 읽는 사람도 RAG도 "지금 필요한 문서 역할"을 헷갈리기 쉽다.
이 문서는 파일명을 바꾸지 않고도 first click 안전선을 먼저 긋는 최소 분류표다.

> beginner-safe 기본 사다리: `meta navigator -> survey -> category navigator -> primer -> primer bridge -> deep dive -> playbook/runbook/recovery`

카테고리 `README`에 이미 들어와 있다면 앞의 두 칸은 생략하고 `category navigator -> primer -> primer bridge -> deep dive`만 기억하면 된다.

| shelf | 여기에 들어가는 역할 | beginner 첫 클릭 규칙 |
|---|---|---|
| `entry shelf` | `survey`, `category navigator`, `primer`, `primer bridge` | broad 질문이면 먼저 여기서 한 칸 고른다 |
| `helper shelf` | `meta navigator`, `routing helper`, `taxonomy` | 역할만 해석하고 entry shelf로 내려간다 |
| `follow-up shelf` | `deep dive`, `playbook`, `runbook`, `recovery`, `master note`, `question bank` | entry shelf를 지난 뒤에만 연다 |

| 헷갈리는 조합 | beginner-first 해석 | 안전한 첫 클릭 | 나중으로 미루는 것 |
|---|---|---|---|
| `survey` vs `primer` | `survey`는 순서, `primer`는 첫 설명 | `처음`, `뭐부터`면 `survey` | `deep dive` |
| `primer` vs `primer bridge` | `primer`는 개념 축, `primer bridge`는 분기 정리 | `what is`, `basics`면 `primer` | incident 문서 |
| `deep dive` vs `playbook` | `deep dive`는 원인 심화, `playbook`은 대응 절차 | 원인 이해가 먼저면 `deep dive` | `runbook`, `recovery` |
| `README` vs `taxonomy` | `README`는 다음 문서를 고르고, `taxonomy`는 역할 이름을 해석 | category `README` | helper 문서를 primer처럼 끝까지 읽기 |

| 먼저 보인 라벨 | 초심자용 역할 해석 | beginner-safe first click? | 바로 다음 한 걸음 |
|---|---|---|---|
| `survey`, `roadmap` | 큰 그림 entry shelf | yes | category `README`로 내려가기 |
| `README`, `navigator`, `taxonomy` | route decoder 또는 category shelf | yes, but routing first | `primer` / `primer bridge` 위치만 찾기 |
| `primer`, `primer bridge` | 첫 설명과 첫 분기 | yes | 필요한 `deep dive` 1개로만 확장 |
| `playbook`, `runbook`, `recovery`, `incident matrix` | 운영/장애 follow-up | no | `survey`나 `primer`로 한 칸 되돌아가기 |

## Shelf First

| 지금 보이는 말 | 먼저 붙일 역할 | beginner-safe 다음 한 걸음 | 아직 첫 클릭으로 올리지 않는 것 |
|---|---|---|---|
| `처음`, `뭐부터`, `학습 순서`, `big picture` | `survey` | roadmap이나 category `README`로 내려간다 | `playbook`, `recovery`, `master note` |
| `what is`, `basics`, `헷갈려요` | `primer` / `primer bridge` | 같은 category의 primer 구간을 고른다 | advanced incident docs |
| `taxonomy`, `navigator`, `routing`, `query playbook` | `helper shelf` | 역할만 확인하고 실제 primer나 category `README`로 이동한다 | helper 문서를 concept 본문처럼 끝까지 읽기 |
| `incident`, `recovery`, `runbook`, `case study`, `cutover`, `senior` | follow-up shelf | 한 칸 되돌아가 `survey`나 `primer`부터 다시 고른다 | broad beginner 질문의 첫 답으로 쓰기 |

## File Role Split

README / navigator 파일 이름이 같아도 역할은 아래처럼 다르게 읽는다.

| 먼저 보인 파일 | 먼저 붙일 역할 | beginner-safe 다음 한 걸음 |
|---|---|---|
| 루트 `README.md` | `meta navigator` | `survey`나 category `README`로 내려간다 |
| `contents/*/README.md` | `category navigator` | 같은 README 안의 `primer` / `primer bridge`를 고른다 |
| `rag/README.md`, `navigation-taxonomy.md` | `routing helper` | 역할 이름만 확인하고 category `README`나 linked `primer`로 이동한다 |
| `master note`, `playbook`, `recovery`가 먼저 보이는 파일 | advanced follow-up shelf | beginner라면 한 칸 되돌아가 `survey`나 `primer`부터 다시 고른다 |

## Navigator Difficulty Rule

mixed README에서 자주 생기는 오해는 "안에 advanced 링크가 많으니 이 README도 advanced겠지"라고 읽는 것이다.
하지만 beginner/junior 독자 기준에서는 navigator의 난이도는 가장 깊은 링크가 아니라, 첫 클릭을 얼마나 안전하게 안내하느냐로 붙이는 편이 맞다.

| 문서 타입 | 난이도 라벨을 붙일 때 기준 | beginner 오해를 줄이는 이유 |
|---|---|---|
| `meta navigator`, `category navigator`, `routing helper` | first click을 안전하게 고르는가 | advanced incident 링크가 있어도 entrypoint 문서로 읽을 수 있다 |
| `primer`, `primer bridge` | 개념 축과 첫 분기를 혼자 설명할 수 있는가 | `what is`, `basics`, `헷갈려요` query와 바로 맞물린다 |
| `deep dive`, `playbook`, `recovery`, `retirement` | 원인/운영 절차를 이미 좁힌 뒤 읽어야 하는가 | follow-up 문서를 beginner starter로 오해하지 않게 한다 |

README repair 때는 heading만 고치지 말고, 난이도 라벨과 첫 문장도 같은 방향으로 맞춘다.
`category navigator`라면 "이 문서는 입문 경로를 고르는 인덱스"라는 뜻이 제목 아래에 바로 보여야, junior reader가 advanced incident shelf를 첫 클릭으로 착각하지 않는다.

헷갈리면 먼저 "entry shelf / helper shelf / follow-up shelf" 3칸으로만 자른다.

| shelf | 여기에 들어가는 역할 | beginner 첫 클릭 여부 |
|---|---|---|
| `entry shelf` | `survey`, `category navigator`, `primer`, `primer bridge` | yes |
| `helper shelf` | `meta navigator`, `routing helper`, `taxonomy` | yes, but only for route decoding |
| `follow-up shelf` | `deep dive`, `playbook`, `runbook`, `recovery`, `master note`, `question bank` | usually no |

초심자가 가장 자주 하는 실수는 `deep dive`와 `playbook`을 같은 "자세한 문서"로 묶어 버리는 것이다.
하지만 beginner-safe 기준에서는 둘의 목적이 다르다.
`deep dive`는 개념과 원인을 더 좁히는 follow-up이고, `playbook` / `recovery`는 실제 장애 대응 순서가 먼저일 때만 여는 운영 follow-up이다.

## Question Translation

역할 라벨을 초심자 질문으로 다시 번역하면 더 단순해진다.

| 보이는 역할 | 초심자 질문으로 번역 | safe first click? |
|---|---|---|
| `survey` | "어디부터 봐요?" | yes |
| `primer` | "이게 뭐예요?" | yes |
| `primer bridge` | "헷갈리는 분기를 어디서 나눠요?" | yes |
| `deep dive` | "원인 하나를 왜 이렇게 설계했어요?" | after primer |
| `playbook` / `recovery` | "지금 장애 대응 순서가 뭐예요?" | only when incident-first |
...
`Cluster Shortcuts`처럼 중립적인 heading보다 `Deep-Dive Follow-Up Shortcuts`, `Advanced Incident / Recovery Shortcuts`처럼 역할을 앞에 붙인 heading이 beginner-safe하다. section 이름만 보고도 "entry shelf인지, follow-up shelf인지"가 드러나야 mixed README에서 incident docs를 primer처럼 오해하지 않는다.

mixed README를 볼 때 heading만으로도 1차 stop rule을 적용할 수 있다.

| heading에 먼저 보이는 말 | beginner-safe 해석 | 첫 클릭으로 바로 올리지 않는 것 |
|---|---|---|
| `Beginner`, `Primer`, `Quick Start`, `First-Step` | entry shelf | 없음. 이쪽에서 시작 |
| `Case Study`, `Cutover`, `Recovery`, `Incident`, `Retirement` | follow-up shelf | broad `처음`, `what is`, `basics` 질문의 첫 답 |
| `Taxonomy`, `Navigator`, `Routing`, `Query Playbook` | helper shelf | helper 문서를 concept primer처럼 끝까지 읽는 것 |

## Heading Repair Pattern

README repair를 할 때는 section heading을 아래처럼 노출하면 혼선이 가장 적다.

| 애매한 heading | beginner-safe heading | 왜 더 안전한가 |
|---|---|---|
| `Quick Links` | `Beginner Entrypoints` | 첫 클릭 선반임을 바로 드러낸다 |
| `Shortcuts` | `Primer Bridge Shortcuts` | symptom bridge인지 deep dive 모음인지 구분된다 |
| `Advanced Topics` | `Deep-Dive Follow-up Shelf` | 첫 클릭이 아님을 제목에서 바로 알 수 있다 |
| `Troubleshooting` | `Incident / Recovery Follow-up` | 운영 대응 문서를 primer처럼 오해하지 않게 한다 |

security처럼 incident-heavy README를 손볼 때도 같은 규칙을 그대로 적용하면 된다.

| 기존 heading | beginner-first heading | 왜 더 안전한가 |
|---|---|---|
| `빠른 시작` | `Beginner Entrypoints (`primer` / `survey` / incident split)` | 첫 클릭 선반과 incident 분리 의도를 제목에서 바로 보여 준다 |
| `첫 분기` | `Beginner Primer Bridge (첫 분기)` | 입문 설명 다음의 handoff라는 역할이 분명해진다 |
| `OAuth Follow-Ups` | `Deep-Dive Follow-Up: OAuth` | social login 입문과 callback hardening follow-up을 섞지 않게 한다 |
...
## Mixed README Repair Rule (`beginner-first order`)
...
`Entrypoints`라는 heading도 안전하지 않을 때가 있다.
같은 파일 안에 beginner starter와 advanced follow-up가 같이 있으면 heading에 역할을 같이 적는 편이 더 낫다.

| 애매한 section title | beginner-safe title | 왜 바꾸는가 |
|---|---|---|
| `Case Study Entrypoints` | `Case Study Entrypoints (`deep dive`, follow-up)` | 사례형 설계를 primer starter로 오해하지 않게 한다 |
| `Replay and Cutover Entrypoints` | `Replay and Cutover Entrypoints (`system design`, follow-up)` | migration/cutover 묶음을 beginner 첫 클릭에서 분리한다 |
| `Retirement Route` | `Retirement Route (`system design`, advanced follow-up)` | cleanup/retirement를 entry shelf로 착각하지 않게 한다 |
| `Stale Triage Entrypoints` | `Stale Triage Entrypoints (`primer bridge`)` | 증상형 bridge인지 incident playbook인지 바로 구분하게 한다 |

## TOC Alignment

TOC label도 같은 원칙을 따라야 한다.
mixed README에서는 본문 heading만 고치고 TOC를 그대로 두면 검색/스크롤 단계에서 다시 섞이기 쉽다.

| TOC에서 먼저 보이는 label | beginner-safe TOC label | 왜 더 안전한가 |
|---|---|---|
| `Security / System-Design Incident Bridge` | `Security / System-Design Incident Bridge (`playbook` / `recovery`, follow-up)` | troubleshooting bridge와 live incident follow-up을 TOC 단계에서 바로 분리한다 |
| `Auth Session Troubleshooting Bridge` | `Auth Session Troubleshooting Bridge (`primer bridge` -> `recovery` / `system design`)` | browser/session symptom 분기와 system-design handoff를 같은 entrypoint로 오해하지 않게 한다 |
| `초심자 1회차 라우트` | `초심자 1회차 라우트 (`survey` / `primer`, first click)` | mixed advanced README 안에서도 first-click shelf를 TOC에서 먼저 고정한다 |

section 이름만 바꾸고 끝내면 혼선이 남는다.
mixed README / navigator를 고칠 때는 아래 3개가 같은 방향을 보게 맞추는 편이 안전하다.

| 같이 맞춰야 하는 곳 | beginner-safe 기준 | 왜 필요한가 |
|---|---|---|
| section heading | `entry shelf`인지 `follow-up shelf`인지 제목에서 드러난다 | 스크롤만 해도 역할을 오해하지 않는다 |
| TOC label | heading과 같은 역할 라벨을 유지한다 | 검색 결과와 TOC에서 다른 해석이 생기지 않는다 |
| section 첫 문장 | `first click`인지 `follow-up`인지 문장으로 다시 못 박는다 | 같은 heading 아래 mixed bullet이 있어도 입문 경로가 흔들리지 않는다 |

## Category Bridge Rule

category README 안의 bridge subsection도 역할을 따로 적는 편이 안전하다.
특히 security처럼 browser/session symptom routing과 incident/recovery catalog가 한 README에 같이 있으면 아래처럼 읽게 해야 한다.

| category README subsection | 먼저 읽는 역할 | junior 첫 클릭 여부 | 혼동 방지 메모 |
|---|---|---|---|
| `Beginner Ladder`, `Troubleshooting Path`, symptom-first bridge | `primer` / `primer bridge` / `category navigator` | yes | 증상 문장을 `cookie-missing`, `server-anonymous`, `stale deny` 같은 route 이름으로 먼저 번역한다 |
| `Incident / Recovery / Trust`, `Failover`, `Cutover`, `Outage` | `playbook` / `drill` / `incident matrix` / `recovery` | no | beginner 질의라도 live incident가 아닌 이상 첫 클릭으로 올리지 않는다 |

즉 category README에서 `incident`, `recovery`, `cutover`, `failover`가 heading 앞쪽에 보이면 section 자체가 advanced follow-up shelf라는 뜻을 숨기지 말아야 한다.
반대로 `처음`, `뭐예요`, `헷갈려요`, `why`, `what is`, `basics` query를 받을 entry shelf는 heading 첫 문장에서 `primer`인지 `primer bridge`인지 바로 드러내는 편이 retrieval과 사람이 같이 덜 헤맨다.
...
- mixed category snapshot에서는 "이 shelf는 follow-up"이라는 경고를 한 줄로라도 남긴다. 예: ``[playbook]`과 runtime diagnostics는 entrypoint가 아니라 follow-up shelf`처럼 role warning을 숨기지 않는다.
...
## Symptom-First Section Labels

- symptom-first section rule: `증상별 바로 가기`, `Quick Routes`, `Troubleshooting Path` 같은 heading은 이름만으로는 incident entrypoint처럼 읽힐 수 있다. 이런 section을 mixed README에 남길 때는 첫 문장이나 바로 아래 표에서 역할을 먼저 못 박는다.

| 섹션에서 먼저 보이는 말 | 기본 역할로 먼저 읽기 | 바로 드러나야 하는 safety cue |
|---|---|---|
| `증상별 바로 가기`, `Quick Routes`, `Troubleshooting Path` | symptom-first `catalog` 또는 `primer bridge` | `이 표는 survey가 아니라 symptom-first catalog` |
| `outage`, `blast radius`, `failover`, `recovery`, `runbook` | incident-first follow-up | `beginner first click 아님` |
| 둘이 같은 섹션에 섞여 있음 | row-level badge 반복 | `catalog -> primer/primer bridge -> deep dive/playbook/recovery` |

- beginner-first reading rule: symptom section 자체는 beginner-safe할 수 있지만, 그 안의 incident row는 같은 shelf가 아니다. `처음`, `헷갈려요`, `왜`, `뭐예요`, `basics` 질문으로 들어오면 같은 section 안에서도 `primer` / `primer bridge` row가 먼저 보여야 하고, `playbook` / `recovery` row는 follow-up임이 즉시 드러나야 한다.
...
README 맨 위 `관련 문서:` 블록도 beginner가 첫 클릭을 정하는 shelf다.
그래서 링크를 많이 넣더라도 아래 순서가 덜 헷갈린다.

1. `primer route`, `primer`, `primer bridge`
2. `category navigator`, `cross-category bridge`, `safe return`
3. `deep dive follow-up`, `playbook`, `recovery`, `question bank`

같은 블록에서 `primer`와 `recovery`를 섞어야 한다면, label을 링크 앞쪽에 남겨 "입문용 첫 클릭"과 "운영형 follow-up"이 한눈에 갈리게 둔다.
예를 들어 `deadlock`, `timeout`, `cutover`, `incident`처럼 advanced signal이 제목 초반에 보이면 `[deep dive follow-up]`나 `[playbook]` cue를 숨기지 않는 편이 beginner-safe하다.
...
## README clarity audit checklist (`entrypoint safety`)
...
broad beginner query에서는 `survey` / `primer` / `primer bridge`를 먼저 고르고, `playbook` / `recovery` / `master note`는 첫 클릭으로 올리지 않는다는 규칙을 이 표에도 그대로 적용한다.
...
`README`, `navigator`, `taxonomy`가 검색 결과에 같이 보이면 더 짧게 아래처럼 잘라도 된다.

| 먼저 보인 라벨 | 이때 읽는 역할 | beginner 첫 클릭 여부 |
|---|---|---|
| `survey`, `roadmap` | 큰 그림 / 학습 순서 | yes |
| `README`, `navigator`, `taxonomy` | 역할 판별 / next-step routing | yes, but routing only |
| `primer`, `basics`, `intro` | 첫 개념 설명 | yes |
| `primer bridge`, `헷갈려요`, `why`, symptom split | 첫 분기 정리 | yes |
| `deep dive`, `playbook`, `recovery`, `master note` | follow-up 심화 / 운영 / 종합 | no |
