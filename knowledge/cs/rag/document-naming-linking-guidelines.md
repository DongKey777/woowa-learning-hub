...
- review 전에 markdown/HTML asset target이 실제로 존재하는지 먼저 보려면 repo root에서 `python docs/local_asset_existence_lint.py`를 먼저 돌린다.
- review 전에 local `.md` target이나 `#anchor` slug가 함께 살아 있는지 보려면 `python docs/local_doc_target_lint.py`를 함께 돌린다.
...
#### rename wave QA order note

asset basename/path rename을 한 wave로 밀 때는 아래 네 체크를 같은 흐름으로 묶어 두는 편이 가장 덜 헷갈린다.

1. 문서 안 fenced example도 함께 손볼 예정이면 먼저 `python docs/link_fence_false_link_check.py <touched-path>`로 literal markdown noise를 비운다.
2. local `.md` path나 heading slug를 건드렸다면 `python docs/local_doc_target_lint.py <touched-path>`로 beginner가 따라갈 다음 문서/복귀 링크가 아직 살아 있는지 먼저 본다.
3. rename 뒤 실제로 link된 새 asset path 이름이 scanner-safe한지 `python docs/asset_filename_lint.py <touched-path>`로 본다.
4. old basename/path가 `knowledge/cs/`와 `docs/`에 남았는지 `rg -n --glob '*.md' -F '<old-basename.ext>' knowledge/cs docs`와 `python docs/stale_asset_reverse_link_check.py <touched-path>`로 바로 닫는다.

짧게 기억하면 `fence precheck -> doc target lint -> filename lint -> stale reverse-link sweep` 순서다.
첫 단계는 false positive를 줄이고, 둘째 단계는 local doc jump를 안전하게 지키고, 셋째 단계는 새 이름이 QA-safe한지 확인하고, 넷째 단계는 old path 잔존 link를 비운다.
...
- markdown/HTML이 가리키는 `code/` target 자체가 살아 있는지 먼저 보려면 `python docs/local_asset_existence_lint.py`를 같이 돌린다.
...
- review 전에 linked local asset target 존재 여부를 먼저 재점검하려면 `python docs/local_asset_existence_lint.py`를 돌린다.
