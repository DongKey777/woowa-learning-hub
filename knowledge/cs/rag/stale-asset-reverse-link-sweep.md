...
- asset basename은 바꿨는데 README, deep dive, HTML `src`/`href`/`srcset` 중 어디가 아직 old path를 들고 있는지 빠르게 보고 싶을 때
...
2. 같은 턴에 markdown link, image link, HTML `src` / `href` / `srcset`을 모두 새 path로 바꾼다.
