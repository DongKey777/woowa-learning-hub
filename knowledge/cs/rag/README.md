...
- login-loop canonical label이 root README, security README, RAG routing docs에서 같은 primer/bridge 순서와 child label(`cookie-missing` / `server-anonymous`)을 유지하는지 보고 싶으면 [Auth Ladder Label Parity Lint](./auth-ladder-label-parity-lint.md)를 먼저 돌린다.
