...
>
> 문서 역할: 이 문서는 operating-system 입문 축에서 `process vs thread`, `stack vs heap`, `virtual memory`, `context switch`, `scheduler basics`를 한 번에 다시 묶는 entrypoint primer다.
...
- [Process, Thread, Stack, Heap, Virtual Memory, Runnable, Blocked 분류 연습](./process-thread-memory-state-classification-practice-drill.md)
...
retrieval-anchor-keywords: process vs thread, stack vs heap, virtual memory basics, context switch basics, scheduler basics, what is process thread memory, address space basics, rss vs vsz, running ready blocked example, ready vs runnable difference, sleeping vs blocked difference, ready runnable sleeping 헷갈려요, 운영체제 처음, 왜 context switch 느려요, when scheduler runs
...
이 문서는 아래처럼 "운영체제 기초가 한꺼번에 섞여 보이는 질문"을 먼저 정리하려고 만든 primer다.

- "`process`와 `thread`가 뭐예요? 왜 따로 구분하죠?"
- "`stack`과 `heap`은 둘 다 메모리인데 언제 다르게 말하죠?"
- "`virtual memory`는 왜 필요한가요? RAM이 부족할 때만 쓰는 건가요?"
- "`context switch`와 `scheduler`는 무엇이 다르고 왜 같이 나오죠?"

질문이 "`왜 context switch 느려요?`", "`스레드를 늘렸더니 왜 더 버벅여요?`"처럼 비용 감각에서 시작해도 이 문서가 첫 entrypoint다. 여기서는 먼저 "CPU 전환 비용"과 "실제 원인 분기"를 분리하고, 더 깊은 진단은 다음 문서로 넘긴다.

질문이 이 네 갈래에 걸쳐 동시에 섞인다면, 개별 deep dive보다 이 문서를 먼저 읽는 편이 덜 헷갈린다.
...
처음에는 "프로세스는 집, 스레드는 그 안의 작업자" 비유가 도움이 되지만, 이 비유는 자원 격리와 공유 범위를 설명하는 데까지만 유효하다. 실제 스케줄링 대상, page table, 캐시 locality 같은 세부 동작은 비유만으로는 설명되지 않는다.
...
질문을 받았을 때 아래처럼 분기하면 운영체제 입문 문서를 고르기 쉽다.

| 질문 모양 | 먼저 잡아야 할 축 | 이 문서에서 바로 볼 구간 | 다음 문서 |
|-----------|-------------------|--------------------------|-----------|
| "프로세스와 스레드가 뭐가 달라요?" | 격리 vs 공유 | `## 1. 프로세스와 스레드` | [프로세스와 스레드 기초](./process-thread-basics.md) |
| "stack이랑 heap이 왜 따로 있죠?" | 스레드별 상태 vs 프로세스 공유 객체 | `## 3. 코드 / 데이터 / 힙 / 스택`, `## 4. stack vs heap` | [메모리 관리 기초](./memory-management-basics.md) |
| "가상 메모리는 왜 필요한가요?" | 주소 추상화 vs 실제 RAM | `## 2. 가상 메모리` | [가상 메모리 기초](./virtual-memory-basics.md) |
| "context switch가 왜 느리죠?", "왜 context switch 느려요?" | CPU 전환 비용 | `## 5. context switch` | [컨텍스트 스위칭, 데드락, lock-free](./context-switching-deadlock-lockfree.md) |
| "scheduler는 언제 누구를 고르죠?" | runnable 경쟁과 time slice | `## 6. scheduler basics`, `## 7. scheduler가 동시에 보는 목표와 time slice` | [CPU 스케줄링 기초](./cpu-scheduling-basics.md) |
...
여기서 초급자가 가장 많이 헷갈리는 용어가 `ready`, `runnable`, `sleeping`이다. 이 문서에서는 먼저 아래처럼 단순화해서 읽으면 된다.

- `ready`와 `runnable`: beginner 문맥에서는 거의 같은 뜻으로 쓴다. "지금 당장 실행은 가능하지만 CPU 차례를 기다릴 수 있는 상태"다.
- `sleeping`: beginner 문맥에서는 보통 `blocked` 또는 `waiting` 쪽으로 읽는다. 즉 CPU만 기다리는 게 아니라, I/O나 이벤트 완료를 기다리는 상태다.
- Linux 관찰 문맥에서는 단어가 더 세분된다. 예를 들어 `ps`의 `R`은 running 또는 runnable을 함께 가리키고, `S`/`D`는 둘 다 넓게는 sleeping 계열이지만 깨울 수 있는지 여부가 다르다.

즉 "`ready`랑 `runnable`이 뭐가 달라요?"라는 질문에는 "입문 설명에서는 거의 같은 말로 보되, Linux 표기에서는 `running`까지 묶일 수 있다"고 답하면 안전하다. "`sleeping`이 쉬는 중이라는 뜻인가요?"라는 질문에는 "보통은 놀고 있는 게 아니라 I/O나 락 같은 외부 조건을 기다리는 blocked/waiting 상태"라고 잡아 주는 편이 정확하다.
...
>
> - "`process/thread/stack/heap/runnable` 분류를 짧게 한 번 더 해 보고 싶다"면: [Process, Thread, Stack, Heap, Virtual Memory, Runnable, Blocked 분류 연습](./process-thread-memory-state-classification-practice-drill.md)
