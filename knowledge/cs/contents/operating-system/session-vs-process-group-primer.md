...
retrieval-anchor-keywords: session vs process group primer, session process group basics, job control basics, process group job control, controlling terminal basics, foreground process group, terminal foreground pgid, Ctrl-C process group, terminal generated signals, SIGINT foreground process group, SIGTSTP foreground process group, setpgid basics, setsid basics, setsid vs setpgid, process group vs session, session leader, process group leader, PGID SID TPGID, posix_spawn process group follow-up, POSIX_SPAWN_SETPGROUP, tcsetpgrp basics, daemon detach terminal basics
...
- `posix_spawnattr_t`에서 process group / signal mask / signal default가 왜 같이 나오는지 다시 보려면 [posix_spawn Attributes Primer](./posix-spawn-attributes-primer.md)
- subprocess의 fd 상속, pipe EOF, redirect가 헷갈리면 [Subprocess FD Hygiene Basics](./subprocess-fd-hygiene-basics.md)
- graceful shutdown과 supervisor signal 감각을 더 넓히려면 [signals, process supervision](./signals-process-supervision.md)
- container PID 1에서 signal forwarding과 reaping을 이어 보려면 [PID 1, SIGTERM, and Container Reaping Basics](./container-pid-1-sigterm-zombie-reaping-basics.md)
