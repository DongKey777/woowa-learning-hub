...
| process / scheduler / memory 기초 축 복습 | `primer` | [Process, Thread, Virtual Memory, Context Switch, Scheduler Basics](./process-thread-virtual-memory-context-switch-scheduler-basics.md), [Scheduler Observation Starter Guide](./scheduler-observation-starter-guide.md), [Demand Paging and Page Fault Primer](./demand-paging-page-fault-primer.md), [TLB and Page Table Walk Bridge](./tlb-page-table-walk-bridge.md), [Process Lifecycle and IPC Basics](./process-lifecycle-and-ipc-basics.md), [PID 1, SIGTERM, and Container Reaping Basics](./container-pid-1-sigterm-zombie-reaping-basics.md), [Subprocess FD Hygiene Basics](./subprocess-fd-hygiene-basics.md), [Subprocess Pipe Backpressure Primer](./subprocess-pipe-backpressure-primer.md), [popen and Runtime Wrapper Mapping](./popen-runtime-wrapper-mapping.md), [Stdio Buffering After Redirect](./stdio-buffering-after-redirect.md), [posix_spawn File Actions Primer](./posix-spawn-file-actions-primer.md), [posix_spawn Attributes Primer](./posix-spawn-attributes-primer.md), [Signal Mask vs Disposition Bridge: `fork()`, `exec()`, `posix_spawn()`](./signal-mask-vs-disposition-fork-exec-posix-spawn.md), [Session vs Process Group Primer](./session-vs-process-group-primer.md), [Process Spawn API Comparison: `fork()`, `vfork()`, `posix_spawn()`, `exec()`, `clone()`](./process-spawn-api-comparison.md), [`clone()` Flags Mental Model: Thread-Like, Process-Like, Namespace-Isolated](./clone-flags-thread-like-process-like-namespace-isolated.md), [입문 primer](#입문-primer) 아래 기본 섹션 |
...
아래 구간은 운영체제 기초 개념 축을 맞추는 `primer`다. Linux runtime symptom 기반 catalog로 내려가기 전에 process / scheduling / memory 기본을 빠르게 복습할 때 쓰면 된다.

- [Process, Thread, Virtual Memory, Context Switch, Scheduler Basics](./process-thread-virtual-memory-context-switch-scheduler-basics.md): process vs thread, stack vs heap, virtual memory, context switch, scheduler basics를 한 번에 묶은 입문 정리노트
- [Scheduler Observation Starter Guide](./scheduler-observation-starter-guide.md): scheduler basics 직후에 `load average`, `vmstat r`, `/proc/<pid>/sched`, 필요할 때만 `/proc/schedstat`, 마지막에 `runqlat`를 어떤 순서로 읽을지 잡아 주는 beginner bridge
- [Load Average Triage: CPU Saturation vs cgroup Throttling vs I/O Wait](./load-average-triage-cpu-saturation-cgroup-throttling-io-wait.md): high `load average`를 scheduler saturation으로 성급히 단정하지 않도록 CPU saturation, quota throttling, I/O wait를 1분 triage 기준으로 나누는 beginner bridge
- [Demand Paging and Page Fault Primer](./demand-paging-page-fault-primer.md): virtual memory 입문 다음 단계에서 demand paging, first touch, copy-on-write, `mmap()`, major/minor page fault를 한 흐름으로 묶는 intermediate bridge
- [Swap-In, Reclaim, and Major Fault Path Primer](./swap-in-reclaim-fault-path-primer.md): major fault를 reclaim history, swap-in, cold-page refault 문맥으로 읽게 만드는 intermediate bridge
- [TLB and Page Table Walk Bridge](./tlb-page-table-walk-bridge.md): demand paging primer 다음 단계에서 TLB hit/miss, hardware page-table walk, page fault, major/minor fault 분기 위치를 한 경로로 묶는 intermediate follow-up
- [Process Lifecycle and IPC Basics](./process-lifecycle-and-ipc-basics.md): primer 다음 단계에서 `fork()` / `exec()` / `waitpid()` 흐름, zombie vs orphan 차이, `pipe` / `socketpair` / shared memory / signal 계열 IPC 감각을 연결하는 beginner bridge
...
- [Subprocess FD Hygiene Basics](./subprocess-fd-hygiene-basics.md): `fork()` + `exec()` / `posix_spawn()` / high-level subprocess wrapper를 같은 기본 규칙으로 묶어 stdout/stderr redirect, `pipe EOF`, `CLOEXEC`, leaked fd across `exec()`를 beginner-safe하게 연결하는 follow-up
- [popen and Runtime Wrapper Mapping](./popen-runtime-wrapper-mapping.md): `popen()`과 language-level subprocess 옵션(`PIPE`, `close_fds`, `pass_fds`, `shell=True`)을 OS-level pipe, fd table, file actions mental model로 번역하는 beginner bridge
- [Stdio Buffering After Redirect](./stdio-buffering-after-redirect.md): stdout/stderr redirect 배선은 맞는데 child 출력이 pipe/file redirection 뒤 늦게 보일 때 fd wiring과 stdio buffering을 분리해 설명하는 beginner bridge
