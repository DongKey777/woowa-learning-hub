...
| process / scheduler / memory 기초 축 복습 | `primer` | [Process, Thread, Virtual Memory, Context Switch, Scheduler Basics](./process-thread-virtual-memory-context-switch-scheduler-basics.md), [Scheduler Observation Starter Guide](./scheduler-observation-starter-guide.md), [Demand Paging and Page Fault Primer](./demand-paging-page-fault-primer.md), [TLB and Page Table Walk Bridge](./tlb-page-table-walk-bridge.md), [Process Lifecycle and IPC Basics](./process-lifecycle-and-ipc-basics.md), [PID 1, SIGTERM, and Container Reaping Basics](./container-pid-1-sigterm-zombie-reaping-basics.md), [Subprocess FD Hygiene Basics](./subprocess-fd-hygiene-basics.md), [Process Spawn API Comparison: `fork()`, `vfork()`, `posix_spawn()`, `exec()`, `clone()`](./process-spawn-api-comparison.md), [`clone()` Flags Mental Model: Thread-Like, Process-Like, Namespace-Isolated](./clone-flags-thread-like-process-like-namespace-isolated.md), [입문 primer](#입문-primer) 아래 기본 섹션 |
...
- [Subprocess FD Hygiene Basics](./subprocess-fd-hygiene-basics.md): `fork()` / `exec()` 기본 뒤에 subprocess stdout/stderr redirect, `pipe EOF`, `CLOEXEC`, leaked fd across `exec()`를 beginner-safe rule로 고정하는 follow-up
