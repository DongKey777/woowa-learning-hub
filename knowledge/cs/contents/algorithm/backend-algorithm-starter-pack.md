...
|---|---|---|
| "queue가 보이는데 BFS인지 worker handoff인지 모르겠어요" | 백엔드 미션에서 자료구조와 알고리즘 문맥이 섞인 코드 읽기 | 최소 이동 계산인지, 작업 순서 처리인지 먼저 자른다 |
| "정렬, 이분 탐색, BFS, DP 중 무엇을 의심해야 할지 모르겠어요" | 요구사항을 알고리즘 패턴으로 처음 번역하는 단계 | 입력 크기, 답의 모양, 상태 변화 방식으로 첫 분기를 만든다 |
| "코딩테스트가 아니라 미션 코드에서 알고리즘 감각을 어디까지 써야 하나요?" | 서비스 책임과 계산 규칙이 같이 보이는 리뷰 | 계산 질문과 레이어 책임 질문을 분리한 뒤 필요한 패턴만 고른다 |

관련 문서:

- [algorithm README - 초보자 첫 분기](./README.md#초보자-첫-분기)
...
retrieval-anchor-keywords: backend algorithm starter pack, algorithm triage checklist, algorithm what to study first, sorting basics, binary search beginner, dfs bfs beginner, greedy beginner, dp beginner, shortest path basics, 알고리즘 뭐부터 공부, 백엔드 알고리즘 입문, 처음 알고리즘 공부, service 코드인데 queue가 보여요, queue가 bfs인지 worker handoff인지 헷갈려요, 계산 질문인지 책임 질문인지 헷갈려요

## 길 잃었을 때 복귀

이 primer는 입구 문서다. 읽다가 다시 `처음`, `왜`, `헷갈려요` 상태가 되면 아래처럼 category README나 옆 카테고리 primer로 한 칸만 복귀하면 된다.

| 지금 다시 막히는 문장 | 여기로 되돌아오기 | 바로 다음 안전한 한 걸음 |
|---|---|---|
| `정렬인지 이분 탐색인지 BFS인지 아직도 모르겠어요` | [algorithm README - 초보자 첫 분기](./README.md#초보자-첫-분기) | [시간복잡도 입문](./time-complexity-intro.md)이나 [DFS와 BFS 입문](./dfs-bfs-intro.md) 중 한 장만 고른다 |
| `queue가 보여서 알고리즘인지 자료구조인지 다시 헷갈려요` | [algorithm README - BFS, Queue, Map 먼저 분리하기](./README.md#bfs-queue-map-먼저-분리하기) | [큐 기초](../data-structure/queue-basics.md) -> [DFS와 BFS 입문](./dfs-bfs-intro.md) |
| `문제 풀이가 아니라 Service/worker 흐름 같아요` | [Software Engineering README - 연결해서 보면 좋은 문서](../software-engineering/README.md#연결해서-보면-좋은-문서-cross-category-bridge) | [Service 계층 기초](../software-engineering/service-layer-basics.md) |
| `처음 백엔드 미션 흐름을 읽는데 queue가 먼저 보여요` | [우테코 백엔드 미션 선행 개념 입문](../software-engineering/woowacourse-backend-mission-prerequisite-primer.md) | [Backend Data-Structure Starter Pack](../data-structure/backend-data-structure-starter-pack.md) -> [Service 계층 기초](../software-engineering/service-layer-basics.md) |
...
| `문제 풀이가 아니라 Service/worker 흐름인데 queue가 나와요` | [Backend Data-Structure Starter Pack](../data-structure/backend-data-structure-starter-pack.md) | [Service 계층 기초](../software-engineering/service-layer-basics.md) -> [Ports and Adapters Beginner Primer](../software-engineering/ports-and-adapters-beginner-primer.md) | [Software Engineering README - 30초 starter 지도](../software-engineering/README.md#30초-starter-지도) |
