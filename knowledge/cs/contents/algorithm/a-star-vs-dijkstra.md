...
contextual_chunk_prefix: |
  이 문서는 목표 정점이 분명한 길찾기에서 전체 거리표 대신 한 목적지까지
  빨리 가고 싶은 학습자가 A*와 Dijkstra 중 무엇을 고를지 결정하는
  chooser다. 지도처럼 도착점을 향해 탐색을 몰아도 되나, 휴리스틱이
  빗나가면 얼마나 손해인가, 출발점 기준 전체 비용표가 필요한가, 한 쌍의
  경로만 찾으면 되나 같은 자연어 paraphrase가 본 문서의 선택 경계에
  매핑된다.
---
# A* vs Dijkstra
