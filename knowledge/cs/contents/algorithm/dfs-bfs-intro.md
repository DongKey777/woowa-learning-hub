...
> 한 줄 요약: DFS는 한 갈래를 끝까지 파고드는 탐색이고, BFS는 레벨 순서로 가까운 노드부터 넓히는 탐색이며, beginner는 `연결 확인`과 `최소 이동 횟수`를 먼저 구분하면 된다.
...
retrieval-anchor-keywords: dfs bfs 입문, dfs bfs 차이, bfs shortest path beginner, 최소 이동 횟수, bfs queue, bfs level order, bfs는 레벨 순서 탐색, queue는 fifo 도구, dfs bfs 헷갈림, tree dfs vs graph dfs, connected component beginner, 언제 bfs 써요, 처음 bfs 뭐예요, bfs랑 dfs 뭐가 달라요, 왜 bfs예요
...
|---|---|---|
| `갈 수 있나?`, `같은 그룹인가?` | 연결 확인이 먼저다 | [Connectivity Question Router](../data-structure/connectivity-question-router.md) |
...
`DFS냐 BFS냐`는 구현 취향보다 `무엇을 답해야 하나`를 자른 뒤에 붙는 판단이다.
...
그래서 `갈 수 있나?`처럼 방문 자체가 중요하면 둘 다 후보가 될 수 있고, `최소 몇 번 만에 가나?`처럼 간선 수 최소가 중요하면 BFS가 먼저 나온다. 이때 `queue는 FIFO 순서를 만드는 도구`이고, `BFS는 그 도구를 써서 레벨 순서로 넓히는 탐색`이다.
