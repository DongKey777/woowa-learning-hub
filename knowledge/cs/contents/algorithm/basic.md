...
상각 분석은 resize, rehash, 확장 같은 드문 비용을 전체 연산에 분산해 보는 관점이다.
하지만 코딩테스트의 O 표기와 서비스의 p99 지연 시간은 다르므로, 둘을 혼동하면 안 된다.
...
2. 다른 보석을 빼고 넣었을 때 이전 단계보다 낮은 가치를 가진다.
