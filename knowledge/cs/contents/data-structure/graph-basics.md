...
retrieval-anchor-keywords: graph basics, graph beginner, what is graph, 그래프 입문, 그래프가 뭐예요, vertex edge, graph vs tree, 처음 graph, 그래프 헷갈림, 왜 미로도 그래프

## 점과 선으로 먼저 보기

그래프는 `정점(vertex)`과 `간선(edge)`의 집합이다. 처음에는 용어를 다 외우기보다 아래 두 줄로 번역하면 충분하다.

- 점: 사람, 역, 페이지, 칸처럼 `상태 하나`
- 선: 친구 관계, 이동 가능, 링크처럼 `상태 사이 연결 규칙`
...
| `queue` | FIFO 순서를 만드는 도구 | 그 도구가 BFS용인지 작업용인지 | [DFS와 BFS 입문](../algorithm/dfs-bfs-intro.md), [큐 기초](./queue-basics.md) |
...
이 예시에서 `queue`는 아직 그래프 자체가 아니다. `가까운 역부터 넓게 본다`가 필요할 때 BFS 구현 도구로 붙는 것이다. 그래서 `그래프`, `BFS`, `queue`를 같은 층위로 읽지 않는 것이 중요하다.
