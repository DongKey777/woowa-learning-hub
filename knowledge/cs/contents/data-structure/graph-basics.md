---
schema_version: 3
title: 그래프 기초 (Graph Basics)
concept_id: data-structure/graph-basics
canonical: false
...
- graph-entrypoint
- graph-vs-tree-basics
- graph-vs-bfs-vs-queue-split
aliases:
- graph basics
- what is graph
- graph beginner
...
- 그래프가 뭐예요
- vertex edge basics
- graph vs tree beginner
- graph vs bfs vs queue
symptoms:
- 그래프가 구조인지 알고리즘인지부터 헷갈려서 BFS 문서를 읽어도 감이 안 잡혀
- 미로, 지하철, 친구 관계가 왜 다 그래프로 묶이는지 한 줄로 이해하고 싶어
- graph, bfs, queue가 같은 말처럼 보여서 처음 분기가 잘 안 된다
intents:
- definition
- comparison
prerequisites:
...
- contents/data-structure/queue-basics.md
expected_queries:
- 그래프가 뭐예요를 처음 배우는 사람 기준으로 설명해줘
- 미로와 지하철을 왜 그래프로 보는지 한 번에 이해하고 싶어
- graph, bfs, queue를 처음에 어떻게 분리해서 읽어야 해?
- 트리와 그래프는 어떤 관계인지 beginner 기준으로 알려줘
- 그래프 문제를 보면 무엇이 점이고 무엇이 선인지부터 어떻게 잡아?
contextual_chunk_prefix: |
  이 문서는 그래프를 처음 배우는 학습자가 graph, vertex, edge, tree,
  BFS, queue를 한꺼번에 같은 층위로 읽지 않도록 돕는 primer다. 그래프가
  뭐예요, 왜 미로도 그래프예요, 트리도 그래프인가요, graph와 BFS는 뭐가
  다르죠, 무엇이 점이고 무엇이 선인지 먼저 어떻게 잡죠 같은 자연어 질문을
  구조-질문-도구 세 단계로 분리하는 입문 맥락을 제공한다.
---
# 그래프 기초 (Graph Basics)
...
retrieval-anchor-keywords: graph basics, graph beginner, what is graph, 그래프 입문, 그래프가 뭐예요, vertex edge, graph vs tree beginner, 처음 graph, 그래프 헷갈림, 왜 미로도 그래프, bfs랑 graph 차이, 언제 graph로 읽어요, graph 다음 뭐 봐요, 그래프 다음 문서, mst 위상정렬 다익스트라 헷갈려요
...
| `트리랑 계속 섞여요` | [트리 기초](./tree-basics.md) |
| `MST`, `위상 정렬`, `flow`, `가중치 최단 경로`가 한꺼번에 보여요 | [그래프 관련 알고리즘](../algorithm/graph.md) |
