...
처음 읽는다면 용어보다 아래 세 질문을 먼저 자르면 된다.

| 먼저 확인할 것 | `예`면 떠올릴 뜻 | 다음 문서 |
|---|---|---|
| `상태 여러 개와 연결 규칙이 보이나?` | 그래프는 구조다 | 이 문서 |
| `갈 수 있나`, `같은 그룹인가`를 묻나? | 연결 여부 질문이다 | [Connectivity Question Router](./connectivity-question-router.md) |
| `최소 몇 번 만에 가나`를 묻나? | 구조 위에서 BFS를 붙일 수 있다 | [DFS와 BFS 입문](../algorithm/dfs-bfs-intro.md) |

`graph`, `bfs`, `queue`를 한 번에 봐도 먼저 답할 것은 항상 `무엇이 점이고, 무엇을 묻는가`다.
