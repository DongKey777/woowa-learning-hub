...
| 계층 | `루트에서 어떻게 내려가고 순회하나?` | [트리 기초](./tree-basics.md) |
...
- `왜 tree인데 graph 문서가 같이 나와요?`는 트리가 그래프의 한 종류라는 큰 그림을 놓쳤을 때 생긴다.
...
| `30분 빈 시간 있나?`, `양옆 예약 사이 길이는 어떻게 재죠?` | [TreeMap Gap Detection Mini Drill](./treemap-gap-detection-mini-drill.md) | gap 계산 |
