...
retrieval-anchor-keywords: data structure readme, beginner data structure routing, hashmap treemap linkedhashmap primer, queue deque priority queue, connectivity router, queue는 fifo 순서를 만드는 도구, bfs는 가까운 칸부터 한 겹씩 퍼지는 탐색, 자료구조 처음인데 뭐부터, map set queue tree 뭐부터, set map 차이 처음, queue deque 차이 처음, deque가 뭐예요, treemap 왜 써요, 범위 조회 자료구조 뭐부터, bfs랑 queue 차이 뭐예요

## 길 잃었을 때 복귀 경로

길을 잃으면 아래 표에서 다시 한 칸만 고르면 된다.
...
| `중복 제거는 되는데 왜 TreeSet/TreeMap이 또 필요한지 모르겠어요` | [증상별 빠른 길](#증상별-빠른-길) | [HashSet vs TreeSet Beginner Bridge](./hashset-vs-treeset-beginner-bridge.md) -> [HashMap, TreeMap, LinkedHashMap Primer](./hashmap-treemap-linkedhashmap-beginner-selection-primer.md) |
...
| `덱이 queue의 다른 이름인지, 양쪽에서 빼는 건지 헷갈려요` | [증상별 빠른 길](#증상별-빠른-길) | [덱 기초](./deque-basics.md) |
...
| `왜 lower랑 floor가 달라요?`, `exact match 포함 여부가 헷갈려요` | [증상별 빠른 길](#증상별-빠른-길) | [TreeSet Exact-Match Drill](./treeset-exact-match-drill.md) -> [TreeMap Neighbor-Query Micro Drill](./treemap-neighbor-query-micro-drill.md) |
...
짧게 외우면 `그래프는 구조`, `BFS/DFS는 탐색`, `queue는 FIFO 도구`다.

초보자에게는 `graph -> bfs -> queue` 순서가 덜 헷갈린다.

| 먼저 자를 것 | 지금 답해야 하는 질문 | 바로 다음 문서 |
|---|---|---|
| 구조 | `무엇이 점이고 무엇이 연결인가?` | [그래프 기초](./graph-basics.md) |
| 탐색 | `갈 수 있나, 최소 몇 번 만에 가나?` | [DFS와 BFS 입문](../algorithm/dfs-bfs-intro.md) |
| 도구 | `FIFO로 꺼내나? visited는 무엇으로 저장하나?` | [큐 기초](./queue-basics.md), [알고리즘 README - BFS, Queue, Map 먼저 분리하기](../algorithm/README.md#bfs-queue-map-먼저-분리하기) |

- `왜 queue인데 bfs예요?`는 탐색 질문을 도구 질문으로 잘못 읽은 경우가 많다.
- `왜 graph인데 queue 문서로 가요?`는 FIFO가 핵심일 때 생긴다.
...
`왜 queue인데 bfs예요`, `왜 graph인데 queue 문서가 먼저예요` 같은 symptom은 표에서 `연결`, `최소 이동`, `작업 순서`를 안 자를 때 생긴다.

처음 읽는 단계라면 `최단 경로`, `MST`, `위상 정렬`까지 한 번에 잡지 말고 위 세 갈래에서 한 칸만 고른다. 넓은 그래프 비교는 그다음에 [그래프 관련 알고리즘](../algorithm/graph.md)의 상단 라우터만 보고, 세부 구현은 [BFS vs Dijkstra shortest path mini card](../algorithm/bfs-vs-dijkstra-shortest-path-mini-card.md)나 [Minimum Spanning Tree: Prim vs Kruskal](../algorithm/minimum-spanning-tree-prim-vs-kruskal.md)로 다시 내려가면 된다.
...
같은 서비스 문장도 아래처럼 먼저 번역하면 beginner가 덜 헷갈린다.

| 지금 읽는 문장 | 실제로 먼저 고를 것 | 바로 다음 문서 |
|---|---|---|
| `창고와 도로가 어떻게 연결돼 있지?` | 연결 그림 자체 = 그래프 | [그래프 기초](./graph-basics.md) |
| `이 창고에서 저 창고로 갈 수 있지?` | yes/no 연결 여부 = connectivity | [Connectivity Question Router](./connectivity-question-router.md) |
| `어떻게 가는지 경로 하나만 보여줘` | path 복원 질문 | [Shortest Path Reconstruction Bridge](../algorithm/shortest-path-reconstruction-bridge.md) |
| `가장 가까운 창고부터 넓혀 보자` | 거리 순서 탐색 = BFS | [DFS와 BFS 입문](../algorithm/dfs-bfs-intro.md) |
| `들어온 배송 작업부터 처리하자` | FIFO 처리 도구 = queue | [큐 기초](./queue-basics.md) |

`왜 queue인데 bfs예요?`는 같은 장면을 `구조`, `탐색`, `도구`로 다시 자르면 대부분 풀린다.

### 질문이 바뀌면 출발점도 바뀐다

질문 문장이 바뀌면 출발점도 바로 달라진다.

| 같은 그래프 장면에서 바뀐 질문 | 먼저 필요한 답 | 첫 출발점 |
|---|---|---|
| `갈 수 있나?` | yes/no | [Connectivity Question Router](./connectivity-question-router.md) |
| `어떻게 가나?` | 경로 하나 | [Shortest Path Reconstruction Bridge](../algorithm/shortest-path-reconstruction-bridge.md) |
| `가장 짧게 가나?` | 최소 간선 수 | [DFS와 BFS 입문](../algorithm/dfs-bfs-intro.md) |

## 증상별 빠른 길

처음엔 자료구조 이름을 다 외우기보다, 지금 막힌 문장을 beginner bridge에 직접 붙이는 편이 더 빠르다.
아래는 `set/map/queue/deque/treemap` 계열에서 초급자가 가장 자주 하는 첫 질문 라우터다.
...
| `대상자 선택 문제에서 set이랑 bitmap 중 뭐예요?`, `audience segment 계산은 어떤 쪽으로 먼저 자르죠?` | [Set vs Bitmap Audience Selection Mini Drill](./set-vs-bitmap-audience-selection-mini-drill.md) | audience-selection 문장에 decision table을 바로 적용하는 4문항 셀프 체크를 제공한다 |
...
| `audience selection에서 set인지 bitmap인지 감이 안 와요`, `세그먼트 대상자 계산은 어떤 구조로 시작하죠?` | [Set vs Bitmap Audience Selection Mini Drill](./set-vs-bitmap-audience-selection-mini-drill.md) | 세그먼트 대상 계산 문장을 `membership only`와 `set algebra`로 바로 다시 자르게 돕는다 |
...
| `문자열 key에서 startsWith랑 사전순 범위가 같이 보여요`, `다음 문자열 key를 찾는 건 trie예요 treemap예요` | [Trie Prefix Search vs TreeMap Ordered Map Beginner Card](./trie-prefix-search-vs-treemap-ordered-map-beginner-card.md) | `Trie`와 ordered map의 질문 축을 분리한다 |
...
| `문자열 key의 다음 값`, `사전순 범위`, `prefix 후보`가 한 문제에 같이 보여요 | [Trie Prefix Search vs TreeMap Ordered Map Beginner Card](./trie-prefix-search-vs-treemap-ordered-map-beginner-card.md) | 문자열 key 위에서 prefix와 ordered range를 먼저 나눈다 |
...
예약표나 정렬된 key에서 양옆 이웃을 읽는 순간부터는 아래 줄만 따로 보는 편이 빠르다.

| 지금 문제에서 가장 먼저 보이는 문장 | 먼저 볼 문서 | 왜 여기서 시작하나 |
|---|---|---|
| `다음 예약 시간`, `범위 조회` | [TreeMap Interval Entry Primer](./treemap-interval-entry-primer.md) | 이웃/범위 탐색 |
| `왜 lower랑 floor가 달라요?`, `TreeSet에서는 알겠는데 TreeMap으로 가면 또 헷갈려요` | [TreeSet Exact-Match Drill](./treeset-exact-match-drill.md) -> [TreeMap Neighbor-Query Micro Drill](./treemap-neighbor-query-micro-drill.md) -> [TreeMap Key/Entry Strictness Bridge](./treemap-key-entry-strictness-bridge.md) | set-only 직관을 ordered map으로 옮긴다 |
| `"[start, end)"가 뭐예요?`, `끝과 다음 시작이 같으면 겹치나요?` | [Reservation Interval Half-Open Boundary Card](./reservation-interval-half-open-boundary-card.md) -> [TreeMap Interval Entry Primer](./treemap-interval-entry-primer.md) | 예약 경계 해석 |
| `30분 빈 시간 있나?`, `양옆 예약 사이 길이는 어떻게 재죠?` | [TreeMap Gap Detection Mini Drill](./treemap-gap-detection-mini-drill.md) | `floorEntry`/`ceilingEntry` 결과를 free-slot 길이 판정으로 바꾼다 |
...
| `firstKey랑 firstEntry 차이가 뭐예요?`, `floorKey랑 floorEntry는 반환이 어떻게 달라요?` | [TreeMap `firstKey` vs `firstEntry`, `floorKey` vs `floorEntry` Return-Shape Card](./treemap-firstkey-firstentry-floorkey-floorentry-return-shape-card.md) | key-only와 entry 반환 shape, empty/null 반응을 한 장에서 같이 자른다 |
...
| `최단 경로, MST, 위상 정렬이 한꺼번에 섞인다` | [그래프 기초](./graph-basics.md) -> [DFS와 BFS 입문](../algorithm/dfs-bfs-intro.md) -> [그래프 관련 알고리즘](../algorithm/graph.md) | 그래프 그림과 탐색 질문을 먼저 자른 뒤, `graph.md`에서는 상단 라우터만 보고 세부 구현은 전용 follow-up 문서로 넘기는 편이 beginner-safe 하다 |
...
- `그래프 기초`에서 바로 다익스트라나 MST로 들어가면 초보자 범위를 넘기 쉽다. `그래프 기초 -> DFS와 BFS 입문`까지 본 뒤에도 안 잘릴 때만 [그래프 관련 알고리즘](../algorithm/graph.md)의 상단 라우터로 넘기고, 구현 비교는 다시 전용 follow-up 문서로 내려가는 편이 낫다.
...
예약/캘린더/gap-check 문제에서 `lower/floor/ceiling/higher`가 아직 손에 안 붙으면 먼저 [TreeSet Exact-Match Drill](treeset-exact-match-drill.md)에서 값만 있는 정렬 줄로 `strict/inclusive`를 떼어 연습하고, exact match 다음으로 `subSet/headSet/tailSet` 같은 단순 range slicing을 붙이고 싶다면 [TreeSet Range View Mini Drill](treeset-range-view-mini-drill.md)에서 `앞쪽 창`, `뒤쪽 창`, `가운데 창`을 먼저 손으로 자른다. 그다음 [TreeMap Neighbor-Query Micro Drill](treemap-neighbor-query-micro-drill.md)에서 6개 일정 예제로 `왼쪽/오른쪽` 감각을 예약표로 옮겨 보는 편이 덜 막힌다. 여기서 `끝 시각과 다음 시작 시각이 같으면 겹치나요?`, `[start, end)`가 뭐예요?가 먼저 막히면 [Reservation Interval Half-Open Boundary Card](reservation-interval-half-open-boundary-card.md)로 경계 규칙을 먼저 고정한 뒤, `lowerKey/floorKey/ceilingKey/higherKey`를 `lowerEntry/floorEntry/ceilingEntry/higherEntry`로 연결하는 한 장이 필요하면 [TreeMap Key/Entry Strictness Bridge](treemap-key-entry-strictness-bridge.md)를 끼워 넣고, 오전/오후 시간창만 먼저 잘라 보고 싶다면 [TreeMap `subMap` Schedule-Window Mini Drill](treemap-submap-schedule-window-mini-drill.md)에서 `오전 창`, `점심 포함 오후 창`, `점심 이후 창`을 손으로 예측해 본다. 그다음 [TreeMap `floorEntry`/`ceilingEntry` Value-Read Micro Drill](treemap-floorentry-ceilingentry-value-read-micro-drill.md)에서 `entry.getValue()`로 종료 시각 읽기를 붙이고, 질문이 바로 `30분 빈 시간 있나?`라면 [TreeMap Gap Detection Mini Drill](treemap-gap-detection-mini-drill.md)에서 free-slot 길이 판단을 한 번 더 손으로 맞힌 뒤, 마지막으로 [TreeMap Interval Entry Primer](treemap-interval-entry-primer.md)로 들어가 충돌 검사와 gap check를 함께 묶는 편이 덜 막힌다.
