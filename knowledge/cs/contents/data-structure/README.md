...
`ScheduledExecutorService`를 쓰면서 내부에서 작업이 어떻게 기다리는지 자료구조 그림이 안 잡히면 [ScheduledExecutorService vs DelayQueue Bridge](scheduledexecutorservice-vs-delayqueue-bridge.md)에서 `DelayQueue`의 head/deadline 감각을 `deadline ticket -> delayed work queue -> worker` 흐름으로 먼저 연결해두면 좋다.
`ScheduledFuture.cancel()`을 호출했는데 취소된 작업이 내부 queue에 남는지, `removeOnCancelPolicy`를 켜야 하는지 헷갈리면 [ScheduledFuture Cancellation Stale Entries](scheduledfuture-cancel-stale-entries.md)에서 실행 취소와 queue entry 제거를 먼저 분리해두면 좋다.
...
Java timer executor나 delayed task queue를 만들 때 `PriorityQueue`로 끝내도 되는지, `DelayQueue`가 필요한지 헷갈리면 [DelayQueue vs PriorityQueue Timer Pitfalls](delayqueue-vs-priorityqueue-timer-pitfalls.md)에서 `정렬`과 `delay-aware blocking`, cancellation stale-entry trade-off를 먼저 분리해두는 편이 안전하다.
...
- [ScheduledExecutorService vs DelayQueue Bridge](scheduledexecutorservice-vs-delayqueue-bridge.md) (`DelayQueue` head/deadline 감각과 scheduled executor worker queue 연결)
- [ScheduledFuture Cancellation Stale Entries](scheduledfuture-cancel-stale-entries.md) (`ScheduledFuture.cancel()`, lazy stale entry, `removeOnCancelPolicy` queue-cost trade-off)
...
- [DelayQueue vs PriorityQueue Timer Pitfalls](delayqueue-vs-priorityqueue-timer-pitfalls.md) (delay-aware blocking, cancellation, stale-entry timer trade-off)
