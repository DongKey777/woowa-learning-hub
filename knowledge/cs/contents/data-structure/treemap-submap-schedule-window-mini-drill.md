...
- [TreeMap `headMap`/`tailMap` 기본 포함 규칙 비교 카드](./treemap-headmap-tailmap-default-boundary-card.md)
...
retrieval-anchor-keywords: treemap submap mini drill, treemap submap beginner, schedule window treemap, morning afternoon schedule slice, treemap range query practice, submap 뭐예요, treemap submap 처음 배우기, range view before live view, booking conflict window, overlap gap check beginner, submap empty but conflict, treeset subset treemap submap same rule, 시작 포함 끝 제외 treemap, treeset range view to treemap submap bridge, submap inclusive exclusive 헷갈림
...
`TreeSet` range view를 먼저 익힌 사람이라면 더 단순하게 이어 붙이면 된다.

| `TreeSet`에서 읽던 문장 | `TreeMap`으로 옮긴 문장 | 변하지 않는 규칙 |
|---|---|---|
| `subSet(20, 50)` = `20` 이상 `50` 미만 | `subMap(09:00, true, 12:00, false)` = `09:00` 이상 `12:00` 미만 key | 기본형 감각은 `시작 포함, 끝 제외`다 |
| 값이 창 안에 들어오나 | key가 창 안에 들어오나 | 경계 포함 여부는 그대로 읽는다 |
| `headSet`, `tailSet`으로 앞/뒤 창 자르기 | `headMap`, `tailMap`으로 앞/뒤 창 자르기 | 이름만 `Set -> Map`으로 바뀐다 |

즉 beginner 기준 브리지 문장은 이것 하나면 된다.

> `TreeSet`에서 `subSet`을 "`시작 포함, 끝 제외` 창"으로 읽었다면, `TreeMap subMap`도 같은 창을 key 줄에 그린 뒤 value가 같이 따라온다고 보면 된다.
...
여기서 기본 규칙을 따로 고정하면 더 안전하다.

- Java `TreeMap` 기본형에서 `headMap(toKey)`는 끝 경계를 기본으로 제외한다
- Java `TreeMap` 기본형에서 `tailMap(fromKey)`는 시작 경계를 기본으로 포함한다
- 이 asymmetric default가 헷갈리면 [TreeMap `headMap`/`tailMap` 기본 포함 규칙 비교 카드](./treemap-headmap-tailmap-default-boundary-card.md)에서 `(-inf, to)`와 `[from, +inf)` 한 쌍으로 다시 보면 된다
