...
| "조인 결과가 많으면 `DISTINCT` 붙이면 해결" | 중복 원인, 잘못된 조인 키, 결과 단위 문제를 가릴 수 있다 | 먼저 [Result-row Explosion Debugging Checklist](./result-row-explosion-debugging-checklist.md)로 원인을 분리한다 |
...
- 조인 뒤 row가 왜 늘었는지 먼저 분리하려면 → [Result-row Explosion Debugging Checklist](./result-row-explosion-debugging-checklist.md)
