...
## 같은 코드에서 질문 3개를 분리하는 법

처음엔 한 메서드 안에 `@Transactional`, `save()`, `@Entity`가 같이 보여서 전부 한 설명처럼 느껴지기 쉽다. beginner-first 해석은 아래처럼 질문을 나눠 읽는 것이다.
...
| 코드 단서 | 지금 붙일 첫 질문 | 바로 갈 문서 |
|---|---|---|
| `@Transactional` | "어디까지 같이 commit/rollback하지?" | [트랜잭션 기초](./transaction-basics.md) |
| `new Member(email)` / `@Entity` | "무엇을 저장하려는 거지?" | [JDBC · JPA · MyBatis 기초](./jdbc-jpa-mybatis-basics.md), [SQL 읽기와 관계형 모델링 기초](./sql-reading-relational-modeling-primer.md) |
| `memberRepository.save(member)` | "SQL은 여기서 직접 보이나, 뒤에서 만들어지나?" | [JDBC · JPA · MyBatis 기초](./jdbc-jpa-mybatis-basics.md) |

핵심은 `트랜잭션 경계`, `저장 대상`, `SQL 위치`를 같은 질문으로 읽지 않는 것이다.
...
| "`WHERE` 조건 하나인데 왜 느리죠?" | [인덱스 기초](./index-basics.md) | [인덱스와 실행 계획](./index-and-explain.md) | `Using filesort`, 통계, optimizer trace 같은 실제 실행 계획 단서가 보일 때 |
...
| "`WHERE` 조건 하나인데 왜 느리죠?" | [인덱스 기초](./index-basics.md) | [인덱스와 실행 계획](./index-and-explain.md) | `Using filesort`, 통계, 튜닝 체크리스트처럼 `EXPLAIN` 결과가 실제로 보일 때 |
