...
처음인데 `deadlock`, `retry`, `failover`, `cdc` 같은 단어가 먼저 보여도 여기서 다 해결하려 하지 않는다. 이 브리지의 목표는 "지금 무슨 축의 질문인지"만 고정하는 것이다.
...
|---|---|---|---|
| "`save()` 뒤 SQL이 어디서 만들어져요?" | [JDBC · JPA · MyBatis 기초](./jdbc-jpa-mybatis-basics.md) | [JDBC, JPA, MyBatis 심화](./jdbc-jpa-mybatis.md) | `flush`, `OSIV`, connection pool 점유 시간처럼 런타임 해석이 실제로 필요할 때 |
| "`왜 같이 rollback돼요?`", "`어디까지 같이 실패해요?`" | [트랜잭션 기초](./transaction-basics.md) | [트랜잭션 격리 수준 기초](./transaction-isolation-basics.md) | `deadlock`, `40001`, `lock wait`처럼 실제 충돌 증상이 붙을 때 |
...
- `playbook`, `runbook`, `failover`, `cdc`, `replay`, `deadlock case study`가 먼저 보이면 이 문서의 primer/follow-up까지만 읽고 멈춘다.
...
| "`save()`만 보여서 SQL이 어디서 만들어지는지 모르겠어요" | [JDBC · JPA · MyBatis 기초](./jdbc-jpa-mybatis-basics.md) | [JDBC, JPA, MyBatis 심화](./jdbc-jpa-mybatis.md) 또는 [JDBC 실전 코드 패턴](./jdbc-code-patterns.md) | `flush`, OSIV, pool 점유 시간처럼 런타임 해석이 필요할 때 |
