...
- [PostgreSQL `Buffers: shared hit/read/dirtied/written`는 뭐예요?](./postgresql-buffers-hit-read-dirtied-written-mini-card.md)
...
| `shared read`는 비슷한데 `shared hit`만 커지고 시간이 늘었다 | disk read보다 page revisit, heap check, memory touch 비용이 커졌을 수 있다 | [PostgreSQL `Buffers: shared hit/read/dirtied/written`는 뭐예요?](./postgresql-buffers-hit-read-dirtied-written-mini-card.md), [PostgreSQL `Index Only Scan`인데 왜 `Heap Fetches`가 남아요?](./postgresql-index-only-scan-heap-fetches-beginner-card.md) |
| `Buffers: shared hit/read/dirtied/written` 숫자 자체가 낯설다 | 같은 `Buffers` 줄 안에서도 읽기 숫자와 쓰기 숫자를 나눠 봐야 한다 | [PostgreSQL `Buffers: shared hit/read/dirtied/written`는 뭐예요?](./postgresql-buffers-hit-read-dirtied-written-mini-card.md) |
...
## `Heap Fetches`가 늘면 `buffers`와 시간도 꼭 같이 오르나요?

꼭 그렇지는 않다.
`Heap Fetches`는 "heap 재확인이 몇 번 있었나"를, `buffers`는 "그 과정에서 page를 얼마나 만졌나"를, 실행 시간은 "그 모든 비용이 실제로 얼마나 느렸나"를 보여 준다.
그래서 셋은 자주 같이 움직이지만, PostgreSQL cache 상태와 다른 plan node 비용 때문에 1:1로 묶이지는 않는다.

| 보인 패턴 | beginner용 첫 해석 | 왜 이런 일이 생기나 |
| --- | --- | --- |
| `Heap Fetches` 증가 + `buffers` 증가 + 시간 증가 | heap 재확인이 실제 page touch와 지연으로 이어졌을 가능성이 크다 | 새 page를 더 만졌거나, shared read가 늘었거나, cache hit여도 재확인 양이 커졌을 수 있다 |
| `Heap Fetches` 증가 + `buffers`는 비슷 + 시간도 비슷 | heap 재확인은 늘었지만 같은 page 묶음 안에서 처리됐을 수 있다 | 이미 메모리에 있는 page를 다시 확인해 추가 latency가 작을 수 있다 |
| `Heap Fetches` 증가 + `buffers`는 조금 증가 + 시간은 거의 그대로 | "조금 더 확인했지만 아직 체감 병목은 아니다"에 가깝다 | shared hit 위주라 disk read가 없고, 다른 node 비용이 더 클 수 있다 |
| `Heap Fetches` 증가 + 시간 증가, 그런데 `buffers`는 거의 안 변함 | 느린 이유를 `Heap Fetches` 하나로 단정하면 위험하다 | CPU 경쟁, loop 증가, 상위 node 정렬/조인 비용 같은 다른 원인이 시간을 밀었을 수 있다 |

초보자에게는 아래 한 문장이 가장 안전하다.

> `Heap Fetches`는 "heap 재확인 횟수", `buffers`는 "page touch 양", 시간은 "최종 체감 비용"이라서 보통 관련은 있지만 항상 같이 오르지는 않는다.

특히 `Buffers: shared hit`가 대부분인 경우에는 `Heap Fetches`가 늘어도 disk I/O가 새로 생기지 않아 시간이 거의 안 움직일 수 있다.
반대로 `shared read`가 같이 뛰면 같은 `Heap Fetches` 증가라도 체감 지연이 더 크게 붙기 쉽다.
...
- `Heap Fetches`가 늘면 시간도 반드시 똑같이 늘어난다 -> 아니다. cache hit인지, 다른 node가 병목인지에 따라 같이 안 움직일 수 있다.
...
- `Buffers: shared hit/read/dirtied/written`를 각각 나눠 읽고 싶으면 [PostgreSQL `Buffers: shared hit/read/dirtied/written`는 뭐예요?](./postgresql-buffers-hit-read-dirtied-written-mini-card.md)
