...
관련 문서: [Read-Your-Writes와 Session Pinning 전략](./read-your-writes-session-pinning.md), [Monotonic Reads와 Session Guarantees](./monotonic-reads-session-guarantees.md), [Replica Lag Observability와 Routing SLO](./replica-lag-observability-routing-slo.md)
Retrieval anchors: `consistency token`, `client token`, `version token`, `freshness contract`, `session watermark`
