...
contextual_chunk_prefix: |
  이 문서는 *데이터베이스* 학습자가 여러 사람이 같은 *DB row*를 동시에
  바꾸려 할 때 충돌을 어떻게 막는지, *데이터베이스 동시성 제어*로서 lock이
  무엇이고 왜 필요한지 처음 잡는 primer다. 데이터베이스 동시 수정 충돌,
  여러 사람이 같은 DB 데이터 동시 변경, 같은 row 동시에 update, 데이터
  동시 변경 충돌 방지(git이나 코드 충돌이 아니라 DB row level), DB 락,
  optimistic vs pessimistic lock, shared vs exclusive row lock 같은
  자연어 paraphrase가 본 문서의 핵심 DB lock 개념에 매핑된다.
---
