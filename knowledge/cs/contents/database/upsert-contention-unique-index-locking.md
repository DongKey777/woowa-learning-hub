...
retrieval-anchor-keywords: upsert, insert on duplicate key update, on conflict do update, unique index arbitration, insert race, duplicate key retry, deadlock retry, idempotent write path, hot unique key, guard row upsert, ordered upsert plus lock, pre-seeded guard row, guard creation deadlock
