...
retrieval-anchor-keywords: causal consistency, happens-before, session causality, session consistency, causal order, cause before effect, effect before cause, dependency token, causal token, read-your-writes, monotonic reads, monotonic writes, comment visible before post, payment visible before order, role change seen without approval, 원인보다 결과가 먼저 보임, 인과 순서, 댓글만 먼저 보임
...
이 보장을 분산 요청에 실어 나르는 실무 도구가 dependency token, 즉 [Client Consistency Tokens](./client-consistency-tokens.md) 패턴이다.
