...
retrieval-anchor-keywords: postgresql serializable retry playbook, postgres serializable beginner, postgresql ssi retry, serializable snapshot isolation retry, sqlstate 40001 handling, serialization failure retry whole transaction, could not serialize access retry, read write dependencies among transactions, concurrent update 40001, spring serializable retry, @transactional serializable retry, transactiontemplate serializable retry, serializable read check write, serializable absence check insert, serializable count sum invariant, serializable retry envelope, postgres 40P01 vs 40001, postgresql service layer transaction retry, no retry inside same transaction, outbox after commit serializable
...
### 2. 동기 API인데 외부 부작용이 따라오는 command
