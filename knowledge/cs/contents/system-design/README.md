...
> retrieval-anchor-keywords: scim disable했는데 아직 접근돼요, deprovision 끝났는데 access tail이 남아요, 왜 어떤 pod는 막고 어떤 pod는 허용해요, cleanup evidence는 뭐가 있어야 해요, decision parity를 어디서 확인해요, mixed-version authority cutoff beginner, shadow evaluation beginner question, cleanup gate beginner route
...
- 초보자가 `deprovision 끝났는데 왜 어떤 pod는 막고 어떤 pod는 허용해요`, `decision parity를 어디서 확인해요`라고 묻는다면 system-design README는 runtime symptom을 mixed-version rollout과 parity gate 설계 질문으로 번역하는 입구다.
...
- 초보자가 `cleanup evidence는 뭐가 있어야 해요`, `shadow evaluation으로 뭘 비교해요`라고 묻는다면 이 subsection은 implementation 세부가 아니라 cleanup gate를 닫는 증거 체크리스트 입구라고 보면 된다.
