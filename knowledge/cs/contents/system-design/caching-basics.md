...
retrieval-anchor-keywords: caching basics, 캐시 입문, cache 뭐예요, 처음 cache 배우기, 왜 cache 쓰나요, 언제 cache 쓰면 안 되나요, cache hit miss 기초, ttl 캐시, 캐시 무효화 입문, read through cache aside, 캐시 레이어 기초, beginner caching, source of truth vs cache copy, stale read beginner, 왜 수정했는데 예전 값이 보여요
...
비유는 여기까지만 안전하다.
실제 시스템에서는 "복사본이 있으니 원본을 안 봐도 된다"가 아니라, **원본을 덜 보게 해 주지만 기준점은 여전히 원본**이라는 점이 더 중요하다.
...
|---|---|---|
| "cache가 뭐예요?" | 예 | 이 문서 |
| "언제 cache를 쓰고 언제 queue를 써요?" | 반쯤 예 | [Queue vs Cache vs DB Decision Drill](./queue-vs-cache-vs-db-decision-drill.md) |
...
처음엔 아래 질문으로 자르면 훨씬 덜 헷갈린다.

1. 같은 값을 정말 여러 번 읽는가?
2. 잠깐 예전 값이 보여도 설명 가능한가?
3. cache miss가 나도 DB에서 다시 읽어 복구 가능한가?

세 질문 중 `예`가 많을수록 cache 후보에 가깝다.
...
### 1분 예시: 상품 상세와 잔액 조회는 왜 다르게 보나

| 요청 | 기본 선택 | 이유 |
|---|---|---|
| 상품 상세 조회 | cache 후보 | 반복 조회가 많고, 잠깐 stale여도 치명상이 아닐 때가 많다 |
| 결제 직전 잔액 조회 | DB 직접 조회 우선 | stale 한 번이 중복 결제나 과다 사용으로 이어질 수 있다 |

둘 다 "조회"지만 최신성 비용이 다르기 때문에 같은 규칙을 쓰지 않는다.
초보자가 `조회는 다 cache 아닌가요?`라고 느끼면, 보통 이 차이를 아직 안 자른 상태다.
...
- **"TTL이 짧으면 최신성이 보장된다"**: TTL은 오래된 값을 `오래 두지 않게` 도와줄 뿐, write 직후 몇 초 안에 반드시 최신값이 보인다는 보장은 아니다.
...
여기서 Redis 같은 도구 이름은 "원격 캐시의 흔한 예시" 정도로만 보면 된다.
처음에는 특정 프레임워크 어노테이션보다 `원본 앞에 복사본을 둔다`는 구조와 `언제 stale이 위험한가`를 먼저 잡는 편이 안전하다.
...
처음 읽은 뒤 안전한 다음 한 칸은 보통 아래 둘 중 하나다.

- `왜 수정했는데 예전 값이 보이나요?`가 남으면 [Read-After-Write Consistency Basics](./read-after-write-consistency-basics.md)
- `캐시를 비울지 TTL로 버틸지`가 남으면 [Cache Invalidation Patterns Primer](./cache-invalidation-patterns-primer.md)
