...
로드밸런서를 잠시 접어 두고 `stateless app / cache / database / queue` 네 박스의 역할만 먼저 묶어 보고 싶다면 [Stateless 백엔드, 캐시, 데이터베이스, 큐 스타터 팩](./stateless-backend-cache-database-queue-starter-pack.md)부터 읽어도 된다.
...
처음에는 모든 문서를 다 읽기보다, 막히는 질문에 맞춰 한 갈래만 먼저 잡는 편이 빠르다.

| 지금 막히는 질문 | 먼저 읽을 문서 | 바로 다음 문서 |
|---|---|---|
| "박스 역할 자체가 헷갈린다" | [Stateless 백엔드, 캐시, 데이터베이스, 큐 스타터 팩](./stateless-backend-cache-database-queue-starter-pack.md) | [Request Path Failure Modes Primer](./request-path-failure-modes-primer.md) |
| "캐시는 왜 빠른데 위험하다고 하나" | [캐시 기초](./caching-basics.md) | [Cache Invalidation Patterns Primer](./cache-invalidation-patterns-primer.md) |
| "큐를 넣으면 어디까지 끝난 건지 모르겠다" | [메시지 큐 기초](./message-queue-basics.md) | [Consistency, Idempotency, and Async Workflow Foundations](./consistency-idempotency-async-workflow-foundations.md) |
| "sync/async 경계를 어떻게 나누지" | [Consistency, Idempotency, and Async Workflow Foundations](./consistency-idempotency-async-workflow-foundations.md) | [Read-After-Write Consistency Basics](./read-after-write-consistency-basics.md) |
...
이 흐름을 더 짧은 예시로 다시 잡고 싶다면 [메시지 큐 기초](./message-queue-basics.md), sync/async 경계와 duplicate 흡수까지 같이 보려면 [Consistency, Idempotency, and Async Workflow Foundations](./consistency-idempotency-async-workflow-foundations.md), 설계 심화는 [Job Queue 설계](./job-queue-design.md)로 이어진다.
...
- queue에 넣었다고 비즈니스 완료가 보장되는 것은 아니다
