...
retrieval-anchor-keywords: read-after-write consistency basics, read-after-write 뭐예요, replica lag basics, primary fallback, session stickiness, recent-write pinning, stale read after write, write then read stale, order status freshness, payment confirmation consistency, beginner consistency, 방금 쓴 값 안 보여요, 왜 저장했는데 안 보여요, how to read after write, read after write consistency basics basics
...
여기서 초보자가 먼저 구분해야 할 단어는 세 개만 잡아도 충분하다.

- replica lag: primary commit 후 replica가 변경을 따라오는 데 걸리는 시간
- primary fallback: 최신성이 중요한 읽기만 잠시 primary로 보내는 방식
- recent-write routing: 최근 write를 한 사용자 읽기만 더 신선한 경로로 잠깐 보내는 방식

핵심은 이것이다.

- 모든 읽기를 가장 신선한 경로로 강제하면 비싸다
...
처음 질문이 `왜 안 보여요?`일 때는 아래 두 줄부터 고르면 된다.

| 먼저 묻는 질문 | 첫 선택 | 이유 |
|---|---|---|
| 방금 저장한 직후 한두 화면만 보호하면 되나 | primary fallback | 구현이 단순하고 증상을 바로 줄이기 쉽다 |
| 저장 직후 내 화면만 잠깐 더 신선하면 되나 | recent-write routing | 전체 읽기를 비싸게 만들지 않고 최근 write 사용자만 보호할 수 있다 |
| 검색/피드처럼 약간 늦어도 되는가 | replica 기본 경로 유지 | 모든 화면을 강하게 읽을 필요는 없다 |
...
## recent-write routing은 그다음 카드다

### 4. recent-write routing은 load balancer의 sticky session과 다른 개념이다
...
| Read-after-write recent-write routing | 최근 write한 사용자의 읽기를 fresher path에 붙인다 | 방금 쓴 값을 다시 읽게 한다 |

read-after-write에서의 recent-write routing은 보통 아래처럼 쓴다.
...
이 표를 처음 읽을 때는 `strong consistency` 같은 더 넓은 분산 시스템 용어보다 `이 화면은 stale이면 사고인가, UX 문제인가`만 먼저 자르면 충분하다.
...
`가장 신선한 읽기를 전체 경로에 적용하자` — 최신성은 좋아질 수 있지만 latency와 primary load 비용이 함께 오른다.
...
이때는 전체 강한 읽기보다 **짧은 recent-write routing**이 잘 맞는다.
...
| Recent-write routing | 필요한 세션만 보호할 수 있다 | 라우팅 상태 관리가 필요하다 | 저장 후 redirect되는 UX 흐름 |
...
> read replica를 쓰면 replica lag 때문에 이 보장이 깨질 수 있어서, 실무에서는 모든 읽기를 가장 신선하게 만들기보다 최신성이 중요한 흐름만 primary fallback이나 recent-write routing으로 보호합니다.
> 특히 결제, 재고, 권한처럼 틀렸을 때 비용이 큰 경로는 strong read consistency가 더 중요하고, 검색이나 피드처럼 약간 stale해도 되는 경로는 replica나 cache를 유지하는 식으로 나눕니다.

## 꼬리질문

> Q: read-after-write consistency와 모든 화면의 강한 최신성 보장은 같은 말인가요?
> 의도: `내가 방금 쓴 값` 보호와 전체 시스템 정책을 구분하는지 확인
> 핵심: read-after-write는 보통 "내가 방금 쓴 값을 다시 읽는가"에 가깝고, 모든 화면을 가장 신선하게 읽는 정책은 더 넓고 비싼 선택이다.

> Q: recent-write routing과 sticky session은 왜 다르죠?
...
Read-after-write consistency는 replica lag를 제품 버그처럼 보이지 않게 다루는 설계 문제이며, 보통은 모든 읽기를 가장 신선하게 만들기보다 primary fallback과 recent-write routing을 필요한 흐름에만 선택적으로 적용한다.
