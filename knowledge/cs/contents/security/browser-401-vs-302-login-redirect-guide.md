...
retrieval-anchor-keywords: browser 401 vs 302, login redirect guide, browser login redirect, login loop beginner guide, saved request bounce, hidden jsessionid, cookie 있는데 다시 로그인, request cookie header empty, application cookie vs request cookie header, auth.example.com cookie stored app.example.com request cookie header missing, cookie header gate, secure cookie behind proxy, server-anonymous branch, browser session troubleshooting path, 처음 배우는데 뭐예요
...
| `302 Found` 뒤 최종 응답이 login HTML `200` | 첫 `302`에 있음 | API가 redirect를 따라가면서 login page를 받아 버렸을 수 있다 |
