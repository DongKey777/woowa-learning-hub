...
retrieval-anchor-keywords: session cookie jwt basics, 세션 쿠키 jwt 차이, 로그인 상태 유지, http stateless, 쿠키가 뭔가요, 세션이 뭔가요, jwt 뭐예요, 왜 로그인이 유지되나요, 쿠키 세션 jwt 헷갈림, jsessionid, 서버 세션 vs jwt, 토큰 기반 인증, security beginner session route, cookie session jwt next step, spring security 들어가기 전
...
처음엔 이 문서 하나로 끝내려 하기보다 `primer -> primer bridge -> deep dive` 사다리에서 지금 위치만 고정하는 편이 안전하다.

| 지금 막힌 질문 | 여기서 맡는 역할 | 다음 안전한 한 걸음 |
|---|---|---|
| `쿠키, 세션, JWT가 뭐가 다른지부터 헷갈려요` | 이 문서에서 기본 용어와 저장 위치를 맞춘다 | [Signed Cookies / Server Sessions / JWT Trade-offs](./signed-cookies-server-sessions-jwt-tradeoffs.md)로 넘어가 browser 저장 방식과 서버 상태 저장 방식을 비교한다 |
| `왜 로그인 후 상태가 유지되나요`, `JSESSIONID가 왜 보여요` | "브라우저가 다시 보내는 증거"와 "서버가 다시 찾는 상태"를 분리한다 | [Login Redirect, Hidden `JSESSIONID`, `SavedRequest` 입문](../network/login-redirect-hidden-jsessionid-savedrequest-primer.md)으로 가서 redirect, cookie, `SavedRequest`를 한 장면으로 본다 |
| `cookie 있는데 왜 다시 로그인돼요`, `API가 login HTML을 받아요` | 아직 Spring 원인 수사 전 단계다. 증상을 `redirect`, `cookie 전송`, `server 복원`으로 자른다 | [Browser `401` vs `302` Login Redirect Guide](./browser-401-vs-302-login-redirect-guide.md)에서 primer bridge를 먼저 탄다 |
| `Spring Security 문서에서 SavedRequest, SessionCreationPolicy가 갑자기 나와요` | Spring deep dive 전에 필요한 보안 primer를 끝낸 상태로 본다 | [Security README: Browser / Session Troubleshooting Path](./README.md#browser--session-troubleshooting-path)로 돌아가 branch를 고른 뒤에만 Spring deep dive로 내려간다 |
...
beginner 단계에서 login loop나 redirect incident를 이 문서 안에서 끝까지 파고들 필요는 없다. 먼저 증상을 한 줄로 자르고, 맞는 문서로 이동하는 편이 덜 헷갈린다.
...
이 문서의 역할은 원인 수사보다 "세션/쿠키/JWT가 각각 어떤 인증 증거인지"를 구분해 주는 데 있다. 증상이 붙는 순간에는 위 라우팅 문서로 넘기는 편이 scope가 맞다.
