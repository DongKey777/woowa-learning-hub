...
retrieval-anchor-keywords: jwt, claim set, signature validation, jwks, refresh token, revocation, jwt primer return, security readme return path, authz freshness before jwt deep dive, onceperrequestfilter beginner route, onceperrequestfilter safe next doc, spring filter example before jwt deep dive, 처음 배우는데 onceperrequestfilter, onceperrequestfilter 큰 그림, onceperrequestfilter 언제 쓰는지

---

## 처음 읽는다면

- `OncePerRequestFilter` 예시를 보고 이 문서에 들어왔다면, 지금 필요한 큰 그림은 filter 구현 디테일보다 `브라우저가 무엇을 보내고 서버가 무엇으로 로그인 상태를 복원하는가`일 가능성이 크다. 처음 배우는데 `JWT filter가 뭐예요`, `OncePerRequestFilter는 언제 쓰는지`가 먼저 궁금하면 [세션·쿠키·JWT 기초](./session-cookie-jwt-basics.md) -> [Security README 기본 primer 묶음](./README.md#기본-primer) 순서로 올라와 beginner route를 먼저 고정한다.
- `JWT가 그냥 로그인 문자열처럼 들린다`, `claim`, `signature`, `issuer`, `audience`가 아직 낯설면 [세션·쿠키·JWT 기초](./session-cookie-jwt-basics.md) -> [Signed Cookies / Server Sessions / JWT Tradeoffs](./signed-cookies-server-sessions-jwt-tradeoffs.md) 순서로 먼저 mental model을 고정한 뒤 이 문서로 올라오는 편이 안전하다.
- 지금 질문이 `권한을 바꿨는데 왜 토큰엔 예전 값이 남나`, `revoked role인데 still works`처럼 freshness 쪽에 더 가깝다면 deep dive로 바로 내려오기보다 [Claim Freshness After Permission Changes](./claim-freshness-after-permission-changes.md)나 [Grant Path Freshness and Stale Deny Basics](./grant-path-freshness-stale-deny-basics.md)에서 먼저 갈림길을 잡는 편이 덜 헷갈린다.
- JWT 관련 route를 다시 고르고 싶으면 입문 재진입점은 [Security README 기본 primer 묶음](./README.md#기본-primer), 권한 반영/tenant 문맥까지 함께 다시 고를 때는 [Security README: AuthZ / Tenant / Response Contracts](./README.md#authz--tenant--response-contracts-deep-dive-catalog)로 돌아가면 된다.
...
### 2. logout / revoke / version check 개념
