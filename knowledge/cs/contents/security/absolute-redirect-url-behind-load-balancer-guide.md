...
> 문서 역할: 이 문서는 load balancer 뒤에서 absolute redirect URL이 왜 internal host로 뒤집히는지 설명하는 beginner guide다. proxy 제품별 설정 문법과 framework deep dive는 관련 문서로 넘긴다.

> target query shape: `redirect_uri mismatch behind proxy`, `로그인 후 왜 내부 host로 가요`, `absolute redirect wrong origin`, `what is x-forwarded-host redirect`
...
비유는 여기까지만 맞다.
"앱 주소"가 하나라고 느껴져도 실제로는 browser origin, forwarded origin, internal hop 주소가 동시에 존재할 수 있고, 서로 다른 용도로 써야 한다.
...
| 지금 확인한 사실 | 다음 한 걸음 |
|---|---|
| host는 틀리고 scheme은 맞다 | [Forwarded Header Trust Boundary Primer](./forwarded-header-trust-boundary-primer.md)로 trusted proxy/header 복원을 확인한다 |
| host는 맞는데 `http://`로만 내려온다 | [Secure Cookie Behind Proxy Guide](./secure-cookie-behind-proxy-guide.md)로 `X-Forwarded-Proto`와 `Secure` cookie를 본다 |
| browser redirect는 정상인데 email/OAuth callback만 틀리다 | [Password Reset, Magic Link, Public Origin Guide](./password-reset-magic-link-public-origin-guide.md) 또는 [OAuth2 Authorization Code Grant](./oauth2-authorization-code-grant.md)에서 absolute URL source를 다시 고정한다 |
