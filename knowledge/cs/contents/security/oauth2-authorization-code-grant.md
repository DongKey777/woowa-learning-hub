...
- code exchange를 맡는 backend가 confidential client라면 token endpoint에서 어떤 client authentication을 쓸지 [OAuth Client Authentication: `client_secret_basic`, `private_key_jwt`, mTLS](./oauth-client-authentication-private-key-jwt-mtls.md)로 이어 보면 된다.
- code flow를 OIDC login에 붙였을 때 `id token`, `UserInfo`, 내부 권한 부여 경계를 어디서 끊을지 헷갈리면 [OIDC, ID Token, UserInfo](./oidc-id-token-userinfo-boundaries.md)로 내려가면 된다.
- 브라우저 대신 CLI, TV, 콘솔 같은 제한 입력 기기에서 로그인해야 하면 [OAuth Device Code Flow / Security Model](./oauth-device-code-flow-security.md)로 분기해서 보면 된다.
- login 이후 logout propagation과 RP local session 정리는 [OIDC Back-Channel Logout / Session Coherence](./oidc-backchannel-logout-session-coherence.md)에서 이어 볼 수 있다.
...
- code exchange를 서버로 넘기면 client authentication, PKCE, redirect URI 검증을 함께 적용할 수 있다
...
여기서 `id token`과 `UserInfo`를 둘 다 받는 순간 "무엇을 인증 증거로 보고, 무엇을 내부 role 매핑 전까지만 신뢰할지"를 따로 정해야 한다. 이 경계는 [OIDC, ID Token, UserInfo](./oidc-id-token-userinfo-boundaries.md)에서 이어 본다.
...
#### callback 성공 뒤에 남는 hardening

`state`, `redirect_uri`, `code_verifier`를 통과했다고 login completion이 끝난 것은 아니다.

- callback 직후 익명 세션을 그대로 재사용하면 login CSRF와 fixation이 이어질 수 있다
- BFF가 HttpOnly cookie를 세팅했다면 이후 POST/PATCH/DELETE부터는 별도 CSRF token과 `Origin` 검증이 필요하다
- 최종 post-login redirect는 사용자가 들고 온 `next`를 그대로 반사하지 말고 서버가 승인한 destination으로만 보내야 한다

정리하면 browser login completion hardening의 최소 묶음은 이렇다.

1. `state` / `redirect_uri` / `code_verifier`를 검증한다
2. authorization code와 verifier를 1회성으로 소비한다
3. callback 직후 session id와 anti-CSRF secret을 재발급한다
4. 서버가 고른 안전한 post-login redirect로 이동한다

verifier 저장/소비 failure mode는 [PKCE Failure Modes / Recovery](./pkce-failure-modes-recovery.md), cookie 기반 후속 mutation hardening은 [CSRF in SPA + BFF Architecture](./csrf-in-spa-bff-architecture.md), session 재발급 축은 [Session Fixation in Federated Login](./session-fixation-in-federated-login.md)에서 이어 본다.
...
### 8. 브라우저가 없는 기기는 다른 흐름이 필요하다

Authorization Code Grant는 "브라우저가 callback을 받을 수 있는가"를 전제로 한다.

- CLI, TV, 콘솔처럼 입력이 제한된 기기는 같은 브라우저 callback을 기대하기 어렵다
- 이 경우에는 사용자가 다른 기기에서 승인하는 cross-device login 모델이 더 자연스럽다
- 대표적인 대안이 [OAuth Device Code Flow / Security Model](./oauth-device-code-flow-security.md)이다

즉 browser-first app의 기본값은 authorization code + PKCE지만, no-browser device를 억지로 같은 흐름에 끼워 넣으면 verification, phishing, polling 문제가 새로 생긴다.
...
> Q: 서버가 code를 교환한다면 client authentication은 무엇으로 하나?
> 의도: authorization code 흐름과 token endpoint client proof를 분리하는지 확인
> 핵심: flow 자체와 별개로 confidential client의 token endpoint auth를 따로 고른다. 다음 분기점은 [OAuth Client Authentication: `client_secret_basic`, `private_key_jwt`, mTLS](./oauth-client-authentication-private-key-jwt-mtls.md)다.
