...
> 문서 역할: 이 문서는 tenant custom domain을 처음 붙일 때 "입력값"과 "검증된 origin"을 분리하는 beginner primer다. DNS 운영 세부, reverse proxy 설정, tenant isolation 테스트 설계는 관련 문서로 넘긴다.

> target query shape: `tenant custom domain verification 뭐예요`, `password reset 링크가 왜 다른 도메인으로 가요`, `custom domain은 언제 verified로 써야 하나`, `what is tenant custom domain verification`
...
비유는 여기까지만 맞다.
"간판 이름"처럼 보여도 auth link와 redirect에 들어가는 순간, 이 값은 UI 문구가 아니라 보안 의사결정 입력값이 된다.
...
| 처음 드는 질문 | beginner 답 |
|---|---|
| tenant가 방금 입력했는데 왜 바로 쓰면 안 되나 | 아직 소유 증명이 안 끝난 문자열이기 때문이다 |
| request `Host`가 맞아 보이는데 왜 DB를 다시 보나 | request 값은 힌트일 뿐이고, auth link 선택 기준은 verified mapping이어야 하기 때문이다 |
| DNS TXT를 등록했으면 redirect allowlist도 자동으로 끝난 건가 | 보통 아니다. domain 소유 검증과 redirect 허용 정책은 연결되지만 같은 규칙은 아니다 |
