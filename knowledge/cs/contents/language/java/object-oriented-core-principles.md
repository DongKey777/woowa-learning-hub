...
처음 배우는 단계에서는 OOP의 역사보다 "상태와 행동을 왜 한 객체 안에 묶는가"를 먼저 잡는 편이 쉽다.

| 지금 떠올릴 질문 | OOP가 주는 첫 답 |
|---|---|
| 값이 아무 데서나 바뀌면 왜 불안한가 | 객체 안에 규칙을 모아 캡슐화한다 |
| 같은 종류의 일을 여러 방식으로 바꾸려면 어떻게 하나 | 공통 역할을 만들고 구현을 바꿔 낀다 |
| 공통 코드를 어디까지 부모로 올려도 되나 | is-a 관계일 때만 좁게 상속을 쓴다 |

즉 이 문서에서 먼저 가져갈 문장은 이것이다. **객체지향은 관련 상태와 행동을 한 책임 단위로 묶고, 필요한 만큼만 공개하는 방식**이다.
...
초보자 첫 읽기에서는 "메시지"를 어려운 용어로 잡기보다 **한 객체가 다른 객체의 메서드를 호출해 일을 부탁하는 것** 정도로 이해하면 충분하다.

```java
car.changeGear(lowerGear);
```

위 한 줄도 "Car 객체에게 기어를 바꾸라고 요청한다"는 메시지로 볼 수 있다. 여기서는 분산 시스템이나 프로세스 경계 같은 주제로 확장하지 않고, 객체 협력의 최소 감각만 가져간다.

## 클래스의 구조

### 클래스의 구성 요소

처음에는 클래스 문법 전체를 외우기보다 "상태, 생성 규칙, 행동" 세 칸으로 보는 편이 쉽다.

| 칸 | 코드에서 보이는 자리 | 초보자용 질문 |
|---|---|---|
| 상태 | 필드 | 이 객체가 무엇을 기억하나 |
| 생성 규칙 | 생성자 | 처음 만들 때 무엇이 꼭 필요하나 |
| 행동 | 메서드 | 이 객체가 스스로 무엇을 할 수 있나 |
...
modifier 세부 규칙은 이 문서의 중심이 아니다. beginner 첫 읽기에서는 `public class`, `private field`, `public method` 정도만 구분하고, 자세한 접근 제한자는 [Java 접근 제한자와 멤버 모델 입문](./java-access-modifiers-member-model-basics.md)으로 넘긴다.
...
처음에는 아래 두 줄만 먼저 구분해도 충분하다.

- `private`: 객체 안에서만 직접 만진다
- `public`: 바깥에서 써도 되는 통로다
...
메서드 구성 요소로는 **접근 제한자, 반환형, 메서드명, 매개변수**가 있다. 초보자 첫 읽기에서는 "누가 호출할 수 있는가", "무엇을 돌려주는가", "어떤 입력을 받는가" 정도만 분리해도 충분하다.
...
`final`, `static`, `abstract` 세부 차이는 관련 primer에서 이어서 보고, 여기서는 메서드가 "상태를 읽거나 바꾸는 객체의 행동"이라는 점을 먼저 잡는다.

초보자 단계에서는 `오버로딩`과 `오버라이딩`을 구분할 때도 JVM 내부 규칙까지 들어가기보다 "이름은 같고 매개변수가 다르면 다른 메서드", "부모 메서드를 같은 형태로 다시 정의하면 재정의" 정도로 먼저 잡는 편이 낫다. 더 자세한 규칙은 [Java 오버로딩 vs 오버라이딩 입문](./java-overloading-vs-overriding-beginner-primer.md)으로 넘긴다.
