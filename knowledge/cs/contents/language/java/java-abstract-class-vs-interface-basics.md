...
- [Template Method vs Strategy Quick Check Card](./template-method-vs-strategy-quick-check-card.md) - 추상 클래스 vs 인터페이스 다음에 `부모가 흐름을 쥐는가`와 `밖에서 구현을 넣는가`를 4개 사례로 바로 자르는 quick card
...
- "부모가 흐름을 쥔다 vs 밖에서 구현을 갈아끼운다"를 바로 4개 사례로 확인하려면 [Template Method vs Strategy Quick Check Card](./template-method-vs-strategy-quick-check-card.md)
