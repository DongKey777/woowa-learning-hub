...
여기서 한 가지를 같이 고정해 두면 null 관련 오해를 줄일 수 있다.

- 경계에 맞는 원소가 없으면 결과는 보통 `null`이 아니라 **비어 있는 range view**다
- 즉 "`30`을 제외했다"는 말은 "`30` 자리 exact match를 창에서 뺐다"는 뜻이지, null 허용 정책 얘기가 아니다
...
그리고 결과 해석도 같이 분리해 두는 편이 안전하다.

```java
System.out.println(gradeByScore.headMap(10));              // {}
System.out.println(gradeByScore.tailMap(40, false));       // {}
System.out.println(gradeByScore.subMap(20, false, 30, false)); // {}
```

- 위 셋은 모두 "조건에 맞는 key가 없는 비어 있는 view"다
- `Map.get(key)`처럼 "못 찾으면 `null`을 줄까?"를 묻는 API가 아니다
- 즉 empty result와 null-policy를 섞어 읽지 않는 것이 중요하다
...
- `headMap(10)`이나 `subMap(20, false, 30, false)`가 비면 `null`이 나올 것처럼 읽기 쉽다. 실제로는 비어 있는 range view가 돌아온다.
