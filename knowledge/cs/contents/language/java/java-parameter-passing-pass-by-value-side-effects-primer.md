...
> retrieval-anchor-keywords: java parameter passing basics, java pass by value, java always pass by value, java reference type pass by value, java object reference copy, java method argument copy, java parameter reassignment basics, java mutation vs reassignment java, java object state tracking, java side effect basics, java aliasing basics, java array mutation parameter, java beginner reference semantics, java call by value reference type, 자바 값 전달 참조 전달 차이, 자바는 참조에 의한 전달인가요, 처음 배우는데 pass by value, 기본형은 안 바뀌는데 객체는 왜 바뀌나요, 메서드에서 객체 바꾸면 원본도 바뀌는지, 자바 파라미터 전달 큰 그림, 자바 side effect 기초, mutation reassignment 차이
