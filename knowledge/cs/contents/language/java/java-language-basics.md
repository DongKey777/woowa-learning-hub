...
retrieval-anchor-keywords: java language basics, 자바 처음 배우는데 뭐부터, 자바 기초 큰 그림, jdk vs jvm beginner, 자바 처음인데 jdk jvm 차이, java source bytecode jvm flow, main 메서드 어디서 시작, java primitive vs reference, 기본형 참조형 처음 차이, int integer 차이 어디서 봐요, 자바 int integer 차이 입문, java local variable not initialized compile error, 자바 컴파일 에러 왜 나요, java array basics, 배열 기초 어디부터
...
| "`int`와 `Integer` 차이가 뭐예요?`, `왜 `int` 대신 `Integer`를 써요?`" | [Java 데이터 타입 먼저 보는 큰 그림](#java-데이터-타입-먼저-보는-큰-그림) | [Primitive-wrapper choice primer: `int`/`long`/`boolean` vs `Integer`/`Long`/`Boolean`](./primitive-wrapper-choice-primer.md) |
...
여기서 검색 질문이 바로 "`int`와 `Integer` 차이"로 좁혀지면, 이 문서에서는 "기본형 vs 참조형 큰 그림"까지만 잡고 다음 칸으로 [Primitive-wrapper choice primer: `int`/`long`/`boolean` vs `Integer`/`Long`/`Boolean`](./primitive-wrapper-choice-primer.md)로 이동하는 편이 빠르다.
