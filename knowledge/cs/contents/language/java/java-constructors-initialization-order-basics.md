...
retrieval-anchor-keywords: java constructor chaining basics, java initialization order basics, java this super basics, java this constructor call, java super constructor call, java object creation constructor order, java new constructor instance method order, java field initialization order, java static initialization block, java instance initialization block, java initializer block basics, java parent child constructor order, java subclass initialization order, java constructor first line rule, java beginner constructor primer
...
처음에는 생성자를 독립 문법으로 보지 말고 아래 네 칸으로 이어 읽는 편이 덜 헷갈린다.

| 장면 | 코드에서 보이는 모양 | 초보자용 해석 |
|---|---|---|
| 타입 정의 | `class Order { ... }` | 어떤 필드와 메서드를 가진 객체를 만들지 정한다 |
| 객체 생성 | `new Order("ramen", 2)` | 새 객체가 생기고 constructor 진입이 시작된다 |
| 시작 상태 채우기 | `this.productName = productName` | constructor가 필드와 불변식을 맞춘다 |
| 생성 후 사용 | `order.changeQuantity(1)` | instance method는 태어난 뒤의 객체를 여러 번 움직인다 |

짧게 줄이면 생성자는 "태어날 때 한 번", 인스턴스 메서드는 "태어난 뒤 여러 번" 등장한다. 그래서 이 문서는 메서드 문서의 다음 칸에서 "constructor 안에서는 무슨 순서로 초기화가 일어나나"를 더 정확히 보는 역할을 한다.
