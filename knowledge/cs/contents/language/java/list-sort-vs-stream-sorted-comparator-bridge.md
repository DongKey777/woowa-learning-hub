...
`List.sort(...)`와 `stream.sorted(...)`는 같은 `Comparator` chain을 공유할 수 있으며, 선택 기준은 "원본 리스트를 직접 바꿀 것인가, 정렬된 새 결과를 받을 것인가"이고, boxing 미세 최적화보다 먼저 정렬 의도가 읽히게 쓰는 것이 초보자에게 더 중요하다.
