...
cross-category로 한 번 건넜더라도 복귀 기준은 같다. database 쪽에서는 `트랜잭션 기초`나 `JDBC · JPA · MyBatis 기초`까지만, spring 쪽에서는 `요청 파이프라인` primer까지만 읽고, 질문이 다시 `new`, `equals()`, `HashMap#get(...)`, `Optional` 축으로 줄어들면 바로 이 README의 [beginner 복귀 사다리](#beginner-복귀-사다리)나 [빠른 탐색](#빠른-탐색)으로 돌아온다.
...
| [`Optional`에서 끝낼까, 컬렉션/도메인 타입으로 옮길까 beginner bridge](./java/optional-collections-domain-null-handling-bridge.md) | [Java Optional 입문](./java/java-optional-basics.md) 또는 [Java enum 기초](./java/java-enum-basics.md) | 이 README의 [equality / collections / optional 복귀 표](#language-equality-collections-optional-quick-return) |
| language를 읽다가 질문이 `save()`, `Repository`, `@Transactional`, SQL 위치로 넓어짐 | [Database README의 빠른 탐색](../database/README.md#빠른-탐색) | 이 README의 [빠른 탐색](#빠른-탐색) |
...
- database primer를 읽을 때도 바로 `격리 수준`, `flush`, `pool timeout` deep dive로 내려가지 말고, 먼저 `트랜잭션 기초` 또는 `JDBC · JPA · MyBatis 기초` 한 장만 확인한 뒤 이 README나 Database README의 빠른 탐색으로 돌아와 남은 질문 축을 다시 고른다.
