...
- Java `primer`부터 읽고 싶다면:
  ...
  - 처음 배우는데 `상속 언제 쓰는지`, `상속 vs 조합`, `템플릿 메소드가 예외인지`가 궁금하면 [Java 타입, 클래스, 객체, OOP 입문](./java/java-types-class-object-oop-basics.md) -> [Java 상속과 오버라이딩 기초](./java/java-inheritance-overriding-basics.md) -> [상속보다 조합 기초](../design-pattern/composition-over-inheritance-basics.md) -> [템플릿 메소드 패턴 기초](../design-pattern/template-method-basics.md) -> [템플릿 메소드 vs 전략](../design-pattern/template-method-vs-strategy.md)
  ...
  - [Comparator Utility Patterns](./java/java-comparator-utility-patterns.md) - `comparingInt`/`thenComparingInt` 계열과 boxing 없는 tie-breaker 조립 감각까지 이어서 보기
  ...
  - [Nullable Wrapper Comparator Bridge](./java/nullable-wrapper-comparator-bridge.md) - wrapper 숫자 필드에서 `nullsFirst`/`nullsLast`를 먼저 고르고, 내림차순에서도 `null`을 뒤에 두는 법까지 보기
  ...
  - [NavigableMap and NavigableSet Mental Model](./java/navigablemap-navigableset-mental-model.md) - `first`/`last`/`floor`/`ceiling`/`lower`/`higher`를 comparator order 위의 이웃 찾기로 이해하는 primer
  ...
  - [객체지향 핵심 원리](./java/object-oriented-core-principles.md) - 처음 배우는데 OOP 큰 그림, 객체지향 4대 원칙, 상속 언제 쓰는지부터 잡는 primer
  - [불변 객체와 방어적 복사 입문](./java/java-immutable-object-basics.md)
  - [추상 클래스 vs 인터페이스 입문](./java/java-abstract-class-vs-interface-basics.md) - 처음 배우는데 `abstract class`, `interface`, `상속 vs 조합`, `템플릿 메소드 언제 쓰는지`가 같이 헷갈릴 때 보는 첫 진입점
...
- [`List.sort` vs `Stream.sorted` Comparator Bridge](./java/list-sort-vs-stream-sorted-comparator-bridge.md) - 같은 comparator chain을 mutable list 정렬과 stream 정렬 결과에 재사용하는 감각 잡기
- [Nullable Wrapper Comparator Bridge](./java/nullable-wrapper-comparator-bridge.md) - wrapper 숫자 필드에서 `nullsFirst`/`nullsLast`를 먼저 고르고, 내림차순에서도 `null`을 뒤에 두는 법까지 보기
- [Comparator in TreeSet and TreeMap](./java/treeset-treemap-comparator-tie-breaker-basics.md)
...
- [NavigableMap and NavigableSet Mental Model](./java/navigablemap-navigableset-mental-model.md) - `first`/`last`/`floor`/`ceiling`/`lower`/`higher`를 comparator order 위의 이웃 찾기로 보는 primer
- [HashSet vs TreeSet Duplicate Semantics](./java/hashset-vs-treeset-duplicate-semantics.md)
- [Sorting and Searching Arrays Basics](./java/java-array-sorting-searching-basics.md)
- [객체지향 핵심 원리](./java/object-oriented-core-principles.md) - 처음 배우는데 OOP 큰 그림, 객체지향 4대 원칙, 상속 언제 쓰는지부터 잡는 primer
- [불변 객체와 방어적 복사 입문](./java/java-immutable-object-basics.md)
- [불변 객체와 방어적 복사](./java/immutable-objects-and-defensive-copying.md)
- [추상 클래스 vs 인터페이스 입문](./java/java-abstract-class-vs-interface-basics.md) - 공통 상태/흐름은 추상 클래스, 바꿔 끼울 계약은 인터페이스+조합으로 나누는 beginner bridge
...
- 상속을 처음 배우는데 "언제 쓰는지"가 막히면 [Java 타입, 클래스, 객체, OOP 입문](./java/java-types-class-object-oop-basics.md), [Java 상속과 오버라이딩 기초](./java/java-inheritance-overriding-basics.md), [상속보다 조합 기초](../design-pattern/composition-over-inheritance-basics.md), [템플릿 메소드 패턴 기초](../design-pattern/template-method-basics.md), [템플릿 메소드 vs 전략](../design-pattern/template-method-vs-strategy.md)을 순서대로 보면 문법 질문이 설계 선택 질문으로 자연스럽게 이어진다.
