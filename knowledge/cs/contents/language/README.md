...
> - "`한쪽만 바꿨는데 왜 같이 바뀌죠?` 다음에 `그래서 set/map에서 왜 못 찾죠?`로 이어지면 `실행 모델 -> equality -> collections` 순서만 유지한다. 바로 `hashCode()` 규약부터 외우지 않는다
...
> - comparator, sorted collection, `BigDecimal` key, record serialization 같은 follow-up은 `실행 모델 -> equality -> collections` primer를 지난 뒤 한 장씩만 붙인다
...
cross-category로 한 번 건넜더라도 복귀 기준은 같다. database 쪽에서는 [Database README의 빠른 탐색](../database/README.md#빠른-탐색)에서 `트랜잭션 기초`나 `JDBC · JPA · MyBatis 기초`까지만, spring 쪽에서는 `요청 파이프라인` primer까지만 읽고, 질문이 다시 `new`, `equals()`, `HashMap#get(...)`, `Optional` 축으로 줄어들면 바로 이 README의 [beginner 복귀 사다리](#beginner-복귀-사다리)나 [빠른 탐색](#빠른-탐색)으로 돌아온다.

처음엔 "문서를 다 읽어야 이해된다"보다 "지금 코드가 무슨 질문을 하고 있나"를 먼저 자르는 편이 낫다.

<a id="language-java-return-ladder"></a>
## beginner 복귀 사다리

primer를 읽고 바로 deep dive catalog로 미끄러지지 않게, "다음 한 칸"과 "돌아올 자리"를 먼저 같이 본다.

| 방금 읽은 primer | 바로 다음 한 칸 | 막히면 돌아올 자리 |
|---|---|---|
| [Java 실행 모델과 객체 메모리 mental model 입문](./java/java-execution-object-memory-mental-model-primer.md) | [Java Equality and Identity Basics](./java/java-equality-identity-basics.md) | 이 README의 [빠른 탐색](#빠른-탐색) |
| [Java 타입, 클래스, 객체, OOP 입문](./java/java-types-class-object-oop-basics.md) | [Java 실행 모델과 객체 메모리 mental model 입문](./java/java-execution-object-memory-mental-model-primer.md) | 이 README의 [빠른 탐색](#빠른-탐색) |
| [Java Equality and Identity Basics](./java/java-equality-identity-basics.md) | [Collections, Equality, and Mutable-State Foundations](./java/collections-equality-mutable-state-foundations.md) | 이 README의 [equality / collections / optional 복귀 표](#language-equality-collections-optional-quick-return) |
| [Java 컬렉션 프레임워크 입문](./java/java-collections-basics.md) | [Map `get()` null 의미와 `containsKey()`/`getOrDefault()` 선택 프라이머](./java/map-get-null-containskey-getordefault-primer.md) 또는 [HashMap vs TreeMap 초급 선택 브리지](./java/hashmap-vs-treemap-beginner-selection-bridge.md) | 이 README의 [equality / collections / optional 복귀 표](#language-equality-collections-optional-quick-return) |
| [`Optional`에서 끝낼까, 컬렉션/도메인 타입으로 옮길까 beginner bridge](./java/optional-collections-domain-null-handling-bridge.md) | [`Optional<List<T>>` vs 빈 컬렉션 증상 카드](./java/optional-list-empty-collection-symptom-card.md) 또는 [Java enum 기초](./java/java-enum-basics.md) | 이 README의 [equality / collections / optional 복귀 표](#language-equality-collections-optional-quick-return) |
| language를 읽다가 질문이 `save()`, `Repository`, `@Transactional`, SQL 위치로 넓어짐 | [Database의 Spring -> Database handoff](../database/README.md#database-entry-handoff) | 이 README의 [빠른 탐색](#빠른-탐색) |
| language를 읽다가 질문이 `controller`, bean, 요청 흐름으로 넓어짐 | [Spring README: 빠른 탐색](../spring/README.md#빠른-탐색) | 이 README의 [빠른 탐색](#빠른-탐색) |

복귀 루프는 이 한 줄로 고정한다.

`이 README의 빠른 탐색 -> primer 1장 -> 다음 한 칸 1장 -> 막히면 이 README의 빠른 탐색 또는 equality / collections / optional 복귀 표`
...
| `==`, `equals()`, `String`, 같은 객체 vs 같은 값 | 같은 객체 질문인지, 같은 값 질문인지 | [Java Equality and Identity Basics](./java/java-equality-identity-basics.md) |
...
| "`같은 객체예요, 같은 값이에요?`, `String 같은데 왜 false예요?`" | [Java Equality and Identity Basics](./java/java-equality-identity-basics.md) | 문자열이면 [Java String 기초](./java/java-string-basics.md), 해시 컬렉션이면 [Collections, Equality, and Mutable-State Foundations](./java/collections-equality-mutable-state-foundations.md) |
...
- `왜 저장은 되는데 rollback 범위나 SQL 위치가 헷갈리지?`, `처음인데 controller 다음이 안 보여요`처럼 handoff 질문이 되면 [Database의 Spring -> Database handoff](../database/README.md#database-entry-handoff)에서 `트랜잭션 기초`와 `JDBC · JPA · MyBatis 기초` 중 한 칸만 다시 고른다.
- database primer를 읽을 때도 바로 `격리 수준`, `flush`, `pool timeout` deep dive로 내려가지 말고, 먼저 `트랜잭션 기초` 또는 `JDBC · JPA · MyBatis 기초` 한 장만 확인한 뒤 이 README나 [Database README의 빠른 탐색](../database/README.md#빠른-탐색)으로 돌아와 남은 질문 축을 다시 고른다.
...
<a id="language-java-database-handoff"></a>
...
| "`왜 저장은 되는데 rollback 범위나 SQL 위치가 아직 헷갈리죠?`" | [Database의 Spring -> Database handoff](../database/README.md#database-entry-handoff) | [트랜잭션 기초](../database/transaction-basics.md) 또는 [JDBC · JPA · MyBatis 기초](../database/jdbc-jpa-mybatis-basics.md) | 이 README의 [빠른 탐색](#빠른-탐색) |
| "`save()`는 보이는데 `HashMap#get(...)`도 같이 흔들려서 어디부터 복구할지 모르겠어요" | [Database의 DB인지 Java인지 헷갈릴 때](../database/README.md#database-vs-language-return) | [JDBC · JPA · MyBatis 기초](../database/jdbc-jpa-mybatis-basics.md)로 SQL 위치를 먼저 자른 뒤 [Map `get()` null 의미와 `containsKey()`/`getOrDefault()` 선택 프라이머](./java/map-get-null-containskey-getordefault-primer.md)로 되돌아온다 | 이 README의 [equality / collections / optional 복귀 표](#language-equality-collections-optional-quick-return) |
...
language에서 database로 건넌 뒤 길을 잃으면 [Database의 Spring -> Database handoff](../database/README.md#database-entry-handoff)로 돌아가 `Database First-Step Bridge -> 트랜잭션 기초 -> JDBC · JPA · MyBatis 기초` 3칸 중 하나만 다시 고르면 된다. 반대로 database 문서를 읽다가도 질문이 아직 `new`, `객체`, `equals()`, `HashMap#get()` 축이면 [이 README의 beginner 복귀 사다리](#language-java-return-ladder)나 [빠른 탐색](#빠른-탐색)으로 되돌아와 Java primer 한 장만 먼저 복구한다.
...
반대로 Java primer를 한 장 읽고 "`Map 구현체만 한 번 더 고르고 싶어요`", "`DTO에서 값 객체로 올릴지부터 헷갈려요`", "`request DTO의 String을 service까지 그대로 넘겨도 되나요?`"처럼 language 안의 다음 한 칸만 남았다면 큰 catalog보다 [Java Deep Dive Catalog의 컬렉션 빠른 탐색](./java/README.md#컬렉션-빠른-탐색)이나 [Request DTO에서 raw string을 값 객체로 올리는 경계 입문](./java/request-dto-to-value-object-boundary-primer.md)으로 짧게 건너간다. 그 뒤 질문이 다시 넓어지면 이 README의 [빠른 탐색](#빠른-탐색)으로 돌아와 축부터 다시 고른다.
...
| "`Optional`이랑 빈 리스트를 언제 갈라요?" | [`Optional<List<T>>` vs 빈 컬렉션 증상 카드](./java/optional-list-empty-collection-symptom-card.md) | 단건의 없음은 `Optional`, 여러 건의 `0개`는 컬렉션이다 |
...
| "`Optional`, 빈 리스트, enum 상태 중 뭘 써야 할지 모르겠다" | [`Optional<List<T>>` vs 빈 컬렉션 증상 카드](./java/optional-list-empty-collection-symptom-card.md) | 단건의 없음은 `Optional`, 다건의 0개는 빈 컬렉션, 없음의 이유는 상태 타입으로 본다 | [`Optional`에서 끝낼까, 컬렉션/도메인 타입으로 옮길까 beginner bridge](./java/optional-collections-domain-null-handling-bridge.md) |
...
- Java 세부 주제를 category 안에서 다시 찾고 싶다면:
  - [Java Deep Dive Catalog](./java/README.md) - Java sub-navigator. `collections`, `map`, `set`, `queue`, equality follow-up을 한 곳에서 다시 고른다.
...
- collections follow-up을 한 칸만 더 가고 싶다면:
  - `ordered map`, `TreeMap`, `floor`/`ceiling`, `null` 경계가 같이 보이면 [Ordered Map Null-Safe Practice Drill](./java/ordered-map-null-safe-practice-drill.md)
  - `List`/`Set`/`Map` 다음 구현체나 equality follow-up을 category 단위로 다시 고르려면 [Java Deep Dive Catalog의 컬렉션 빠른 탐색](./java/README.md#컬렉션-빠른-탐색)
- Java `primer`부터 읽고 싶다면:
  ...
  - [Java 타입, 클래스, 객체, OOP 입문](./java/java-types-class-object-oop-basics.md) - 클래스 선언/참조 변수 선언/객체 생성을 3단계로 끊어 보고, `객체지향 큰 그림 -> 상속 언제 쓰는지 -> 추상 클래스/인터페이스 -> quick check -> 상속보다 조합` route의 첫 출발점이 되는 beginner bridge
  ...
  - [Java 상속과 오버라이딩 기초](./java/java-inheritance-overriding-basics.md) - 처음 배우는데 `객체지향 큰 그림 -> 상속 언제 쓰는지 -> 추상 클래스/인터페이스 -> quick check -> 상속보다 조합` route에서 `상속 언제 쓰는지` 지점을 붙잡아 주는 handoff primer
...
- [객체지향 핵심 원리](./java/object-oriented-core-principles.md) - 처음 배우는데 OOP 큰 그림을 먼저 잡고, `클래스 선언/참조 변수 선언/객체 생성` 1분 브리지로 헷갈림을 푼 뒤 `상속 언제 쓰는지 -> 추상 클래스/인터페이스 -> quick check -> 상속보다 조합` 순서로 이어지게 만드는 primer
...
- `List`/`Set`/`Map`은 알겠는데 `빈 리스트`, `Optional`, `Map#get(...) == null`, enum 상태가 같이 섞이면 이 짧은 우회로로 자른다.
  [Java 컬렉션 프레임워크 입문](./java/java-collections-basics.md) -> [`Optional<List<T>>` vs 빈 컬렉션 증상 카드](./java/optional-list-empty-collection-symptom-card.md) -> [Map `get()` null 의미와 `containsKey()`/`getOrDefault()` 선택 프라이머](./java/map-get-null-containskey-getordefault-primer.md) -> [Java enum 기초](./java/java-enum-basics.md) -> [Enum 상수 비교와 문자열 payload 비교를 언제 나눌까](./java/enum-string-boundary-bridge.md)
...
- [Java 상속과 오버라이딩 기초](./java/java-inheritance-overriding-basics.md) - 처음 배우는데 `객체지향 큰 그림 -> 상속 언제 쓰는지 -> 추상 클래스/인터페이스 -> quick check -> 상속보다 조합` route에서 `상속 언제 쓰는지`를 묻는 질의(예: `처음 배우는데 상속 언제 쓰는지`)를 직접 받는 handoff primer
...
- [Request DTO에서 raw string을 값 객체로 올리는 경계 입문](./java/request-dto-to-value-object-boundary-primer.md) - "`DTO에서 값 객체로 올릴지부터 헷갈려요`", "`request DTO의 String을 service까지 그대로 넘겨도 되나요?`"를 그대로 받아 DTO와 domain value object 경계를 한 장에서 자르는 beginner primer
...
- [추상 클래스 vs 인터페이스 입문](./java/java-abstract-class-vs-interface-basics.md) - 공통 상태/흐름은 추상 클래스, 바꿔 끼울 계약은 인터페이스+조합으로 나누고 beginner 첫 읽기 bridge를 `추상 클래스/인터페이스 -> quick check -> 상속보다 조합 -> 템플릿 메소드 패턴 기초 -> 템플릿 메소드 vs 전략` 순서로 고정하는 entrypoint primer
...
- [추상 클래스 vs 인터페이스](./java/abstract-class-vs-interface.md) - beginner first-hit은 바로 위 [추상 클래스 vs 인터페이스 입문](./java/java-abstract-class-vs-interface-basics.md)과 [추상 클래스 vs 인터페이스 Follow-up Quick Check](./java/abstract-class-vs-interface-follow-up-drill.md)로 먼저 보내고, 그다음 `상속보다 조합 -> 템플릿 메소드 패턴 기초 -> 템플릿 메소드 vs 전략` handoff를 유지한 뒤 `default method`, sealed interface, binary compatibility 같은 intermediate guardrail로 내려가는 deep dive
- [Java 예외 처리 기초](./java/java-exception-handling-basics.md) - 처음 배우는데 `try-catch`를 왜 쓰는지부터 시작해, checked/unchecked를 언제 쓰는지와 `throw`/`throws`/`catch`를 어디서 나누는지까지 큰 그림과 30초 분기표로 잡는 기초 primer
- [Java 제네릭 입문](./java/java-generics-basics.md) - 처음 배우는데 `<T>`가 왜 필요한지, `List<Object> vs List<String>` 같은 초급 혼동과 와일드카드 첫 분기까지 큰 그림으로 정리하는 기초 primer
...
- [Java Optional 입문](./java/java-optional-basics.md) - 처음 배우는데 `Optional`을 언제 쓰는지, `null`과 차이, `orElse`/`orElseGet` 선택 기준, `get()` 남용을 피하는 기초를 정리한 primer
- [`Optional<List<T>>` vs 빈 컬렉션 증상 카드](./java/optional-list-empty-collection-symptom-card.md) - `Optional<List<T>>`가 더 안전해 보이지만 beginner 단계에서는 왜 오히려 상태 확인을 두 번 만들기 쉬운지, `한 건의 없음`과 `여러 건의 0개`를 어떻게 먼저 자를지 한 장으로 정리한 symptom card
...
- beginner route note: object/equality/collections에서 막혔으면 아래 목록을 위에서 아래로 다 읽지 말고 [Java Equality and Identity Basics](./java/java-equality-identity-basics.md) -> [Collections, Equality, and Mutable-State Foundations](./java/collections-equality-mutable-state-foundations.md) -> [Record and Value Object Equality](./java/record-value-object-equality-basics.md) 순서로 한 장씩만 붙인다.
- comparator, sorted collection, `BigDecimal` key, record 진화/serialization은 같은 beginner 질문을 더 깊게 파는 follow-up이다. 첫 진입에서는 primer를 먼저 고르고, 세부 함정은 관련 문서 링크로 넘긴다.
...
- [Request DTO에서 raw string을 값 객체로 올리는 경계 입문](./java/request-dto-to-value-object-boundary-primer.md) - "`DTO에서 값 객체로 올릴지부터 헷갈려요`", "`request DTO의 String을 service까지 그대로 넘겨도 되나요?`"를 그대로 받아 boundary 변환 시점을 바로 고르는 beginner entrypoint
...
- 상속을 처음 배우는데 "언제 쓰는지"나 `hook method`/`abstract step` 큰 그림이 막히면 먼저 `객체지향 큰 그림 -> 상속 언제 쓰는지 -> 추상 클래스/인터페이스 -> quick check -> 상속보다 조합 -> 템플릿 메소드 패턴 기초 -> 템플릿 메소드 vs 전략` route를 [Java 타입, 클래스, 객체, OOP 입문](./java/java-types-class-object-oop-basics.md) -> [객체지향 핵심 원리](./java/object-oriented-core-principles.md) -> [Java 상속과 오버라이딩 기초](./java/java-inheritance-overriding-basics.md) -> [추상 클래스 vs 인터페이스 입문](./java/java-abstract-class-vs-interface-basics.md) -> [추상 클래스 vs 인터페이스 Follow-up Quick Check](./java/abstract-class-vs-interface-follow-up-drill.md) -> [상속보다 조합 기초](../design-pattern/composition-over-inheritance-basics.md) -> [템플릿 메소드 패턴 기초](../design-pattern/template-method-basics.md) -> [템플릿 메소드 vs 전략](../design-pattern/template-method-vs-strategy.md) 순서로 잡으면 beginner 첫 읽기 동선이 흔들리지 않는다. 그다음 [객체지향 설계 기초](../software-engineering/oop-design-basics.md), [객체지향 디자인 패턴 기초](../design-pattern/object-oriented-design-pattern-basics.md)으로 넓히면 문법 질문이 설계 선택 질문으로 자연스럽게 이어진다. 초급 golden query는 `처음 배우는데 상속 언제 쓰는지`, `기초 상속 vs 조합`, `추상 클래스 인터페이스 다음 뭐 읽지`, `추상 클래스 인터페이스 quick check`, `템플릿 메소드 언제 쓰는지`처럼 conversational phrasing으로도 이 same beginner handoff 체인을 먼저 잡도록 유지한다.
