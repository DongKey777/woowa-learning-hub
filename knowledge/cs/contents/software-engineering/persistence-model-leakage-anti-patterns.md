...
`Repository Interface Contract Primer` 다음에 "`서비스나 API에 `Entity`가 이미 보이는데 이게 왜 문제지?`"가 먼저 눈에 들어왔다면 이 문서가 후속 선택지다.
아직 누수 증상보다 "`domain`과 `Entity`를 어디서 번역하지?`"가 더 큰 질문이면 [Persistence Adapter Mapping Checklist](./persistence-adapter-mapping-checklist.md)를 먼저 보고, test fake가 헷갈리는 쪽이면 [Repository Fake Design Guide](./repository-fake-design-guide.md)로 새는 편이 덜 꼬인다.
target query: `서비스나 API에 Entity가 이미 보이는데 왜 문제지`, `엔티티를 JSON으로 바로 내보내도 되나요`, `service에서 entity setxxx 괜찮나요`, `처음 jpa 경계 헷갈림`.
