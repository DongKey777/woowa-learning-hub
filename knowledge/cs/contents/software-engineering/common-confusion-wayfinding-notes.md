...
- [테스트 4단 사다리 먼저 붙이기](#테스트-4단-사다리-먼저-붙이기)
...
테스트 문서가 여러 장이라도, 초심자 starter 사다리는 `unit test -> slice test -> app integration test -> E2E test` 네 칸으로 먼저 고정하면 된다.

| 사다리 칸 | 지금 묻는 질문 | 대표 예시 | 먼저 볼 문서 |
|---|---|---|---|
| `unit test` | 규칙, 계산, 상태 전이가 맞나 | 만료 시각이 지나면 `ACTIVE -> EXPIRED`가 되나 | [테스트 전략 기초](./test-strategy-basics.md) |
| `slice test` | HTTP/JPA처럼 특정 경계 번역이 맞나 | `POST /coupons/expire` 입력 검증, JPA 조회 조건 | [테스트 전략 기초](./test-strategy-basics.md), [Inbound Adapter Test Slices Primer](./inbound-adapter-test-slices-primer.md) |
| `app integration test` | 여러 빈, 트랜잭션, 저장소 협력이 실제로 이어지나 | 저장과 재고 차감, commit 뒤 후속 처리 | [테스트 전략 기초](./test-strategy-basics.md), [트랜잭셔널 테스트 rollback vs commit 경계 카드](./transactional-test-rollback-vs-commit-boundary-card.md) |
| `E2E test` | 사용자가 바깥에서 끝까지 성공했나 | 로그인 후 주문 완료, 외부 인증 후 복귀 | [Browser E2E cost-vs-signal checklist](./browser-e2e-cost-vs-signal-checklist.md), [컴포넌트 테스트 vs 브라우저 E2E 경계 카드](./component-test-vs-browser-e2e-boundary-card.md) |

- `seam`은 다섯 번째 테스트 종류가 아니다. 네 칸 중 어느 자리에서 끊을지 정하는 기준이다.
- 그래서 safe order는 `첫 테스트 종류 선택 -> 그 테스트를 어디까지 자를지 seam 결정`이다.
- `@SpringBootTest`가 넓어 보여도 보통은 `app integration test` 쪽이고, 실제 브라우저/외부 클라이언트 사용자 여정을 끝까지 보면 그때 `E2E test`로 올린다.
...
| 4. 바깥 사용자 흐름 확인 | "로그인 후 배치 결과를 운영 화면에서 끝까지 확인해야 하는가?" | 필요할 때만 `E2E test` 추가 | 브라우저/외부 클라이언트 여정이 핵심일 때만 마지막 안전망으로 올린다. |
...
- [Browser E2E cost-vs-signal checklist](./browser-e2e-cost-vs-signal-checklist.md)는 `정말 E2E까지 올릴 흐름인지`를 마지막에 다시 거른다.
