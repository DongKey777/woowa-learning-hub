...
- Beginner 경계/영속성 질문형 빠른 경로 (문서 이름보다 질문 중심으로 읽기):
  ...
  - [2) Module API DTO Patterns](./module-api-dto-patterns.md) `선택` — "`모듈 API 계약`, `cross-module dto`, `다른 모듈에 entity를 넘겨도 되나요?`처럼 검색해도 바로 붙는 beginner primer entrypoint"
    `Entity Leakage Review Checklist`에서 "모듈 공개 API에 `Entity`가 보인다"까지 왔다면 여기로 바로 이어 붙인다.
  - [2.5) Command DTO Vs Query View Naming Checklist](./command-dto-vs-query-view-naming-checklist.md) `선택` — "`Command`, `SupportSnapshot`, `View`, `Response` 이름을 어디에 붙이고 어떤 패키지에 둘지 빠르게 고르는 naming bridge"
- [3) Persistence Model Leakage Anti-Patterns](./persistence-model-leakage-anti-patterns.md) `선택` — "JPA/ORM 세부가 서비스 코드로 새고 있지 않은가?"
- [3.2) Service Contract Smell Cards](./service-contract-smell-cards.md) `선택` — "service 메서드가 `ResponseEntity`, `Pageable`, `MultipartFile`, `OrderEntity` 같은 타입을 직접 말하고 있지 않은가?"
...
> service 메서드 시그니처에 `ResponseEntity`, `Pageable`, `MultipartFile`, `OrderEntity` 같은 타입이 보일 때 웹/프레임워크/영속 세부가 유스케이스 계약을 덮는지 바로 점검하게 돕는 compact beginner primer
