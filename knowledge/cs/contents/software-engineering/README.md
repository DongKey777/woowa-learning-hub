...
- [`flush()` vs commit: `AFTER_COMMIT` 초심자 브리지 카드](#flush-vs-commit-after_commit-초심자-브리지-카드)
...
retrieval-anchor-keywords: software engineering readme, 처음 백엔드 미션 뭐부터, 메서드가 너무 길어요, controller/service/repository 구조, dto vs vo vs entity 차이, 첫 테스트 선택표, architecture vs contract beginner, service signature boundary, controller service contract 헷갈려요, pageable responseentity entity leak, entity leakage checklist, entity leakage module dto, 엔티티 누수 체크리스트, 모듈 api 계약, cross-module dto, module api contract beginner, 다른 모듈 dto 어떻게 넘겨요, entity leakage 다음 문서, 처음이라 헷갈려요
...
| [DTO, VO, Entity 기초](./dto-vo-entity-basics.md) | [30초 계약-레이어-테스트 연결표](#30초-계약-레이어-테스트-연결표) | [테스트 전략 기초](./test-strategy-basics.md) 또는 [계층형 아키텍처 기초](./layered-architecture-basics.md) |
...
| 5 | "`ResponseEntity`나 `Entity`가 Service 시그니처에 보여요. 구조 문제예요, 계약 문제예요?" | [30초 아키텍처 vs 계약 경계 결정표](#30초-아키텍처-vs-계약-경계-결정표) | `자리`와 `시그니처`를 나눈 뒤 [Service Contract Smell Cards - self-check 3문항](./service-contract-smell-cards.md#이건-레이어-문제인가-계약-문제인가-self-check-3문항) 또는 [Module API DTO Patterns](./module-api-dto-patterns.md)로 한 칸만 이동한다. |
...
| `ResponseEntity`, `Pageable`, `MultipartFile`, `OrderEntity`가 `Service` 메서드 시그니처에 보인다 | 계약/경계 문제 | [Service Contract Smell Cards](./service-contract-smell-cards.md) | 먼저 [self-check 3문항](./service-contract-smell-cards.md#이건-레이어-문제인가-계약-문제인가-self-check-3문항)으로 `레이어 vs 계약`을 가른 뒤, `Command/Query/Result` 후보 1개만 고른다 |
| `Entity`를 응답으로 내보내거나 `Request DTO`를 `Repository`까지 넘기고 있다 | 아키텍처 + 계약 혼합 | [DTO, VO, Entity 기초](./dto-vo-entity-basics.md) | `Request DTO -> Command -> Entity -> Response DTO` 흐름을 본 뒤 [계층형 아키텍처 기초](./layered-architecture-basics.md)에서 자리를 다시 확인한다 |
...
- Beginner 경계/영속성 빠른 경로:
  - [1) Persistence Follow-up Question Guide](./persistence-follow-up-question-guide.md) `필수` — "`Repository Interface Contract Primer`부터 `Persistence Model Leakage Anti-Patterns`까지를 한 질문형 결정표로 묶어, 지금 막힌 지점을 먼저 고르게 돕는다."
...
- 빠른 예시: 할인 금액 계산식 변경이면 `unit test`, `@WebMvcTest`로 요청 검증을 확인하면 `slice test`, 주문 저장과 재고 차감이 같은 트랜잭션으로 묶이는지 보면 `app integration test`다.
...
| `queue`가 worker handoff인지 BFS 도구인지 모르겠어요 | [Backend Data-Structure Starter Pack](../data-structure/backend-data-structure-starter-pack.md) | `worker 순서`면 [Service 계층 기초](./service-layer-basics.md) -> [Ports and Adapters Beginner Primer](./ports-and-adapters-beginner-primer.md) -> [Message-Driven Adapter Example](./message-driven-adapter-example.md), `최소 이동 횟수`면 [알고리즘 README - BFS, Queue, Map 먼저 분리하기](../algorithm/README.md#bfs-queue-map-먼저-분리하기) -> [DFS와 BFS 입문](../algorithm/dfs-bfs-intro.md) | [같은 주문 예시로 보는 1회 pass](#같은-주문-예시로-보는-1회-pass) |
| `가까운 칸부터`와 `consumer 순서`가 같은 말처럼 보여요 | [DFS와 BFS 입문](../algorithm/dfs-bfs-intro.md) | [큐 기초](../data-structure/queue-basics.md)로 다시 비교하고, `비용/가중치`가 붙으면 [BFS vs Dijkstra shortest path mini card](../algorithm/bfs-vs-dijkstra-shortest-path-mini-card.md)까지만 간다 | [길 잃었을 때 복귀 경로](#길-잃었을-때-복귀-경로) |

- 이 표의 핵심은 `primer -> follow-up -> 복귀` 3칸만 유지하는 것이다.
- beginner 단계에서는 `retry`, `scheduler`, `distributed queue`, `운영 incident 문서`를 이 표보다 먼저 열지 않는다.
...
> 특히 공개 API에서 누수를 찾았다면, 바로 다음 문서는 [Module API DTO Patterns](./module-api-dto-patterns.md)다.
...
> 초심자가 `모듈 API 계약`, `cross-module dto`, `다른 모듈에 entity를 넘겨도 되나요?`처럼 검색해도 바로 붙도록 만든 beginner primer entrypoint
> `Entity Leakage Review Checklist`에서 "모듈 API로 entity가 샌다"까지 확인한 뒤 바로 이어 읽는 다음 문서다.
