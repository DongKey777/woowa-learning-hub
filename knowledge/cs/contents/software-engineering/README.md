...
- [테스트 전략 기초](#테스트-전략-기초)
- [트랜잭셔널 테스트 rollback vs commit 경계 카드](#트랜잭셔널-테스트-rollback-vs-commit-경계-카드)
...
| 4 | "`DTO`를 `Entity`로 바로 넘겨도 되나요?, `모듈 API 계약`이나 `cross-module dto`는 어디서 시작하나요?" | [DTO, VO, Entity 기초](./dto-vo-entity-basics.md) `필수` → [Module API DTO Patterns](./module-api-dto-patterns.md) `선택` — beginner용 `모듈 API 계약` primer → [예외 처리 기초](./exception-handling-basics.md) `필수` |
| 5 | "`ResponseEntity`나 `Entity`가 Service 시그니처에 보여요. 구조 문제예요, 계약 문제예요?" | [30초 아키텍처 vs 계약 경계 결정표](#30초-아키텍처-vs-계약-경계-결정표) | `자리`와 `시그니처`를 나눈 뒤 [Service Contract Smell Cards](./service-contract-smell-cards.md) 또는 [Module API DTO Patterns](./module-api-dto-patterns.md)로 한 칸만 이동한다. |
...
| `ResponseEntity`, `Pageable`, `MultipartFile`, `OrderEntity`가 `Service` 메서드 시그니처에 보인다 | 계약/경계 문제 | [Service Contract Smell Cards](./service-contract-smell-cards.md) | 웹/JPA 타입을 `Command`, `Query`, `Result` 같은 유스케이스 언어로 바꿀 후보 1개만 고른다 |
...
- target query: `controller service contract 헷갈려요`, `service signature entity pageable responseentity`, `architecture vs contract beginner`.
...
빠른 탐색은 software-engineering primer만 두고, adjacent-category handoff는 여기로 모은다.

- 옆 카테고리로 잠깐 갔다 와도 복귀는 [30초 starter 지도](#30초-starter-지도), [같은 주문 예시로 보는 1회 pass](#같은-주문-예시로-보는-1회-pass), [길 잃었을 때 복귀 경로](#길-잃었을-때-복귀-경로) 셋 중 하나로만 잡는다.
- `queue`라는 단어가 보여도 바로 운영 문서로 가지 않는다. `consumer/worker 순서`면 [큐 기초](../data-structure/queue-basics.md) -> [ArrayDeque vs BlockingQueue 서비스 handoff 프라이머](../data-structure/arraydeque-vs-blockingqueue-service-handoff-primer.md) -> [Message-Driven Adapter Example](./message-driven-adapter-example.md), `가까운 칸부터/최소 이동 횟수`면 [알고리즘 README - BFS, Queue, Map 먼저 분리하기](../algorithm/README.md#bfs-queue-map-먼저-분리하기) -> [DFS와 BFS 입문](../algorithm/dfs-bfs-intro.md) -> [BFS vs Dijkstra shortest path mini card](../algorithm/bfs-vs-dijkstra-shortest-path-mini-card.md) 순서로만 내려간다.
...
| `queue`가 worker handoff인지 BFS 도구인지 모르겠어요 | [Backend Data-Structure Starter Pack](../data-structure/backend-data-structure-starter-pack.md) | [알고리즘 README - BFS, Queue, Map 먼저 분리하기](../algorithm/README.md#bfs-queue-map-먼저-분리하기) 또는 [Message-Driven Adapter Example](./message-driven-adapter-example.md) | [같은 주문 예시로 보는 1회 pass](#같은-주문-예시로-보는-1회-pass) |
...
- `Service` 책임을 읽다가 `List`/`Set`/`Map`이나 `Optional` 때문에 규칙 설명이 흐려지면, 바로 `TreeMap`, mutable key, sorted collection deep dive로 내려가지 말고 `software-engineering primer -> language primer -> data-structure primer -> service primer` 사다리를 먼저 탄다. 안전한 순서는 [Java 컬렉션 프레임워크 입문](../language/java/java-collections-basics.md) -> [Backend Data-Structure Starter Pack](../data-structure/backend-data-structure-starter-pack.md) -> [Service 계층 기초](./service-layer-basics.md)다. 정렬/범위 조회가 실제로 필요할 때만 그다음 [HashMap, TreeMap, LinkedHashMap Beginner Selection Primer](../data-structure/hashmap-treemap-linkedhashmap-beginner-selection-primer.md) 또는 [큐 기초](../data-structure/queue-basics.md)로 한 칸 더 간다. `처음`, `왜 queue가 나오지`, `헷갈려요`, `서비스 규칙보다 컬렉션 선택이 먼저 막혀요` 같은 beginner query는 이 ladder를 먼저 타고, `가까운 칸부터`, `최소 이동 횟수`처럼 계산 질문으로 바뀔 때만 [Backend Algorithm Starter Pack](../algorithm/backend-algorithm-starter-pack.md)으로 넘긴다.
...
## 트랜잭셔널 테스트 rollback vs commit 경계 카드

- [트랜잭셔널 테스트 rollback vs commit 경계 카드](./transactional-test-rollback-vs-commit-boundary-card.md)

> `@Transactional` 테스트가 초록인데 왜 실제 commit 뒤 동작은 다를 수 있는지 초심자 기준으로 `트랜잭션 안 검증`과 `commit 뒤 검증`을 짧게 나눠 주는 follow-up card
