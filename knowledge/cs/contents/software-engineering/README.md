...
retrieval-anchor-keywords: software engineering readme, 처음 백엔드 미션 뭐부터, 코드리뷰 용어 정리, 메서드가 너무 길어요, controller가 너무 많은 걸 알아요, 왜 서버 전체 테스트부터, service 테스트가 통합테스트인가요, controller/service/repository 구조, 서비스 레이어 뭐하는 곳, dto vs vo vs entity 차이, 예외 어디서 잡나요, 첫 테스트 선택표, 어디부터 읽어야 하나, beginner software engineering path, question based starter table
...
| `기능은 그대로인데 구조만 정리하고 싶어요`, `리팩토링이 무서워요` | [리뷰 문장을 바로 번역하는 1분 카드](#리뷰-문장을-바로-번역하는-1분-카드) | [리팩토링 기초](./refactoring-basics.md) -> [테스트 전략 기초](./test-strategy-basics.md) |
...
| 리팩토링 pass | "기능은 그대로인데 읽기 경로만 짧게 만들고 싶나?" | [리팩토링 기초](./refactoring-basics.md) |
...
|---|---|---|
| 1 | "가독성 이슈 분기: 코드가 읽히지 않을 때 어디부터 고치지?" | [읽기 좋은 코드, 레이어 분리, 테스트 피드백 루프 입문](./readable-code-layering-test-feedback-loop-primer.md) `필수` - 주문 생성 예시 1개로 `메서드가 길다 -> Controller가 많은 걸 안다 -> 테스트가 무겁다`를 한 번에 번역 |
...
| 4 | "`DTO`를 `Entity`로 바로 넘겨도 되나요?, Entity를 응답으로 내보내도 되나요?" | [DTO, VO, Entity 기초](./dto-vo-entity-basics.md) `필수` (문서 상단의 `request DTO -> service command -> entity 저장` 1분 표로 첫 구현 흐름을 먼저 잡고 읽기 시작) → [Module API DTO Patterns](./module-api-dto-patterns.md) `선택` → [예외 처리 기초](./exception-handling-basics.md) `필수` |

- beginner stop rule: `primer 1개 + 다음 1개`가 정해졌다면 README에서 더 깊게 내려가지 않는다.
- starter 4개를 끝낸 뒤에만 `재시도 2회와 총 3회 시도`, `멱등성`, `운영 복구` 같은 질문을 [Primer On Retry Queue Assertions](./retry-queue-assertions-primer.md#attemptcount를-읽는-최소-기준), [Idempotency, Retry, Consistency Boundaries](./idempotency-retry-consistency-boundaries.md)로 넘긴다.
...
| "첫 테스트 선택표: 무엇부터 쓰면 되지?" | [테스트 전략 기초](./test-strategy-basics.md) `필수` | 4문항 체크리스트와 선택표를 같은 용어로 맞춰 첫 failing test를 빠르게 고르게 돕고, 단위/웹/JPA 미니 시나리오로 starter 경로를 균형 있게 보여 준다. |
...
starter와 10분 follow-up까지 끝냈는데도 운영 질문이 남을 때만 연다. 초심자 첫 진입에서는 `사고 대응 절차`보다 `무엇을 관찰하고 무엇을 줄일지`를 먼저 잡는 순서가 안전하다.
...
- 운영/incident 문서는 `관련 문서 링크로 뒤로 보내는 심화 경로`다. starter 질문이 아직 흔들리면 이 표를 열지 않고 위 primer 3개로 돌아간다.
...
- 구조 primer를 adjacent-category 용어와 연결하고 싶다면 먼저 [Architecture and Layering Fundamentals](./architecture-layering-fundamentals.md)와 [Ports and Adapters Beginner Primer](./ports-and-adapters-beginner-primer.md)에서 `layer`와 `port`를 정리한 뒤, 그다음에만 [Design Pattern: Ports and Adapters vs GoF 패턴](../design-pattern/ports-and-adapters-vs-classic-patterns.md), [Design Pattern: Hexagonal Ports: 유스케이스를 둘러싼 입출력 경계](../design-pattern/hexagonal-ports-pattern-language.md), [System Design: Control Plane / Data Plane Separation 설계](../system-design/control-plane-data-plane-separation-design.md)로 내려간다.
...
- `Service` 책임을 읽다가 컬렉션 선택이 먼저 막히면 [자료구조 README - 초급 10초 라우터](../data-structure/README.md#초급-10초-라우터), `가까운 칸부터`처럼 순서 확장이 핵심이면 [알고리즘 README - BFS, Queue, Map 먼저 분리하기](../algorithm/README.md#bfs-queue-map-먼저-분리하기)로 넘긴 뒤, 과부하가 오면 [길 잃었을 때 복귀 경로](#길-잃었을-때-복귀-경로)로 돌아온다.
...
- `Service` 책임을 읽다가 `List`/`Set`/`Map`이나 `Optional` 때문에 규칙 설명이 흐려지면, 바로 `TreeMap`, mutable key, sorted collection deep dive로 내려가지 말고 `software-engineering primer -> language primer -> data-structure primer` 사다리를 먼저 탄다. 안전한 순서는 [Java 컬렉션 프레임워크 입문](../language/java/java-collections-basics.md) -> [Backend Data-Structure Starter Pack](../data-structure/backend-data-structure-starter-pack.md) -> [HashMap, TreeMap, LinkedHashMap Beginner Selection Primer](../data-structure/hashmap-treemap-linkedhashmap-beginner-selection-primer.md) 또는 [큐 기초](../data-structure/queue-basics.md)다. `처음`, `왜 queue가 나오지`, `헷갈려요`, `서비스 규칙보다 컬렉션 선택이 먼저 막혀요` 같은 beginner query는 이 ladder를 먼저 타고, `가까운 칸부터`, `최소 이동 횟수`처럼 계산 질문으로 바뀔 때만 [Backend Algorithm Starter Pack](../algorithm/backend-algorithm-starter-pack.md)으로 넘긴다.
