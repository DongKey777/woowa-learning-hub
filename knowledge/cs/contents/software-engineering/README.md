...
retrieval-anchor-keywords: software engineering navigator, software engineering primer route, 처음 소프트웨어 공학 뭐부터, software engineering what is, basics, beginner entrypoint, primer bridge, deep dive catalog, incident follow-up, 메서드가 너무 길어요, controller가 너무 많은 걸 알아요, 첫 테스트가 막혀요, 헷갈려요

이 `README`는 software-engineering 카테고리의 `category navigator`다.
초심자 기준으로는 문서가 많아 보여도 `primer entry`, `primer bridge`, `deep dive catalog`, `advanced follow-up` 네 shelf만 먼저 구분하면 된다.

| 지금 눈앞의 질문 | 먼저 읽을 shelf | 첫 클릭 | 바로 다음 한 걸음 |
|---|---|---|---|
| `처음`, `뭐예요`, `what is`, `basics` | `primer entry` | [처음 읽는다면 이 3단계부터](#처음-읽는다면-이-3단계부터) | [Beginner 30-Minute Entry Subset](#beginner-30-minute-entry-subset)에서 primer 1개만 고른다 |
| `메서드가 너무 길어요`, `Controller가 너무 많은 걸 알아요`, `첫 테스트가 막혀요` | `primer bridge` | [30초 starter 지도](#30초-starter-지도) | 증상에 맞는 primer 1개만 연다 |
| `trade-off`, `boundary`, `왜 이 설계를 택하죠?` | `deep dive catalog` | [입문 이후: catalog로 넘어가는 기준](#입문-이후-catalog로-넘어가는-기준) | [최신 설계 / 거버넌스 catalog](#최신-설계--거버넌스-catalog)에서 한 묶음만 고른다 |
| `incident`, `rollback`, `runbook`, `recovery`, `on-call` | `advanced follow-up` | [운영/incident 심화 follow-up](#운영incident-심화-follow-up) | starter primer를 아직 안 읽었다면 먼저 위로 돌아간다 |

- beginner-safe 순서는 `category navigator -> primer entry -> primer bridge -> deep dive catalog -> advanced follow-up`이다.
- `playbook`, `runbook`, `rollback`, `governance`, `cutover`는 useful해도 초심자 첫 클릭 shelf는 아니다.

<details>
<summary>Table of Contents</summary>

- [빠른 탐색](#빠른-탐색)
- [같은 주문 예시로 보는 1회 pass](#같은-주문-예시로-보는-1회-pass)
- [자주 헷갈리는 3문장 빠른 비교](#자주-헷갈리는-3문장-빠른-비교)
...
- [리팩토링 피드백 문장 라우터](#리팩토링-피드백-문장-라우터)
...
- [테스트 전략 기초](#테스트-전략-기초)
- [Browser E2E cost-vs-signal checklist](#browser-e2e-cost-vs-signal-checklist)
- [컴포넌트 테스트 vs 브라우저 E2E 경계 카드](#컴포넌트-테스트-vs-브라우저-e2e-경계-카드)
- [MSW stub vs 실제 백엔드 컴포넌트 테스트 카드](#msw-stub-vs-실제-백엔드-컴포넌트-테스트-카드)
- [`flush()` vs commit: `AFTER_COMMIT` 초심자 브리지 카드](#flush-vs-commit-aftercommit-초심자-브리지-카드)
- [리팩토링과 첫 failing test 연결 브리지](#리팩토링과-첫-failing-test-연결-브리지)
- [Service Refactor First-Test Examples Pack](#service-refactor-first-test-examples-pack)
- [Dummy vs Stub 초심자 미니 카드](#dummy-vs-stub-초심자-미니-카드)
- [Fake vs Mock 첫 테스트 프라이머](#fake-vs-mock-첫-테스트-프라이머)
- [Stub vs Spy 첫 테스트 프라이머](#stub-vs-spy-첫-테스트-프라이머)
- [Outbound Notifier Mock Boundary Primer](#outbound-notifier-mock-boundary-primer)
...
- [Manual Review Backlog Primer](#manual-review-backlog-primer)
...
- [Outbox vs Inbox 주문 예시 입문 카드](#outbox-vs-inbox-주문-예시-입문-카드)
...
- [Entity Leakage Review Checklist](#entity-leakage-review-checklist)
- [JPA Lazy Loading and N+1 Boundary Smells](#jpa-lazy-loading-and-n1-boundary-smells)
- [`Pageable` leak vs query-model pagination bridge](#pageable-leak-vs-query-model-pagination-bridge)
...
- [Command DTO Vs Query View Naming Checklist](#command-dto-vs-query-view-naming-checklist)
...
retrieval-anchor-keywords: software engineering readme, 처음 백엔드 미션 뭐부터, 메서드가 너무 길어요, controller service repository 구조, dto vs vo vs entity 차이, 첫 테스트 선택표, architecture vs contract beginner, service signature boundary, service 시그니처 leak, repository contract vs persistence, entity leakage checklist, entity가 샌다, 엔티티 누수 체크리스트, 모듈 api 계약, 다른 모듈 dto 어떻게 넘겨요, 공개 api에서 entity 누수

## 역할별 시작선

이 `README`는 `category navigator`다. 초심자 첫 클릭은 `primer`, 운영/장애 절차는 `follow-up`, 설계 비교 묶음은 `deep dive catalog`로 읽으면 덜 헷갈린다.

| 지금 보이는 말 | 이 README에서 붙일 역할 | 먼저 여는 곳 | 아직 미루는 것 |
|---|---|---|---|
| `처음`, `뭐부터`, `헷갈려요`, `basics`, `what is` | `primer entry` | [처음 읽는다면 이 3단계부터](#처음-읽는다면-이-3단계부터), [Beginner 30-Minute Entry Subset](#beginner-30-minute-entry-subset) | rollout, incident, governance deep dive |
| `메서드가 길어요`, `Controller가 너무 많은 걸 알아요`, `첫 테스트가 막혀요` | `primer bridge` | [빠른 탐색](#빠른-탐색), [30초 starter 지도](#30초-starter-지도) | catalog 전체 정독 |
| `trade-off`, `boundary`, `왜 이 설계를 택하죠?` | `deep dive catalog` | [입문 이후: catalog로 넘어가는 기준](#입문-이후-catalog로-넘어가는-기준), [최신 설계 / 거버넌스 catalog](#최신-설계--거버넌스-catalog) | starter primer로 다시 내려가야 하는데 catalog만 더 읽기 |
| `incident`, `runbook`, `rollback`, `on-call`, `recovery` | `advanced follow-up` | [운영/incident 심화 follow-up](#운영incident-심화-follow-up) | beginner entrypoint로 오해하고 첫 클릭으로 쓰기 |

- 안전한 기본 순서는 `category navigator -> primer -> primer bridge -> deep dive catalog -> advanced follow-up`이다.
- `incident`, `runbook`, `rollback`, `governance` 같은 단어가 먼저 보여도, starter 질문이 아직 남아 있으면 위 순서를 거꾸로 뛰어넘지 않는다.

## 길 잃었을 때 복귀 경로

이 README는 software-engineering 카테고리 navigator다. primer를 읽다가 과부하가 오면 `증상 문구 -> 책임 위치 -> 첫 테스트` 순서로 다시 돌아오면 된다.

| 지금 다시 묻고 싶은 증상 | 여기로 되돌아오기 | 바로 다음 안전한 한 걸음 |
|---|---|---|
| `처음 백엔드 미션인데 뭐부터 읽어야 하는지 다시 잊었어요` | [처음 읽는다면 이 3단계부터](#처음-읽는다면-이-3단계부터) | [우테코 백엔드 미션 선행 개념 입문](./woowacourse-backend-mission-prerequisite-primer.md) |
| `메서드가 너무 길어요`, `한 메서드가 너무 많은 질문에 답해요` | [빠른 탐색](#빠른-탐색) | [읽기 좋은 코드, 레이어 분리, 테스트 피드백 루프 입문](./readable-code-layering-test-feedback-loop-primer.md) |
| `Controller가 너무 많은 걸 알아요`, `Service가 다 떠안고 있어요` | [같은 주문 예시로 보는 1회 pass](#같은-주문-예시로-보는-1회-pass) | [계층형 아키텍처 기초](./layered-architecture-basics.md) -> [Service 계층 기초](./service-layer-basics.md) |
| `왜 서버 전체 테스트부터 돌리죠?`, `Service 테스트가 통합테스트인가요?` | [30초 starter 지도](#30초-starter-지도) | [테스트 전략 기초](./test-strategy-basics.md) |
| `기능은 그대로인데 구조만 정리하고 싶어요`, `리팩토링이 무서워요` | [리뷰 문장을 바로 번역하는 1분 카드](#리뷰-문장을-바로-번역하는-1분-카드) | [리팩토링과 첫 failing test 연결 브리지](./refactoring-first-failing-test-bridge.md) -> [테스트 전략 기초](./test-strategy-basics.md) |
| `컬렉션 선택 때문에 규칙 설명이 흐려져요` | [연결해서 보면 좋은 문서 (cross-category bridge)](#연결해서-보면-좋은-문서-cross-category-bridge) | [자료구조 README - 초급 10초 라우터](../data-structure/README.md#초급-10초-라우터) |
| `queue/map는 정리했는데 아직도 이게 BFS 계산 규칙인지 Service 책임인지 모르겠어요` | [연결해서 보면 좋은 문서 (cross-category bridge)](#연결해서-보면-좋은-문서-cross-category-bridge) | [알고리즘 README - BFS, Queue, Map 먼저 분리하기](../algorithm/README.md#bfs-queue-map-먼저-분리하기) -> [30초 starter 지도](#30초-starter-지도) |

## 프라이머에서 다시 돌아오는 자리

primer를 읽고 나서 `왜 DTO가 또 나오죠?`, `언제 테스트로 돌아가요?`, `자료구조/알고리즘 문제랑 어디서 갈라져요?`가 섞이면 아래 표로 복귀 지점만 잡으면 된다.

| 방금 읽은 primer | 이 README에서 다시 붙일 자리 | 바로 다음 한 걸음 |
|---|---|---|
| [읽기 좋은 코드, 레이어 분리, 테스트 피드백 루프 입문](./readable-code-layering-test-feedback-loop-primer.md) | [30초 starter 지도](#30초-starter-지도) | [계층형 아키텍처 기초](./layered-architecture-basics.md) |
| [계층형 아키텍처 기초](./layered-architecture-basics.md) | [같은 주문 예시로 보는 1회 pass](#같은-주문-예시로-보는-1회-pass) | [Service 계층 기초](./service-layer-basics.md) |
| [Service 계층 기초](./service-layer-basics.md) | [자주 헷갈리는 3문장 빠른 비교](#자주-헷갈리는-3문장-빠른-비교) | [테스트 전략 기초](./test-strategy-basics.md) |
| [DTO, VO, Entity 기초](./dto-vo-entity-basics.md) | [30초 아키텍처 vs 계약 경계 결정표](#30초-아키텍처-vs-계약-경계-결정표) | `계약 문제`인지 `레이어 문제`인지 다시 자른 뒤 [테스트 전략 기초](./test-strategy-basics.md) 또는 [계층형 아키텍처 기초](./layered-architecture-basics.md)로 한 칸만 간다 |
## 프라이머에서 다시 돌아오는 자리 (계속 1-2)

| 방금 읽은 primer | 이 README에서 다시 붙일 자리 | 바로 다음 한 걸음 |
|---|---|---|
| [테스트 전략 기초](./test-strategy-basics.md) | [길 잃었을 때 복귀 경로](#길-잃었을-때-복귀-경로) | [리팩토링과 첫 failing test 연결 브리지](./refactoring-first-failing-test-bridge.md) |
| [우테코 백엔드 미션 선행 개념 입문](./woowacourse-backend-mission-prerequisite-primer.md) | [처음 읽는다면 이 3단계부터](#처음-읽는다면-이-3단계부터) | [읽기 좋은 코드, 레이어 분리, 테스트 피드백 루프 입문](./readable-code-layering-test-feedback-loop-primer.md)으로 starter를 고정한다 |
| [리팩토링 기초](./refactoring-basics.md) | [리뷰 문장을 바로 번역하는 1분 카드](#리뷰-문장을-바로-번역하는-1분-카드) | [리팩토링과 첫 failing test 연결 브리지](./refactoring-first-failing-test-bridge.md) |
| [자료구조 README - 초급 10초 라우터](../data-structure/README.md#초급-10초-라우터) | [길 잃었을 때 복귀 경로](#길-잃었을-때-복귀-경로) | `List/Set/Map/queue`를 정리했으면 [30초 starter 지도](#30초-starter-지도)로 돌아와 `읽기 / 레이어 / 테스트` 중 한 축만 다시 고른다 |
| [Stateless 백엔드, 캐시, 데이터베이스, 큐 스타터 팩](../system-design/stateless-backend-cache-database-queue-starter-pack.md) | [길 잃었을 때 복귀 경로](#길-잃었을-때-복귀-경로) | 박스 역할은 잡았지만 미션 코드 책임이 다시 흐려지면 [Service 계층 기초](./service-layer-basics.md)로 복귀하고, system-design follow-up은 [System Design README - Backend Mission -> System Design 안전 handoff](../system-design/README.md#backend-mission---system-design-안전-handoff)에서 한 장만 고른다 |
| [Queue vs BFS vs Priority Queue vs Map Lookup Micro Drill](../data-structure/queue-bfs-priorityqueue-map-lookup-micro-drill.md) | [연결해서 보면 좋은 문서 (cross-category bridge)](#연결해서-보면-좋은-문서-cross-category-bridge) | `queue`가 도구인지 계산 규칙인지 갈랐다면 [30초 starter 지도](#30초-starter-지도)로 돌아와 책임 위치만 다시 읽는다 |

## 프라이머에서 다시 돌아오는 자리 (계속 2)

| 방금 읽은 primer | 이 README에서 다시 붙일 자리 | 바로 다음 한 걸음 |
|---|---|---|
| [Backend Data-Structure Starter Pack](../data-structure/backend-data-structure-starter-pack.md) | [연결해서 보면 좋은 문서 (cross-category bridge)](#연결해서-보면-좋은-문서-cross-category-bridge) | `컬렉션 선택`이 정리됐다면 [30초 starter 지도](#30초-starter-지도)로 돌아와 `레이어`, `테스트`, `책임 분리` 중 지금 막힌 한 축만 고른다 |
| [알고리즘 README - BFS, Queue, Map 먼저 분리하기](../algorithm/README.md#bfs-queue-map-먼저-분리하기) | [연결해서 보면 좋은 문서 (cross-category bridge)](#연결해서-보면-좋은-문서-cross-category-bridge) | `최소 이동/가까운 칸부터`를 정리했으면 [같은 주문 예시로 보는 1회 pass](#같은-주문-예시로-보는-1회-pass)나 [30초 starter 지도](#30초-starter-지도)로 돌아와 계산 규칙이 아니라 책임 위치를 다시 읽는다 |

## 프라이머 복귀 한 줄 규칙

`처음 설계`, `왜 DTO가 많아요`, `헷갈려요` 같은 query는 이 표를 먼저 탄다.
구조보다 `queue/map`이나 `BFS`가 먼저 막히면 [자료구조 README - 초급 10초 라우터](../data-structure/README.md#초급-10초-라우터) 또는 [알고리즘 README - BFS, Queue, Map 먼저 분리하기](../algorithm/README.md#bfs-queue-map-먼저-분리하기)로 우회한다.
`read model`, `projection`, `registry`가 먼저 보이면 [Design Pattern README - Read Model / Projection 빠른 진입](../design-pattern/README.md#read-model--projection-빠른-진입)이나 [네이밍 / 레지스트리 초입](../design-pattern/README.md#네이밍--레지스트리-초입)을 잠깐 찍고, 설명 대상이 다시 `책임 분리`로 돌아오면 [30초 starter 지도](#30초-starter-지도)로 복귀한다.
...
먼저 한 줄 멘탈 모델만 잡고 시작하면 동선이 단순해진다.

| 지금 막힌 축 | 한 줄 질문 | 첫 문서 |
|---|---|---|
| 읽기 좋은 코드 | "한 메서드가 질문 1개만 답하나?" | [읽기 좋은 코드, 레이어 분리, 테스트 피드백 루프 입문](./readable-code-layering-test-feedback-loop-primer.md) |
| 레이어 분리 | "입력, 규칙, 저장이 섞였나?" | [계층형 아키텍처 기초](./layered-architecture-basics.md) |
| Service 책임 | "Controller는 얇은데 Service가 규칙, 조회, 응답 조립을 다 안고 있나?" | [Service 계층 기초](./service-layer-basics.md) |
| DTO/계약 분리 | "`request DTO`를 바로 `Entity`로 넘기거나, `Entity`를 응답으로 그대로 내보내도 되나?" | [DTO, VO, Entity 기초](./dto-vo-entity-basics.md) |
| 테스트 선택 | "지금 확인할 질문이 규칙인가, 경계인가?" | [테스트 전략 기초](./test-strategy-basics.md) |
| 리팩토링 pass | "기능은 그대로인데 읽기 경로만 짧게 만들고 싶나?" | [리팩토링과 첫 failing test 연결 브리지](./refactoring-first-failing-test-bridge.md) |

- 짧게 외우면 `읽기 -> 자리 -> 검증` 순서다.
- `첫 문서 1개 -> 바로 다음 1개`만 정하면 starter로 충분하다.
- 증상별 비교는 바로 아래 [자주 헷갈리는 3문장 빠른 비교](#자주-헷갈리는-3문장-빠른-비교)에서 이어 보면 된다.

## 30초 계약-레이어-테스트 연결표

처음엔 `DTO`, `Service`, `테스트`를 따로 외우기보다, **같은 주문 생성 흐름에서 어디서 모양을 끊고 어디서 검증할지**만 잡으면 된다.

| 지금 보이는 장면 | 먼저 붙일 이름 | starter 문서 | 바로 다음 한 걸음 |
|---|---|---|---|
| `Request DTO`를 그대로 `Repository`까지 넘기고 있다 | 계약/레이어 혼합 | [DTO, VO, Entity 기초](./dto-vo-entity-basics.md) | [계층형 아키텍처 기초](./layered-architecture-basics.md)에서 `입력 / 규칙 / 저장` 자리를 다시 본다 |
| Controller가 규칙까지 처리하면서 `Entity`를 바로 응답으로 내보낸다 | 레이어/계약 혼합 | [계층형 아키텍처 기초](./layered-architecture-basics.md) | [DTO, VO, Entity 기초](./dto-vo-entity-basics.md)에서 `Request DTO -> Command -> Entity -> Response DTO` 흐름을 본다 |
| `quantity` 누락 `400`과 재고 부족 실패를 같은 테스트로 보려다 헷갈린다 | 계약/규칙 분리 | [Controller 계약 변경 vs Service 규칙 변경 첫 failing test 미니 카드](./controller-contract-vs-service-rule-first-test-mini-card.md) | 같은 주문 예시에서 `HTTP 계약`과 `비즈니스 규칙`을 먼저 두 갈래로 자른다 |
| 규칙을 Service로 옮겼는데 어떤 테스트를 붙일지 모르겠다 | 테스트 선택 | [테스트 전략 기초](./test-strategy-basics.md) | `규칙 / 웹 계약 / JPA / 협력 흐름` 중 1개만 고른다 |
| 기능은 그대로인데 DTO 이름과 메서드 단계만 정리하고 싶다 | 리팩토링 pass | [리팩토링 기초](./refactoring-basics.md) | 현재 동작을 잠글 최소 테스트 1개를 먼저 고른다 |

- 짧게 외우면 `모양은 DTO`, `자리는 레이어`, `확인은 테스트`다.
- `처음이라 헷갈려요`, `왜 이렇게 타입이 많아요?`처럼 증상이 추상적이면 이 표에서 가장 가까운 행 1개만 고르면 된다.

## 같은 주문 예시로 보는 1회 pass

초심자는 세 문서를 따로 외우기보다, **같은 주문 생성 코드로 `읽기 -> 자리 -> 검증` 1번만 돌려 보는 것**이 더 빠르다.

| 순서 | 지금 하는 일 | 왜 여기서 멈추나 | 바로 읽을 문서 |
|---|---|---|---|
| 1 | `create()` 안에 `요청 해석 / 재고 규칙 / 저장 / 응답 조립`이 같이 있는지 표시한다 | 먼저 "질문이 많다"는 사실만 잡으면 된다 | [읽기 좋은 코드, 레이어 분리, 테스트 피드백 루프 입문](./readable-code-layering-test-feedback-loop-primer.md#같은-주문-생성-코드로-보는-1회-pass) |
| 2 | `수량은 1 이상`, `재고 부족이면 실패`처럼 업무 규칙 1개만 Controller 밖으로 옮긴다 | 이번 pass에서 옮길 책임 1개면 충분하다 | [계층형 아키텍처 기초](./layered-architecture-basics.md#같은-주문-예시로-보는-1회-pass) |
| 3 | 방금 옮긴 규칙이 깨지지 않게 가장 싼 테스트 1개만 붙인다 | 규칙 1개를 잠그면 다음 분리가 덜 불안하다 | [테스트 전략 기초](./test-strategy-basics.md#첫-테스트-선택표-변경-1건-기준) |
| 4 | 다음 책임 1개로 넘어갈지, 여기서 멈출지 정한다 | `책임 1개 + 테스트 1개`가 끝나면 1회 pass는 완료다 | [리팩토링 기초](./refactoring-basics.md) |

- 이 pass의 핵심은 `구조 전체 교체`가 아니라 `책임 1개 이동 + 테스트 1개 고정`이다.
- 처음부터 정답 구조를 완성하려고 하지 말고, 이번 리뷰 문장을 한 번 처리하는 안전한 이동으로 보면 부담이 줄어든다.
- 짧게 외우면 `안 읽힌다 -> 자리 옮긴다 -> 가장 싼 테스트 1개로 잠근다`다.

## 자주 헷갈리는 3문장 빠른 비교

같은 주문 생성 예시를 봐도, 아래 3문장은 서로 다른 시작점을 가리킨다.

| 들리는 문장 | 먼저 붙일 이름 | 첫 행동 | 시작 문서 |
|---|---|---|---|
| "메서드가 너무 길어요" | 읽기 문제 | 한 메서드가 몇 개 질문에 답하는지 표시한다 | [읽기 좋은 코드, 레이어 분리, 테스트 피드백 루프 입문](./readable-code-layering-test-feedback-loop-primer.md#먼저-자르는-3칸-비교) |
| "Controller가 너무 많은 걸 알아요" | 레이어 문제 | 입력 형식 검증 밖의 규칙이 Controller에 남아 있는지 본다 | [계층형 아키텍처 기초](./layered-architecture-basics.md#controller에-남길-것--service로-옮길-것-빠른-비교) |
| "왜 서버 전체 테스트부터 돌리죠?" | 테스트 문제 | 지금 검증할 질문이 규칙인지 협력인지 먼저 고른다 | [테스트 전략 기초](./test-strategy-basics.md#30초-자가진단-카드-4문항-yesno) |
| "기능은 그대로인데 구조만 정리하고 싶어요" | 리팩토링 문제 | 현재 동작을 잠글 테스트 1개부터 고른다 | [리팩토링 기초](./refactoring-basics.md#같은-주문-예시로-보는-안전한-1회-pass) |

- 짧게 외우면 `길다 -> 질문 수`, `많이 안다 -> 책임 위치`, `무겁다 -> 검증 범위`다.
- 세 문장을 같은 뜻으로 받아들이면 문서는 많이 읽는데 첫 수정이 늦어진다.

## 30초 starter 지도

초심자 기준으로는 문서 제목보다 **"지금 막힌 증상 -> 어디를 읽고 -> 어디서 멈출지"**가 먼저 보여야 한다.

| 지금 바로 보이는 증상 | 먼저 읽을 자리 | 여기서 얻을 한 문장 | 멈추는 기준 |
|---|---|---|---|
| "메서드가 너무 길어요" | [읽기 좋은 코드 입문 - 먼저 자르는 3칸 비교](./readable-code-layering-test-feedback-loop-primer.md#먼저-자르는-3칸-비교) | 먼저 `읽기 / 레이어 / 테스트` 중 무엇인지 나눈다 | 다음 문서 1개가 정해지면 멈춘다 |
| "Controller가 너무 많은 걸 알아요" | [계층형 아키텍처 기초 - 같은 주문 예시로 보는 1회 pass](./layered-architecture-basics.md#같은-주문-예시로-보는-1회-pass) | `규칙 1개 이동 + 테스트 1개` pass로 시작한다 | 입력/규칙/저장 중 어디 책임인지 말하면 멈춘다 |
| "`DTO`를 `Entity`로 바로 넘겨도 되나요?" | [DTO, VO, Entity 기초 - `request DTO -> service command -> entity 저장` 흐름](./dto-vo-entity-basics.md#1분-감각용-request-dto---service-command---entity-저장-흐름) | `전달 계약`, `값 의미`, `식별자 추적`은 다른 질문이다 | `Request DTO`, `Command`, `Entity`, `Response DTO` 중 섞인 지점을 말하면 멈춘다 |
| "왜 서버 전체 테스트부터 돌리죠?" | [테스트 전략 기초 - 30초 자가진단 카드](./test-strategy-basics.md#30초-자가진단-카드-4문항-yesno) | `규칙 / 웹 계약 / JPA / 협력 흐름` 중 하나만 고른다 | 첫 테스트 1개가 정해지면 멈춘다 |
 
- 가장 많이 쓰는 순서는 `증상 문구 -> 책임 위치 -> 첫 테스트`다.
- 흔한 오해는 `메서드 분리 = 책임 분리 완료`다. 이 오해는 각 primer의 혼동 표에서 다시 끊는다.

## 30초 starter 추가 분기

starter 1장을 읽은 뒤에는 아래 follow-up 분기로만 이어 가면 된다.

| 지금 바로 보이는 증상 | 먼저 읽을 자리 | 여기서 얻을 한 문장 | 멈추는 기준 |
|---|---|---|---|
| "`@SpringBootTest`인데 왜 E2E가 아니죠?" | [테스트 전략 기초 - `@SpringBootTest`인데 왜 아직 E2E가 아니죠?](./test-strategy-basics.md) | 큰 컨텍스트와 사용자 여정 검증을 분리한다 | 내부 협력인지, 바깥 여정인지 말하면 멈춘다 |
| "구조를 안전하게 정리하고 싶어요" | [읽기 좋은 코드 입문 - 같은 주문 생성 코드로 보는 1회 pass](./readable-code-layering-test-feedback-loop-primer.md#같은-주문-생성-코드로-보는-1회-pass) | `책임 1개 분리 -> 테스트 1개 고정`만 한다 | 옮길 책임 1개와 테스트 1개가 정해지면 멈춘다 |

- follow-up에서 자주 끊기는 오해는 `Service 테스트 = 자동 통합 테스트`다.

## 리뷰 문장을 바로 번역하는 1분 카드

이 카테고리는 용어를 많이 아는 사람보다, **리뷰 문장을 바로 다음 행동으로 번역하는 사람**에게 더 빨리 도움이 된다.

| 리뷰나 셀프리뷰 문장 | 먼저 붙일 이름 | 첫 primer | 바로 다음 한 걸음 |
|---|---|---|---|
| "메서드가 길어서 읽기 힘들어요" | 읽기 문제 | [읽기 좋은 코드 입문](./readable-code-layering-test-feedback-loop-primer.md) | `한 메서드가 질문 1개만 답하는가`부터 본다 |
| "Controller가 규칙까지 다 알아요" | 레이어 문제 | [계층형 아키텍처 기초](./layered-architecture-basics.md) | `입력 / 규칙 / 저장` 중 어디가 섞였는지 표시한다 |
| "Service 테스트가 왜 이렇게 무거워요?" | 테스트 문제 | [테스트 전략 기초](./test-strategy-basics.md) | `규칙 / 웹 계약 / JPA / 협력 흐름` 중 어떤 질문인지 고른다 |
| "구조를 바꾸고 싶은데 안전한지 모르겠어요" | 리팩토링 pass | [리팩토링 기초](./refactoring-basics.md) | 현재 동작을 잠글 최소 테스트 1개를 먼저 고른다 |

- 짧게 외우면 `읽기 = 질문 분리`, `레이어 = 자리 분리`, `테스트 = 검증 범위 분리`다.
- 이 3개는 다른 과목이 아니라, 한 PR을 `설명하는 말`, `코드를 놓는 자리`, `검증하는 방법`으로 번역한 것이다.

## 15분 starter 동선

처음 보는 초심자라면 이 카테고리를 처음부터 끝까지 읽지 않는다. 아래처럼 **증상 1개 -> primer 1개 -> 바로 다음 문서 1개**만 고르면 된다.

| 리뷰나 셀프리뷰에서 바로 들리는 말 | 먼저 읽을 primer | 바로 다음 한 걸음 |
|---|---|---|
| "메서드가 너무 길어요" | [읽기 좋은 코드 입문 - 먼저 자르는 3칸 비교](./readable-code-layering-test-feedback-loop-primer.md#먼저-자르는-3칸-비교) | [리팩토링 기초](./refactoring-basics.md)에서 `테스트로 현재 동작 고정 -> 구조 1단계 정리` 흐름만 본다 |
| "Controller가 너무 많은 걸 알아요" | [계층형 아키텍처 기초 - 같은 주문 예시로 보는 1회 pass](./layered-architecture-basics.md#같은-주문-예시로-보는-1회-pass) | [Service 계층 기초](./service-layer-basics.md#검증트랜잭션-경계-quick-sync)에서 `검증/트랜잭션 경계 quick sync`만 본다 |
| "왜 서버 전체 테스트부터 돌리죠?" | [테스트 전략 기초 - 30초 자가진단 카드](./test-strategy-basics.md#30초-자가진단-카드-4문항-yesno) | [테스트 전략 기초 - 첫 테스트 선택표](./test-strategy-basics.md#첫-테스트-선택표-변경-1건-기준)에서 `규칙 / 웹 계약 / JPA / 협력 흐름` 중 1개만 고른다 |
| "기능은 그대로인데 구조만 정리하고 싶어요" | [리팩토링 기초](./refactoring-basics.md) | [테스트 전략 기초 - 첫 테스트 선택표](./test-strategy-basics.md#첫-테스트-선택표-변경-1건-기준)에서 가장 싼 테스트 1개를 먼저 잠근다 |

- stop rule: 표에서 고른 `primer 1개 + 다음 한 걸음 1개`까지만 읽으면 starter로 충분하다.
- 짧게 외우면 `길다 -> 읽기`, `많이 안다 -> 레이어`, `무겁다 -> 테스트`, `안전하게 정리 -> 리팩토링`이다.

## 처음 읽는다면 이 3단계부터

README를 처음 펼쳤다면, 카테고리 전체를 다 훑지 말고 아래 3단계만 먼저 고르면 된다.

- stop rule: 지금 막힌 질문에 해당하는 행 1개만 고르고, 그 행의 `바로 다음 한 걸음`까지만 열면 starter로 충분하다.

| 지금 막힌 질문 | 먼저 읽을 1개 | 바로 다음 한 걸음 |
|---|---|---|
| "코드가 안 읽히고 리뷰 코멘트가 추상적이다" | [읽기 좋은 코드, 레이어 분리, 테스트 피드백 루프 입문](./readable-code-layering-test-feedback-loop-primer.md) | [계층형 아키텍처 기초](./layered-architecture-basics.md) |
| "변수명이나 메서드명이 안 읽히는데 구조까지 바꿔야 할지 모르겠다" | [클린 코드 기초](./clean-code-basics.md) | [리팩토링 기초](./refactoring-basics.md) |
| "주문 생성 메서드 하나가 길고 테스트도 너무 무겁다" | [읽기 좋은 코드, 레이어 분리, 테스트 피드백 루프 입문](./readable-code-layering-test-feedback-loop-primer.md) | [테스트 전략 기초](./test-strategy-basics.md) |
| "Controller / Service / Repository를 어디서 나눌지 모르겠다" | [계층형 아키텍처 기초](./layered-architecture-basics.md) | [Service 계층 기초](./service-layer-basics.md) |
| "이번 변경의 첫 테스트를 뭘로 시작할지 모르겠다" | [테스트 전략 기초](./test-strategy-basics.md) | [Hexagonal Testing Seams Primer](./hexagonal-testing-seams-primer.md) |
| "리팩토링이 무섭고 어디까지 잘라도 되는지 모르겠다" | [읽기 좋은 코드, 레이어 분리, 테스트 피드백 루프 입문](./readable-code-layering-test-feedback-loop-primer.md) | [리팩토링 기초](./refactoring-basics.md) |

- 멘탈 모델은 단순하다: `읽기 -> 책임 분리 -> 가장 싼 테스트` 순서로만 따라가면 된다.
- 위 표에서 1개를 고른 뒤에는 그 문서의 `관련 문서`와 `다음에 읽을 문서`만 따라가면 된다.

## 같은 주문 생성 변경을 어떻게 읽을까

문서가 많아 보여도 초심자가 실제로 하는 판단은 한 변경을 네 축으로 나눠 보는 것이다. 같은 주문 생성 코드라도 **읽기 문제인지, 레이어 문제인지, 테스트 선택 문제인지, 리팩토링 pass인지**를 먼저 나누면 동선이 짧아진다.

| 지금 보이는 장면 | 붙일 이름 | 먼저 열 문서 | 첫 행동 |
|---|---|---|---|
| `create()`가 검증, 저장, 응답 조립을 한 메서드에서 다 한다 | 읽기 문제 | [읽기 좋은 코드, 레이어 분리, 테스트 피드백 루프 입문](./readable-code-layering-test-feedback-loop-primer.md) | 질문이 몇 개 섞였는지 적어 본다 |
| Controller가 재고 확인이나 중복 검사까지 직접 한다 | 레이어 문제 | [계층형 아키텍처 기초](./layered-architecture-basics.md) | 입력/규칙/저장 중 어디 책임인지 가른다 |
| 이번 변경의 첫 failing test를 뭘로 시작할지 모르겠다 | 테스트 선택 문제 | [테스트 전략 기초](./test-strategy-basics.md) | `규칙 / 웹 계약 / JPA / 협력 흐름` 중 하나만 고른다 |
| 기능은 그대로 두고 메서드 추출, 이름 변경만 안전하게 하고 싶다 | 리팩토링 pass | [리팩토링 기초](./refactoring-basics.md) | 현재 동작을 테스트 1개로 고정한 뒤 구조 1단계만 정리한다 |

- 한 줄로 줄이면 `무슨 문제인가 -> 어느 primer 1개를 열까 -> 첫 행동 1개는 뭔가`만 정하면 starter 구간은 충분하다.

| 리뷰에서 들은 증상 문구 | 이 README에서 먼저 붙일 이름 | 바로 여는 primer |
|---|---|---|
| "메서드가 너무 길어요" | 읽기 문제 | [읽기 좋은 코드, 레이어 분리, 테스트 피드백 루프 입문](./readable-code-layering-test-feedback-loop-primer.md) |
| "Controller가 너무 많은 걸 알아요" | 레이어 문제 | [계층형 아키텍처 기초](./layered-architecture-basics.md) |
| "왜 서버 전체 테스트부터 돌리죠?" | 테스트 문제 | [테스트 전략 기초](./test-strategy-basics.md) |
| "기능은 그대로인데 구조만 정리하고 싶어요" | 리팩토링 문제 | [리팩토링 기초](./refactoring-basics.md) |

- 짧게 외우면 `길다 -> 읽기`, `많이 안다 -> 레이어`, `무겁다 -> 테스트`, `안전하게 정리하고 싶다 -> 리팩토링`이다.

## 30초 멘탈 모델: 읽기, 레이어, 테스트는 같이 움직인다

세 문서를 따로 외우기보다, **리뷰 문장 1개를 `읽기 문제 -> 책임 위치 -> 첫 테스트`로 번역하는 연습**이라고 생각하면 된다.

| 지금 보이는 증상 | 먼저 붙일 이름 | 바로 열 문서 | 첫 확인 |
|---|---|---|---|
| "메서드가 너무 길어요" | 읽기 문제 | [읽기 좋은 코드, 레이어 분리, 테스트 피드백 루프 입문](./readable-code-layering-test-feedback-loop-primer.md) | 질문이 한 메서드에 몇 개 섞였는지 |
| "Controller가 너무 많은 걸 알아요" | 레이어 문제 | [계층형 아키텍처 기초](./layered-architecture-basics.md) | 입력, 규칙, 저장이 섞였는지 |
| "왜 서버 전체 테스트부터 돌리죠?" | 테스트 문제 | [테스트 전략 기초](./test-strategy-basics.md) | 규칙/HTTP 계약/DB 중 무엇을 검증하는지 |

- stop rule도 같이 기억하면 된다: `증상 이름 1개 + 다음 문서 1개 + 첫 테스트 1개`가 정해지면 starter 구간은 충분하다.
- `외부 API`, `이벤트`, `retry` 같은 단어가 보여도 starter에서는 먼저 위 3개 문서만 잡고, 경계 확장은 관련 문서 링크로 넘긴다.

## 막힌 문장을 바로 문서로 옮기는 짧은 번역표

README를 백과사전처럼 펼치지 말고, 리뷰나 셀프리뷰에서 들은 문장을 아래처럼 **증상 문구 그대로** starter primer에 붙이면 된다.

| 지금 들리는 말 | 먼저 판단할 것 1개 | 시작 문서 | 여기서 멈춘 뒤 다음 1개 |
|---|---|---|---|
| "메서드가 너무 길어요", "이름보다 더 많은 일을 해요" | 질문이 여러 개인가 | [읽기 좋은 코드, 레이어 분리, 테스트 피드백 루프 입문](./readable-code-layering-test-feedback-loop-primer.md) | [클린 코드 기초](./clean-code-basics.md) |
| "이름은 안 읽히는데 레이어까지 흔들 문제인지는 모르겠어요" | 먼저 표현 문제인지, 책임 위치 문제인지 | [클린 코드 기초](./clean-code-basics.md) | [계층형 아키텍처 기초](./layered-architecture-basics.md) |
| "Controller가 너무 많은 걸 알아요", "Repository를 바로 불러도 되나요?" | 입력/규칙/저장이 섞였는가 | [계층형 아키텍처 기초](./layered-architecture-basics.md) | [Service 계층 기초](./service-layer-basics.md) |
| "왜 서버 전체 테스트부터 돌리죠?", "첫 테스트를 뭘로 시작하죠?" | 규칙/HTTP 계약/DB 동작 중 무엇을 확인하나 | [테스트 전략 기초](./test-strategy-basics.md) | [Hexagonal Testing Seams Primer](./hexagonal-testing-seams-primer.md) |

- 초심자 기준 stop rule은 간단하다: `다음에 읽을 문서 1개`가 정해졌다면 README에서는 더 내려가지 않아도 된다.
- 반대로 운영, 거버넌스, 대규모 아키텍처 문서를 먼저 펼치고 있다면 starter 질문을 아직 못 고른 상태일 가능성이 크다.

## 한 변경을 읽는 공통 지도

문서가 많아 보여도, 초심자가 실제로 반복하는 판단은 아래 3개뿐이다.

| 지금 바로 답할 질문 | 짧은 해석 | 먼저 열 문서 |
|---|---|---|
| "이 코드는 왜 안 읽히지?" | 한 메서드/클래스가 질문 여러 개에 답하는지 본다 | [읽기 좋은 코드, 레이어 분리, 테스트 피드백 루프 입문](./readable-code-layering-test-feedback-loop-primer.md) |
| "이 규칙은 어디에 둬야 하지?" | 입력, 규칙, 저장 중 어디 책임인지 가른다 | [계층형 아키텍처 기초](./layered-architecture-basics.md) |
| "이 변경은 어디서 검증하지?" | 규칙, HTTP 계약, DB 동작 중 무엇을 확인할지 고른다 | [테스트 전략 기초](./test-strategy-basics.md) |

- 한 줄로 묶으면 `무슨 질문인가 -> 어느 자리에 둘까 -> 가장 싼 테스트는 뭔가`다.
- README에서는 이 3개를 넘는 세부 용어보다, **지금 받은 리뷰 문장을 어느 primer에 연결할지**만 먼저 잡으면 된다.

## 리뷰 코멘트 문구로 바로 고르는 1분 분기

용어가 아니라 **리뷰에서 들은 문장 그대로** 출발하면 README가 훨씬 덜 무겁다.

| 리뷰/셀프리뷰에서 바로 들리는 말 | 먼저 볼 문서 | 왜 여기서 시작하나 | 바로 다음 한 걸음 |
|---|---|---|---|
| "메서드가 너무 길어요", "이름보다 더 많은 일을 해요" | [읽기 좋은 코드, 레이어 분리, 테스트 피드백 루프 입문](./readable-code-layering-test-feedback-loop-primer.md) | 네이밍 문제처럼 보여도 실제로는 책임 혼합인지 먼저 가르게 해 준다 | [클린 코드 기초](./clean-code-basics.md), [계층형 아키텍처 기초](./layered-architecture-basics.md) |
| "Controller가 너무 많은 걸 알아요", "Service 책임이 애매해요" | [계층형 아키텍처 기초](./layered-architecture-basics.md) | 입력/규칙/저장을 어디서 끊을지 가장 먼저 고정해 준다 | [Service 계층 기초](./service-layer-basics.md) |
| "이 변경에 왜 서버 전체 테스트가 필요하죠?" | [테스트 전략 기초](./test-strategy-basics.md) | 규칙, HTTP 계약, DB 동작 중 무엇을 검증하는지부터 좁혀 준다 | [Hexagonal Testing Seams Primer](./hexagonal-testing-seams-primer.md) |

- 기억할 순서는 하나면 충분하다: `증상 문구 -> 책임 위치 -> 첫 테스트`.
- `포트`, `어댑터`, `외부 API`, `이벤트` 같은 단어가 먼저 보여도, 읽기/레이어/첫 테스트를 정리한 뒤에만 [Ports and Adapters Beginner Primer](./ports-and-adapters-beginner-primer.md)로 확장해도 늦지 않다.
...
테스트 문서는 `규칙 / 웹 계약 / JPA / 협력 흐름` 4칸만 먼저 잡고, `retry`, `incident`, `rollback`은 그다음 follow-up 링크로 넘기는 식으로 읽으면 beginner 동선이 덜 무거워진다.

## Beginner Starter Quick Path

| 60분 starter step | 먼저 답할 질문 | 문서 |
|---|---|---|
| 1 | "가독성 이슈 분기: 코드가 읽히지 않을 때 어디부터 고치지?" | [읽기 좋은 코드, 레이어 분리, 테스트 피드백 루프 입문](./readable-code-layering-test-feedback-loop-primer.md) `필수` - 주문 생성 예시 1개로 `읽기 -> 레이어 -> 테스트`를 묶어 본다 |
| 2 | "Controller/Service/Repository 구조는 어떻게 나누지?" | [계층형 아키텍처 기초](./layered-architecture-basics.md) `필수` → [Service 계층 기초](./service-layer-basics.md) `필수` → [테스트 전략 기초](./test-strategy-basics.md#첫-테스트-선택표-변경-1건-기준) |
| 3 | "첫 테스트 선택표: 이번 변경의 첫 테스트는 무엇부터 쓰면 되지?" | [테스트 전략 기초](./test-strategy-basics.md) `필수` - 먼저 `규칙 / 웹 계약 / JPA / 협력 흐름` 중 1개만 고르고, 운영성 분기는 뒤로 미룬다. |
| 4 | "`DTO`를 `Entity`로 바로 넘겨도 되나요?, `구조는 맞는데 계약이 샌다`는 무슨 뜻이죠?, `모듈 API 계약`이나 `cross-module dto`는 어디서 시작하나요?" | [DTO, VO, Entity 기초](./dto-vo-entity-basics.md) `필수` → [Module API DTO Patterns](./module-api-dto-patterns.md) `선택` — beginner용 `모듈 API 계약` primer → [예외 처리 기초](./exception-handling-basics.md) `필수` |
| 5 | "`entity가 샌다`, `service 시그니처 leak`, `ResponseEntity`나 `Entity`가 Service 시그니처에 보여요. 구조 문제예요, 계약 문제예요?" | [30초 아키텍처 vs 계약 경계 결정표](#30초-아키텍처-vs-계약-경계-결정표) | `자리`와 `시그니처`를 나눈 뒤 [Service Contract Smell Cards - self-check 3문항](./service-contract-smell-cards.md#이건-레이어-문제인가-계약-문제인가-self-check-3문항) 또는 [Module API DTO Patterns](./module-api-dto-patterns.md)로 한 칸만 이동한다. |

- beginner stop rule: `primer 1개 + 다음 1개`가 정해졌다면 README에서 더 깊게 내려가지 않는다.
- 운영 분기(`멱등성`, `재시도`, `복구`)는 starter 5개 뒤에만 [Idempotency, Retry, Consistency Boundaries](./idempotency-retry-consistency-boundaries.md)로 넘긴다.

## 30초 아키텍처 vs 계약 경계 결정표

`처음이라 헷갈려요`, "`Service` 시그니처가 이상해 보여요", "`DTO`를 어디서 끊어야 해요?" 같은 질문은 **자리 문제**와 **말투 문제**가 섞여 들리기 쉽다.

아래 표는 같은 증상을 "어느 계층에 둘까?"와 "어떤 타입으로 약속할까?"로 나눠 starter 문서를 symptom-first로 다시 고르게 돕는다.

| 지금 보이는 증상 | 먼저 붙일 이름 | 가장 먼저 열 문서 | 바로 다음 한 걸음 |
|---|---|---|---|
| `Controller`가 재고 검사, 중복 검사, 저장 호출까지 직접 한다 | 아키텍처/레이어 문제 | [계층형 아키텍처 기초](./layered-architecture-basics.md) | `입력 / 규칙 / 저장` 중 어디 책임이 섞였는지 표시한 뒤 [Service 계층 기초](./service-layer-basics.md)로 넘어간다 |
| "`entity가 샌다`", `ResponseEntity`, `Pageable`, `MultipartFile`, `OrderEntity`가 `Service` 메서드 시그니처에 보인다 | 계약/경계 문제 | [Service Contract Smell Cards](./service-contract-smell-cards.md) | 먼저 [self-check 3문항](./service-contract-smell-cards.md#이건-레이어-문제인가-계약-문제인가-self-check-3문항)으로 `레이어 vs 계약`을 가른 뒤, `Command/Query/Result` 후보 1개만 고른다 |
| `Entity`를 응답으로 내보내거나 `Request DTO`를 `Repository`까지 넘기고 있다 | 아키텍처 + 계약 혼합 | [DTO, VO, Entity 기초](./dto-vo-entity-basics.md) | `Request DTO -> Command -> Entity -> Response DTO` 흐름을 본 뒤, `service` 입출력에도 `Entity`가 남아 있다면 [Entity Leakage Review Checklist](./entity-leakage-review-checklist.md)로 가서 "`entity가 샌다` / `service 시그니처 leak`" 체크리스트로 다시 확인한다 |
| 다른 모듈에 aggregate나 `Entity`를 그대로 넘기고 있다 | 모듈 API 계약 문제 | [Module API DTO Patterns](./module-api-dto-patterns.md) | `Command/Query/Result DTO`로 끊을지, 작은 불변 value object만 공유할지 고른다 |
| "`Controller/Service/Repository` 구조는 맞는데 왜 `계약이 샌다`고 할까요?" | 계약 우선, 필요하면 아키텍처 재확인 | [Service Contract Smell Cards](./service-contract-smell-cards.md) | 시그니처 타입을 먼저 정리하고, 여전히 책임이 흔들리면 [계층형 아키텍처 기초](./layered-architecture-basics.md)로 돌아간다 |

- 짧게 외우면 `자리는 레이어`, `말투는 계약`, `둘 다 섞였으면 DTO 흐름부터`다.
- target query: `controller service contract 헷갈려요`, `service signature entity pageable responseentity`.
...
- Beginner 경계/영속성 빠른 경로:
  - [0.5) Repository Interface Contract Primer](./repository-interface-contract-primer.md) `선택` — "`repository는 왜 계약으로 읽죠?`, `다음에 fake를 봐야 하나요?`가 함께 막힐 때 여는 contract entrypoint"
  - [1) Persistence Follow-up Question Guide](./persistence-follow-up-question-guide.md) `필수` — "`repository 다음 뭐 읽지`를 30초 안에 답하도록 `fake / mapping / leakage` 3갈래 mini card와 질문형 결정표를 함께 제공한다."
    `Repository Interface Contract Primer`를 막 읽고 "`이 저장 계약을 test에서 어떻게 재현하지?`"가 남으면 다음 문서는 [Repository Fake Design Guide](./repository-fake-design-guide.md)다.
  - [2) Module API DTO Patterns](./module-api-dto-patterns.md) `선택` — "`모듈 API 계약`, `cross-module dto`, `다른 모듈에 entity를 넘겨도 되나요?`에 바로 붙는 entrypoint"
    `Entity Leakage Review Checklist`에서 "모듈 공개 API에 `Entity`가 보인다"면 여기로 이어 붙인다.
  - [2.5) Command DTO Vs Query View Naming Checklist](./command-dto-vs-query-view-naming-checklist.md) `선택` — "`Command`, `SupportSnapshot`, `View`, `Response` 이름을 빨리 고르는 naming bridge"

## Beginner Persistence Follow-up

- [3) Persistence Model Leakage Anti-Patterns](./persistence-model-leakage-anti-patterns.md) `선택` — "`서비스나 API에 `Entity`가 이미 보여요`, `엔티티를 JSON으로 바로 내보내도 되나요?`를 JPA/ORM 누수로 번역해 읽는 symptom primer"
- [3.2) Service Contract Smell Cards](./service-contract-smell-cards.md) `선택` — "`구조는 맞는데 계약이 샌다`, `service 시그니처에 `ResponseEntity`, `Pageable`, `MultipartFile`, `OrderEntity`가 보여요`를 `계약 vs 웹/JPA 세부`로 번역하는 smell card"
- [3.25) `Pageable` leak vs query-model pagination bridge](./pageable-service-contract-vs-query-model-pagination-bridge.md) `선택` — "`service가 Pageable을 받아요` 다음에 `그럼 pagination은 어디에 둬요?`가 바로 붙을 때, service 계약 leak과 legitimate query repository/adapter pagination을 한 장으로 가르는 intermediate bridge"
- [3.5) Entity Leakage Review Checklist](./entity-leakage-review-checklist.md) `선택` — "JPA `@Entity`가 API DTO, service 계약, cross-module 호출로 새는지 리뷰에서 어디부터 볼까?"
  cross-module 누수라고 판단되면 다음 문서는 [2) Module API DTO Patterns](./module-api-dto-patterns.md)다.
...
| `Layered`와 `Hexagonal` 중 무엇부터 써야 하나? | 대부분은 [계층형 아키텍처 기초](./layered-architecture-basics.md)부터 시작한다. 같은 유스케이스를 여는 채널이 늘거나 Service가 외부 연동 세부를 직접 안기 시작할 때 [Ports and Adapters Beginner Primer](./ports-and-adapters-beginner-primer.md)로 넘어가면 된다. |
...
초심자가 README에서 가장 자주 멈추는 지점은 "`읽기 좋은 코드` 문서와 `계층형`, `테스트` 문서가 각각 따로 노는 것처럼 보일 때"다. 아래 표는 **같은 증상을 어떤 문서 순서로 읽을지**만 빠르게 묶어 둔 연결표다.

| 지금 보이는 증상 | 먼저 읽을 primer | 그다음 바로 붙는 primer | 왜 이 순서가 beginner-safe인가 |
|---|---|---|---|
| 메서드도 길고 Controller 규칙도 많고 테스트도 무겁다 | [읽기 좋은 코드, 레이어 분리, 테스트 피드백 루프 입문](./readable-code-layering-test-feedback-loop-primer.md) | [계층형 아키텍처 기초](./layered-architecture-basics.md) -> [테스트 전략 기초](./test-strategy-basics.md) | 한 증상에 문서 3개를 한 번에 펼치지 않고, `읽기 문제를 문장으로 자른 뒤 -> 책임 위치를 정하고 -> 첫 테스트를 고르는` 순서로 내려갈 수 있다 |
| 메서드가 길고 이름보다 더 많은 일을 한다 | [읽기 좋은 코드, 레이어 분리, 테스트 피드백 루프 입문](./readable-code-layering-test-feedback-loop-primer.md) | [계층형 아키텍처 기초](./layered-architecture-basics.md) | 먼저 질문을 잘게 자르고, 그다음 그 질문을 어느 계층에 둘지 고를 수 있다 |
| Controller/Service 경계가 자꾸 흔들린다 | [계층형 아키텍처 기초](./layered-architecture-basics.md) | [Service 계층 기초](./service-layer-basics.md) | 구조 지도를 먼저 잡아야 Service 책임 범위를 덜 헷갈린다 |
| 테스트가 너무 무겁거나 서버 전체 테스트부터 붙는다 | [테스트 전략 기초](./test-strategy-basics.md) | [Hexagonal Testing Seams Primer](./hexagonal-testing-seams-primer.md) | 첫 테스트 종류를 먼저 고르고, 그다음 경계를 어디까지 끊을지 좁힐 수 있다 |

- 짧게 외우면 `증상 문구 -> primer 1개 -> 바로 다음 primer 1개`만 따라가면 된다.
- 세 문서를 같은 주문 생성 예시로 왕복하고 싶다면, 시작점은 [읽기 좋은 코드, 레이어 분리, 테스트 피드백 루프 입문](./readable-code-layering-test-feedback-loop-primer.md)의 `같은 주문 생성 예시로 가독성, 레이어, 테스트를 한 번에 묶어 보기` 표다.
- 첫 3분 안에 가장 많이 섞이는 오해만 빨리 끊고 싶다면, 같은 문서의 [초심자가 자주 하는 혼동 한 번에 끊기](./readable-code-layering-test-feedback-loop-primer.md#초심자가-자주-하는-혼동-한-번에-끊기) 표부터 보면 `메서드 분리`, `얇은 controller`, `service 테스트`를 서로 다른 질문으로 바로 나눌 수 있다.

## 테스트 용어 한 줄 비교

테스트 용어도 먼저 정의를 외우기보다, **"이번 변경에서 어디까지 연결해 확인하나?"** 로 보면 덜 헷갈린다. 이 README에서는 beginner starter 사다리를 `unit -> slice -> app integration -> E2E` 네 칸으로 고정한다.
...
| `app integration test` | 여러 조각을 실제에 가깝게 함께 붙여 흐름이 이어지는지 보는 테스트 | 트랜잭션, DB, 보안, 메시지, 외부 연동이 실제로 맞물리는지 볼 때 |
| `E2E test` | 브라우저나 외부 클라이언트 기준으로 사용자 흐름을 바깥에서 끝까지 확인하는 테스트 | 로그인, 결제, 외부 인증, 멀티페이지 완료 흐름처럼 사용자 여정 자체가 핵심일 때 |

- 흔한 오해 1: `slice test`는 작은 `integration test`가 아니라, "특정 층만 집중해서 보는 경계 선택"에 가깝다.
- 흔한 오해 2: `@SpringBootTest`가 넓어 보여도 자동으로 `E2E test`가 되는 것은 아니다. 앱 안쪽 협력을 보면 보통 `app integration test`에 가깝다.
- 흔한 오해 3: `seam`은 테스트 종류 이름이 아니라, unit/slice/app integration/E2E 중 어디에서 끊을지 정하게 해 주는 "경계 질문"이다.
- 빠른 예시: 할인 금액 계산식 변경이면 `unit test`, `@WebMvcTest`로 요청 검증을 확인하면 `slice test`, 주문 저장과 재고 차감이 같은 트랜잭션으로 묶이는지 보면 `app integration test`, 로그인 후 주문 완료 페이지까지 실제 브라우저에서 이어지는지 보면 `E2E test`다.
- 다음 문서 연결: 첫 테스트를 고르는 기준은 [테스트 전략 기초](./test-strategy-basics.md), 경계(seam)를 어디에 둘지 더 보고 싶으면 [Hexagonal Testing Seams Primer](./hexagonal-testing-seams-primer.md), controller/message handler 같은 조각별 예시는 [Inbound Adapter Test Slices Primer](./inbound-adapter-test-slices-primer.md), 브라우저 끝단 비용 판단은 [Browser E2E cost-vs-signal checklist](./browser-e2e-cost-vs-signal-checklist.md)로 이어가면 된다.
...
처음 방문자는 `변경 단위`를 어디서 설명/검증하는지 30분 안에 한 바퀴 도는 게 목표다.

| 30분 질문 | 지금 읽을 1개 문서 | 왜 지금 읽나 |
|---|---|---|
| "처음 백엔드 미션 뭐부터?" | [우테코 백엔드 미션 선행 개념 입문](./woowacourse-backend-mission-prerequisite-primer.md) `필수` | 입문 용어를 정리한다. |
| "코드리뷰 용어 정리: `domain rule`, `DTO`, `transaction`" | [Mission Review Vocabulary Primer](./mission-review-vocabulary-primer.md) `필수` | 리뷰 용어를 책임 경계 언어로 번역한다 |
| "`추상화가 어색해요`, `책임이 섞였어요`, `테스트가 너무 무거워요`가 무슨 뜻인지부터 막힌다" | [리팩토링 피드백 문장 라우터](./refactoring-feedback-phrase-router.md) | 리뷰 문장을 primer 1개와 가장 안전한 첫 움직임 1개로 줄여 준다. |
...
| "서비스 레이어 뭐하는 곳?" | [Service 계층 기초](./service-layer-basics.md) `필수` | 업무 규칙 orchestration과 단순 위임을 구분한다. |
| "DTO vs VO vs Entity 차이" | [DTO, VO, Entity 기초](./dto-vo-entity-basics.md) `필수` | 경계 타입을 섞는 초반 실수를 줄인다. |
| "구조는 나눈 것 같은데 왜 `Service` 계약이 이상하다고 하나요?" | [30초 아키텍처 vs 계약 경계 결정표](#30초-아키텍처-vs-계약-경계-결정표) | `레이어 자리`와 `시그니처 계약`을 따로 읽는 분기점이다. |
| "첫 테스트 선택표: 무엇부터 쓰면 되지?" | [테스트 전략 기초](./test-strategy-basics.md) `필수` | 첫 failing test 후보를 빨리 고르게 돕는다. |

## Beginner Starter Stop Line

처음 30분 동선에서는 `retry`, `runbook`, `incident`, `rollback` 같은 운영 단어가 보여도 바로 파고들 필요는 없다. 먼저 `가독성 -> 레이어 -> 첫 테스트` 세 문서만 잡고, 운영 follow-up이 정말 필요할 때만 [Backend Delivery and Observability Foundations Primer](./backend-delivery-observability-foundations-primer.md)나 [Idempotency, Retry, Consistency Boundaries](./idempotency-retry-consistency-boundaries.md)로 좁혀 가면 beginner 동선이 덜 흔들린다.
...
| 레이어 설계 | [Ports and Adapters Beginner Primer](./ports-and-adapters-beginner-primer.md) `선택` | Controller-Service-Repository 책임이 먼저 정리된 뒤, inbound/outbound 경계를 어디서 끊을지 한 단계만 더 붙인다. batch/message 세부는 follow-up으로 미룬다. |
...
| 재시도/멱등성 follow-up | [Idempotency, Retry, Consistency Boundaries](./idempotency-retry-consistency-boundaries.md) `선택` | `왜 서버 전체 테스트부터?`까지는 이미 정리됐고, 이제 retry 정책이나 멱등성 경계를 더 깊게 따져야 할 때만 들어간다. |
| Spring 웹 입구 큰 그림 | [Servlet Filter vs MVC Interceptor Beginner Bridge](./servlet-filter-vs-mvc-interceptor-beginner-bridge.md) `선택` | 처음 배우는데 `filter`와 `interceptor`가 비슷해 보여도, starter 문서로 `Controller 입력 검증`까지 먼저 잡은 뒤에 여는 편이 덜 흔들린다. |

- stop rule: 여기서도 `문서 1개`만 더 연다. `incident`, `runbook`, `rollback`, `governance`처럼 운영 사고가 커지는 단어는 이 10분 표에서 바로 파고들지 않고, 아래 심화 follow-up으로 넘긴다.

## 초심자라면 지금은 뒤로 미루는 문서군

처음부터 깊게 들어가면 좋은 문서가 아니라, **starter 질문을 푼 뒤에 읽을 때 더 잘 들어오는 문서**들이다.

| 문서군 | 지금 바로 중심에 두지 않는 이유 | 먼저 끝내야 하는 starter |
|---|---|---|
| retry, idempotency, batch recovery | 테스트 입문 전에 운영 예외를 먼저 보면 "첫 테스트를 무엇으로 시작할지"가 흐려진다 | [테스트 전략 기초](./test-strategy-basics.md) |
| ports/adapters 이후의 세부 seam, contract, adapter 매트릭스 | 책임 분리 감각이 아직 없으면 패턴 이름과 파일 수만 먼저 늘어난다 | [읽기 좋은 코드, 레이어 분리, 테스트 피드백 루프 입문](./readable-code-layering-test-feedback-loop-primer.md), [계층형 아키텍처 기초](./layered-architecture-basics.md) |
| 운영/배포/거버넌스 catalog | 초반 PR에서는 조직 운영 규칙보다 메서드 책임, 레이어 경계, 첫 테스트 선택이 우선이다 | [읽기 좋은 코드, 레이어 분리, 테스트 피드백 루프 입문](./readable-code-layering-test-feedback-loop-primer.md), [테스트 전략 기초](./test-strategy-basics.md) |

- 빠른 기준: `가독성 -> 레이어 -> 첫 테스트`가 아직 흔들리면 심화 문서보다 starter primer를 다시 여는 편이 낫다.
- 특히 `incident`, `runbook`, `rollback`, `governance` 같은 단어가 보여도, 지금 PR에서 바로 답해야 할 질문이 아니면 [Backend Delivery and Observability Foundations Primer](./backend-delivery-observability-foundations-primer.md), [Idempotency, Retry, Consistency Boundaries](./idempotency-retry-consistency-boundaries.md) 같은 follow-up 링크로만 넘겨도 된다.

## starter 이후 심화 링크로 넘기는 기준

README가 길어 보여도 beginner가 실제로 기억할 기준은 두 줄이면 충분하다.

| 지금 보이는 문장 | 바로 할 일 |
|---|---|
| `메서드가 길다`, `Controller가 너무 많은 걸 안다`, `왜 서버 전체 테스트부터?` | starter primer 3개 안에서 해결한다. |
| `retry`, `incident`, `rollback`, `governance`, `on-call` | starter 본문에서 붙잡지 말고 [Idempotency, Retry, Consistency Boundaries](./idempotency-retry-consistency-boundaries.md), [Backend Delivery and Observability Foundations Primer](./backend-delivery-observability-foundations-primer.md) 같은 심화 링크로만 넘긴다. |

- 이 기준은 beginner 동선을 좁히기 위한 것이다. 고급 문서가 덜 중요하다는 뜻이 아니다.
- 특히 testing/layering/readability 질문을 아직 정리하지 못했으면 운영 문서를 더 읽기보다 starter 문서를 다시 왕복하는 편이 빠르다.

## 운영/incident 심화 follow-up

starter와 10분 follow-up까지 끝냈는데도 운영 질문이 남을 때만 연다. 이 구간은 `primer shelf`가 아니라 `advanced follow-up shelf`다. 초심자 첫 진입에서는 `사고 대응 절차`보다 `무엇을 관찰하고 무엇을 줄일지`를 먼저 잡는 순서가 안전하다.

| 지금 생긴 심화 질문 | 여는 문서 | 왜 여기서 연다 |
|---|---|---|
| 배포 후 무엇을 보고 줄여야 하지? | [Backend Delivery and Observability Foundations Primer](./backend-delivery-observability-foundations-primer.md) | primer 3개로 읽기/레이어/첫 테스트를 정리한 뒤, 로그/메트릭/롤아웃을 한 흐름으로 묶어 보기 좋다. |
| 재시도, 멱등성, 복구 경계가 헷갈린다 | [Idempotency, Retry, Consistency Boundaries](./idempotency-retry-consistency-boundaries.md) | 첫 테스트를 고른 뒤 운영 분기표를 열어도 늦지 않다. |
| incident, runbook, rollback 용어가 갑자기 많아졌다 | [Backend Delivery and Observability Foundations Primer](./backend-delivery-observability-foundations-primer.md) | 개별 장애 문서로 흩어지기 전에 beginner용 운영 entrypoint에서 한 번 묶어 읽는 편이 덜 무겁다. |

- 운영/incident 문서는 `관련 문서 링크로 뒤로 보내는 심화 경로`다. `playbook`, `runbook`, `recovery` 같은 말이 보여도 beginner 첫 클릭이 아니라는 뜻으로 읽는다. starter 질문이 아직 흔들리면 이 표를 열지 않고 위 primer 3개로 돌아간다.

## 10분 분기 고르기 미니 self-check

문서 제목보다 **"지금 막힌 증상"** 으로 고르면 선택 실패가 줄어든다. beginner starter에서는 아래 3개 분기 중 `YES`가 2개 이상인 문서 1개만 먼저 열면 된다.

| 분기 | 이런 증상일 때 연다 | 바로 읽을 문서 |
|---|---|---|
| 코드리딩 | 용어가 섞여서 리뷰 코멘트의 뜻이 안 잡힌다 | [Common-Confusion Wayfinding Notes](./common-confusion-wayfinding-notes.md) |
...
- `운영/배포` 용어는 여기서 별도 분기로 키우지 않는다. `가독성 -> 레이어 -> 첫 테스트`가 정리된 뒤에만 [Backend Delivery and Observability Foundations Primer](./backend-delivery-observability-foundations-primer.md)로 넘긴다.
...
- 운영/배포 질문은 beginner 10분 분기 바깥의 follow-up이다. starter 3개를 정리한 뒤에만 [Backend Delivery and Observability Foundations Primer](./backend-delivery-observability-foundations-primer.md)나 [Idempotency, Retry, Consistency Boundaries](./idempotency-retry-consistency-boundaries.md)로 넘긴다.
...
- 빠른 고르기 규칙:
  ...
  - 두 분기가 동시에 걸리면 `코드리딩 -> 레이어 설계 -> 테스트` 순으로 하나씩만 연다.
...
- 흔한 오해:
  ...
  - "운영 용어가 보이면 바로 그 문서를 읽어야 한다"가 아니다. starter 질문이 아직 흔들리면 운영 문서보다 primer 3개를 다시 왕복하는 편이 빠르다.
...
- 기초 survey부터 읽고 싶다면:
  ...
  - [Mission Review Vocabulary Primer](./mission-review-vocabulary-primer.md) `필수`
  - [읽기 좋은 코드, 레이어 분리, 테스트 피드백 루프 입문](./readable-code-layering-test-feedback-loop-primer.md) `필수`
  - [Architecture and Layering Fundamentals](./architecture-layering-fundamentals.md) `선택` - `계층형 아키텍처 기초`와 `Service 계층 기초`를 읽은 뒤, 구조 이름 비교가 필요할 때 여는 브리지
  - [Ports and Adapters Beginner Primer](./ports-and-adapters-beginner-primer.md) `선택` - starter 3문서 이후에도 외부 연동 경계가 계속 섞일 때 여는 follow-up
...
repository 관련 질문도 한 번에 persistence 전체로 뭉뚱그리지 말고, 먼저 `repository 계약 자체가 막혔는지`와 `JPA/매핑/누수 같은 persistence 세부가 막혔는지`를 나눠 읽는 편이 검색과 다음 문서 선택이 빠르다.
`Repository Interface Contract Primer` 다음에 바로 "`그 계약을 test fake로 어떻게 재현하지?`"가 붙으면, 그 handoff는 [Repository Fake Design Guide](./repository-fake-design-guide.md)로 바로 이어진다.

  - [Repository Interface Contract Primer](./repository-interface-contract-primer.md) `선택`
  - [Repository Fake Design Guide](./repository-fake-design-guide.md) `선택` — "`repository 계약은 알겠는데 fake가 JPA clone처럼 보여요`"에 붙는 다음 칸
  ...
  - [Entity Leakage Review Checklist](./entity-leakage-review-checklist.md) `선택`
...
## 거버넌스/운영 deep dive 바로가기 (starter 이후)
...
## adjacent-category 왕복 루트

초심자 기준으로는 옆 카테고리 문서에 잠깐 갔다가, 다시 이 README의 starter 자리로 복귀하는 동선부터 고정하는 편이 안전하다.

| 지금 먼저 막힌 문장 | 잠깐 다녀올 문서 | 돌아올 자리 |
|---|---|---|
| `Service 규칙보다 List/Set/Map 선택이 먼저 막혀요`, `왜 queue가 나오지?` | [자료구조 README - 초급 10초 라우터](../data-structure/README.md#초급-10초-라우터) | [30초 starter 지도](#30초-starter-지도) |
| `가까운 칸부터`, `최소 이동 횟수`처럼 계산 질문으로 바뀌었어요 | [알고리즘 README - BFS, Queue, Map 먼저 분리하기](../algorithm/README.md#bfs-queue-map-먼저-분리하기) | [같은 주문 예시로 보는 1회 pass](#같은-주문-예시로-보는-1회-pass) |
| `queue`, `graph`, `BFS`가 한 문장에 같이 보여서 어디까지가 구조고 어디부터가 계산인지 헷갈려요` | [자료구조 README - graph, bfs, queue가 같이 보일 때](../data-structure/README.md#graph-bfs-queue가-같이-보일-때) -> [알고리즘 README - BFS, Queue, Map 먼저 분리하기](../algorithm/README.md#bfs-queue-map-먼저-분리하기) | [길 잃었을 때 복귀 경로](#길-잃었을-때-복귀-경로) |
| `request DTO`, `record`, `Map key`가 한꺼번에 헷갈려요 | [Java 타입, 클래스, 객체, OOP 입문](../language/java/java-types-class-object-oop-basics.md), [Java 컬렉션 프레임워크 입문](../language/java/java-collections-basics.md) | [길 잃었을 때 복귀 경로](#길-잃었을-때-복귀-경로) |

## 연결해서 보면 좋은 문서 (cross-category bridge)

- `queue`라는 단어가 보여도 바로 운영 문서로 가지 않는다. `consumer/worker 순서`면 [큐 기초](../data-structure/queue-basics.md) -> [ArrayDeque vs BlockingQueue 서비스 handoff 프라이머](../data-structure/arraydeque-vs-blockingqueue-service-handoff-primer.md) -> [Service 계층 기초](./service-layer-basics.md) -> [Ports and Adapters Beginner Primer](./ports-and-adapters-beginner-primer.md) -> [Message-Driven Adapter Example](./message-driven-adapter-example.md), `가까운 칸부터/최소 이동 횟수`면 [알고리즘 README - BFS, Queue, Map 먼저 분리하기](../algorithm/README.md#bfs-queue-map-먼저-분리하기) -> [DFS와 BFS 입문](../algorithm/dfs-bfs-intro.md) -> [BFS vs Dijkstra shortest path mini card](../algorithm/bfs-vs-dijkstra-shortest-path-mini-card.md) 순서로만 내려간다.
- `Service 규칙보다 List/Set/Map 선택이 먼저 막히면` [Java 컬렉션 프레임워크 입문](../language/java/java-collections-basics.md) -> [Backend Data-Structure Starter Pack](../data-structure/backend-data-structure-starter-pack.md) -> [HashMap, TreeMap, LinkedHashMap Beginner Selection Primer](../data-structure/hashmap-treemap-linkedhashmap-beginner-selection-primer.md) 순서로 붙인다.
- `List/Set/Map` 이유가 풀리면 deep dive 구현체보다 [Service 계층 기초](./service-layer-basics.md)로 복귀해 `이 구조를 어디서 조립하나`를 먼저 본다.
- `graph`, `queue`, `BFS`가 같이 보여도 같은 층위가 아니다. 구조는 [자료구조 README - graph, bfs, queue가 같이 보일 때](../data-structure/README.md#graph-bfs-queue가-같이-보일-때), 계산은 [알고리즘 README - BFS, Queue, Map 먼저 분리하기](../algorithm/README.md#bfs-queue-map-먼저-분리하기)로 나누고, 복귀는 [같은 주문 예시로 보는 1회 pass](#같은-주문-예시로-보는-1회-pass)나 [30초 starter 지도](#30초-starter-지도)로 잡는다.
- 과부하가 오면 옆 카테고리에서 더 내려가지 말고 [길 잃었을 때 복귀 경로](#길-잃었을-때-복귀-경로)로 돌아온다.

## queue 이름이 섞일 때

- `queue`가 `undo/redo`, `작업 이력`, `왜 어떤 요청은 API로 끝내고 어떤 요청은 queue로 보내요?`, `producer/consumer`처럼 보이면 beginner-safe ladder를 먼저 고정한다. [커맨드 패턴 기초](../design-pattern/command-pattern-basics.md) -> [Per-Key Queue vs Direct API Primer](../system-design/per-key-queue-vs-direct-api-primer.md) -> [메시지 큐 기초](../system-design/message-queue-basics.md)까지만 읽고, `retry`, `DLQ`, `replay`, `backlog tuning`은 다음 질문이 생길 때만 연다.

## cross-category 짧은 사다리

| 지금 증상 | primer | follow-up | deep dive 대신 안전한 복귀 |
|---|---|---|---|
| `Service 규칙보다 List/Set/Map이 먼저 보여요` | [Java 컬렉션 프레임워크 입문](../language/java/java-collections-basics.md) | [Backend Data-Structure Starter Pack](../data-structure/backend-data-structure-starter-pack.md) -> [HashMap, TreeMap, LinkedHashMap Beginner Selection Primer](../data-structure/hashmap-treemap-linkedhashmap-beginner-selection-primer.md) | [30초 starter 지도](#30초-starter-지도) |
| `queue`가 worker handoff인지 BFS 도구인지 모르겠어요 | [Backend Data-Structure Starter Pack](../data-structure/backend-data-structure-starter-pack.md) | `worker 순서`면 [Service 계층 기초](./service-layer-basics.md) -> [Ports and Adapters Beginner Primer](./ports-and-adapters-beginner-primer.md), `최소 이동 횟수`면 [알고리즘 README - BFS, Queue, Map 먼저 분리하기](../algorithm/README.md#bfs-queue-map-먼저-분리하기) -> [DFS와 BFS 입문](../algorithm/dfs-bfs-intro.md) | [같은 주문 예시로 보는 1회 pass](#같은-주문-예시로-보는-1회-pass) |
| `queue`가 command queue인지 message queue인지 모르겠어요 | [커맨드 패턴 기초](../design-pattern/command-pattern-basics.md) | [Per-Key Queue vs Direct API Primer](../system-design/per-key-queue-vs-direct-api-primer.md) -> [메시지 큐 기초](../system-design/message-queue-basics.md) | [30초 starter 지도](#30초-starter-지도) |
| `가까운 칸부터`와 `consumer 순서`가 같은 말처럼 보여요 | [DFS와 BFS 입문](../algorithm/dfs-bfs-intro.md) | [큐 기초](../data-structure/queue-basics.md) -> 필요하면 [BFS vs Dijkstra shortest path mini card](../algorithm/bfs-vs-dijkstra-shortest-path-mini-card.md) | [길 잃었을 때 복귀 경로](#길-잃었을-때-복귀-경로) |

## cross-category 사다리 사용 규칙

- 이 표는 `primer -> follow-up -> 복귀` 3칸만 유지한다.
- beginner 단계에서는 `retry`, `scheduler`, `distributed queue`, `운영 incident`보다 이 표를 먼저 본다.

## DTO/VO/Entity bridge

- `DTO`, `VO`, `Entity` 이름은 알겠는데 "`record`가 VO에 왜 잘 맞지?", "`Map` key로는 ID를 왜 따로 두지?"가 바로 이어서 막히면 [DTO, VO, Entity 기초](./dto-vo-entity-basics.md)에서 `전달 계약 / 값 의미 / 식별자 추적` 3칸을 먼저 고정한 뒤, [Record and Value Object Equality](../language/java/record-value-object-equality-basics.md) -> [Stable ID as Map Key Primer](../language/java/stable-id-map-key-primer.md) -> [Backend Data-Structure Starter Pack](../data-structure/backend-data-structure-starter-pack.md) 순서로 한 칸씩만 넘긴다. 이렇게 읽으면 software-engineering primer에서 곧바로 mutable key나 구현체 암기로 점프하지 않고, `VO equality -> ID key -> map/set 선택` 순서로 안전하게 내려갈 수 있다.
- `Service` 책임을 읽다가 `List`/`Set`/`Map`이나 `Optional` 때문에 규칙 설명이 흐려지면, 바로 `TreeMap`, mutable key, sorted deep dive로 내려가지 말고 `software-engineering primer -> language primer -> data-structure primer -> service primer` 사다리를 먼저 탄다. 안전한 순서는 [Java 컬렉션 프레임워크 입문](../language/java/java-collections-basics.md) -> [Backend Data-Structure Starter Pack](../data-structure/backend-data-structure-starter-pack.md) -> [Service 계층 기초](./service-layer-basics.md)다. 정렬/범위 조회가 실제로 필요할 때만 그다음 [HashMap, TreeMap, LinkedHashMap Beginner Selection Primer](../data-structure/hashmap-treemap-linkedhashmap-beginner-selection-primer.md) 또는 [큐 기초](../data-structure/queue-basics.md)로 한 칸 더 간다. `처음`, `왜 queue가 나오지`, `헷갈려요`, `서비스 규칙보다 컬렉션 선택이 먼저 막혀요` 같은 beginner query는 이 ladder를 먼저 타고, `가까운 칸부터`, `최소 이동 횟수`처럼 계산 질문으로 바뀔 때만 [Backend Algorithm Starter Pack](../algorithm/backend-algorithm-starter-pack.md)으로 넘긴다.

## inbound/runtime bridge

- 채널별 inbound adapter를 command lifecycle과 테스트 경계까지 함께 잡으려면 [Message-Driven Adapter Example](./message-driven-adapter-example.md)로 먼저 "같은 일을 여는 입구인가?"를 판단한 뒤, [Batch Job Scope In Hexagonal Architecture](./batch-job-scope-hexagonal-architecture.md), [Batch Partial Failure Policies Primer](./batch-partial-failure-policies-primer.md), [Batch Run Result Modeling Examples](./batch-run-result-modeling-examples.md), [Batch Idempotency Key Boundaries](./batch-idempotency-key-boundaries.md), [Webhook and Broker Boundary Primer](./webhook-and-broker-boundary-primer.md), [Inbound Adapter Test Slices Primer](./inbound-adapter-test-slices-primer.md), [Inbound Adapter Testing Matrix](./inbound-adapter-testing-matrix.md), [Outbox and Message Adapter Test Matrix](./outbox-message-adapter-test-matrix.md) 다음에 [Design Pattern: Command Handler Pattern](../design-pattern/command-handler-pattern.md), [Design Pattern: Process Manager Deadlines and Timeouts](../design-pattern/process-manager-deadlines-timeouts.md), [System Design: Job Queue 설계](../system-design/job-queue-design.md), [System Design: 분산 스케줄러 설계](../system-design/distributed-scheduler-design.md), [System Design: Workflow Orchestration + Saga 설계](../system-design/workflow-orchestration-saga-design.md)를 이어서 본다.
...
입문 구간을 읽은 다음에는, **"용어를 익히는 단계"에서 "설계 선택지를 비교하는 단계"로 넘어간다**고 보면 된다. 이 지점부터의 catalog는 `정답 암기`보다 `왜 이 경계를 택했는지 설명하는 문서`에 가깝다.
...
- catalog 첫 진입 추천:
  ...
  - 운영/거버넌스 문서는 `입문 이후`에도 바로 첫 선택이 아니다. 코드 경계가 잡힌 뒤 `rollout`, `governance`, `ownership` 질문이 생길 때만 연다.
...
아래 구간은 소프트웨어 공학 입문과 패러다임 복습용 `legacy primer shelf`다. 현재 PR이나 미션 피드백을 바로 풀기 위한 첫 클릭보다, 기초 용어를 다시 맞출 때 쓰는 복귀용 선반으로 읽으면 된다.
...
아래 구간은 최신 설계 선택지, 경계 설계, rollout, governance 문서를 모아 둔 `deep dive catalog`다.
...
> `추상화가 어색해요`, `책임이 섞였어요`, `테스트가 너무 무거워요` 같은 리뷰 문장을 beginner primer와 가장 안전한 첫 행동으로 연결해 주는 짧은 bridge router
...
> 이번 QA 보강으로 `메서드가 너무 길어요 / Controller가 너무 많은 걸 알아요 / 왜 서버 전체 테스트부터 돌리죠?`를 30초 비교표 한 장으로 바로 가르게 했고, `saveAll` 오해는 [JPA Batch Config Pitfalls](./jpa-batch-config-pitfalls.md), 얇은 Controller 뒤의 비대한 Service 오해는 [계층형 아키텍처 기초](./layered-architecture-basics.md)와 [Service 계층 기초](./service-layer-basics.md)로 바로 이어지게 다듬었다.
...
> unit test, `@WebMvcTest`, `@DataJpaTest`, `@SpringBootTest` 중 이번 변경의 첫 테스트 1개를 어떻게 고를지 4문항 체크리스트와 반례로 안내하고, `@WebMvcTest`에서 입력 검증 변경인데 `400/401/403`이 섞일 때 보는 5줄 체크리스트까지 붙인 beginner primer

> 처음 읽는다면 `첫 테스트 선택표 -> 실패 먼저 재현 미니 시나리오 -> A/B/C 시나리오`까지만 starter로 잡고, retry/security 세부는 막혔을 때 follow-up으로 내려가면 된다.

## Browser E2E cost-vs-signal checklist

- [Browser E2E cost-vs-signal checklist](./browser-e2e-cost-vs-signal-checklist.md)

> "브라우저 E2E를 몇 개만 남겨야 하죠?"라는 beginner 질문에 맞춰, flaky cost를 감수할 만큼 신호가 큰 흐름만 남기는 5문항 체크리스트와 `로그인/결제/외부 인증` 중심 선별 기준을 따로 분리한 focused primer

## 컴포넌트 테스트 vs 브라우저 E2E 경계 카드

- [컴포넌트 테스트 vs 브라우저 E2E 경계 카드](./component-test-vs-browser-e2e-boundary-card.md)

> "`버튼 비활성화`, `에러 배너`, `loading spinner` 같은 UI 동작을 언제 컴포넌트 테스트에 남기고 언제 진짜 브라우저 E2E로 올릴지"를 한 화면 규칙 vs 브라우저 경계 약속으로 짧게 가르는 beginner bridge card

## MSW stub vs 실제 백엔드 컴포넌트 테스트 카드

- [MSW stub vs 실제 백엔드 컴포넌트 테스트 카드](./msw-stub-vs-real-backend-component-test-card.md)

> "`컴포넌트 테스트인데 API를 진짜로 붙여야 하나요?`라는 초심자 질문에 맞춰, `화면 상태 검증은 MSW stub`, `응답 shape/인증 wiring 검증은 실제 백엔드 또는 테스트 서버 후보`로 나누는 focused beginner bridge card
...
> 같은 주문 생성 예시로 `quantity` 누락 `400` 같은 HTTP 계약 변경과 `재고 부족이면 실패` 같은 비즈니스 규칙 변경을 갈라서, 첫 failing test를 `@WebMvcTest`와 단위 테스트 중 어디에 둘지 빠르게 고르게 돕는 focused beginner card
...
> "`@Valid`가 재고 부족까지 막아 주나요?`, `binding error랑 도메인 규칙 위반은 첫 테스트를 어디서 나눠요?` 같은 follow-up query에 맞춰, 주문 예시를 `요청 읽기 실패`와 `의미 규칙 실패`로 자르고 `@WebMvcTest` vs 단위 테스트 starter를 바로 연결하는 focused beginner bridge card
...
> "`flush()`까지 했는데 왜 `AFTER_COMMIT` listener나 outbox 검증은 안 맞죠?"라는 beginner query에 맞춰, `flush = SQL 동기화`, `commit = 최종 확정`을 분리하고 commit-visible 테스트를 언제 얇게 추가할지 짧게 이어 주는 bridge card
...
> "`commit-visible 테스트를 추가하기로 했는데 둘 중 뭘 먼저 고르죠?`", "`한 테스트 안에서 commit 전후를 같이 봐야 하나요?`" 같은 beginner query에 맞춰, `중간 commit이 필요하면 TestTransaction`, `메서드 끝 commit이면 @Commit` 판단표를 따로 분리한 focused bridge card
...
> "`rollback 테스트에서는 왜 listener가 안 보여요?`"라는 symptom query에 맞춰, `rollback test = 트랜잭션 안`, `AFTER_COMMIT = commit 뒤`를 한 장 카드로 다시 붙이고 현재 bridge card와 Spring mini card 사이 이동 비용을 줄이는 focused beginner bridge

## 리팩토링과 첫 failing test 연결 브리지

- [리팩토링과 첫 failing test 연결 브리지](./refactoring-first-failing-test-bridge.md)

> `메서드가 너무 길어요`, `이름이 안 읽혀요`, `구조를 안전하게 정리하고 싶어요` 같은 리뷰 문장을 `무엇을 먼저 깨뜨려야 하는가` 질문으로 바꾸고, 첫 failing test 1개와 작은 리팩토링 1단계를 연결해 주는 beginner bridge primer
...
> `service 리팩터링인데 어떤 테스트와 어떤 double을 먼저 고르죠?`, `왜 springboottest부터 말고 fake/mock/spy부터 나누죠?` 같은 follow-up query에 맞춰, 3개 concrete scenario를 `변경 장면 -> 가장 싼 first failing test -> double choice`로 바로 연결해 주는 intermediate bridge pack

## 리팩토링 커밋 분리 프라이머

- [리팩토링 커밋 분리 프라이머](./refactor-commit-split-primer.md)

> `test -> refactor -> feat/fix` 순서로 커밋을 왜 나누는지, 초심자가 리뷰 질문과 rollback 단위를 섞지 않도록 짧은 예시와 선택표로 풀어 주는 beginner follow-up primer

## Dummy vs Stub 초심자 미니 카드

- [Dummy vs Stub 초심자 미니 카드](./dummy-vs-stub-beginner-mini-card.md)

> `고정된 입력을 넣은 테스트 더블`을 전부 `mock`이라고 부르기 쉬운 초심자를 위해, `자리만 채우는 dummy`와 `준비한 답을 돌려주는 stub`을 가장 빠른 질문 2개로 나눠 주는 beginner mini card

## Fake vs Mock 첫 테스트 프라이머

- [Fake vs Mock 첫 테스트 프라이머](./fake-vs-mock-first-test-primer.md)

> 작은 service 리팩토링에서 `mock`과 `fake` 중 무엇으로 첫 failing test를 시작할지, `결과를 읽는가`와 `호출을 읽는가` 기준으로 빠르게 나눠 주는 focused beginner primer

## Stub vs Spy 첫 테스트 프라이머

- [Stub vs Spy 첫 테스트 프라이머](./stub-vs-spy-first-test-primer.md)

> `고정된 반환값`만 필요한 stub과 `호출 기록`이 필요한 spy를 첫 테스트에서 어떻게 가를지, `값이 필요하냐`와 `상호작용이 답이냐` 기준으로 풀어 주는 focused beginner primer

## Outbound Notifier Mock Boundary Primer

- [Outbound Notifier Mock Boundary Primer](./outbound-notifier-mock-boundary-primer.md)

> `알림을 보냈나`, `이벤트를 발행했나`처럼 호출 자체가 결과인 outbound 경계에서 왜 repository fake 대신 mock/spy가 starter로 더 잘 맞는지 좁게 연결해 주는 beginner bridge
...
> `구조는 맞는데 계약이 샌다`, `service 시그니처에 entity가 보여요`, `request dto를 repository까지 넘겨요` 같은 symptom query를 바로 받도록 decision row도 함께 보강했다.
...
> `계층형 아키텍처 기초`와 `Service 계층 기초` 다음에 읽는 브리지 문서. layered / clean / modular monolith / DDD boundary / repository-entity separation을 "지금 어떤 설계 질문이 막혔는가" 기준으로 다시 나눠 준다.
...
> "controller-service-repository로 충분한 때"와 "inbound/outbound port가 실제로 문제를 줄이는 때"를 짧은 비교표로 바로 이어 준다.
...
> 초심자가 `controller용 service`, `consumer용 service`, `batch용 service`를 따로 만들어야 하는지 막힐 때, 먼저 같은 유스케이스 경계를 공유하는지부터 판단하게 돕는 entrypoint이기도 하다.
...
> `배치 테스트 fixture 뭐부터 만들지`, `RunSummary/ChunkResult fixture를 왜 따로 두지`, `fixture 이름이 왜 길어도 되지` 같은 첫 질문에서 giant helper 대신 작은 장면 fixture로 시작하게 잡아 주는 beginner primer

## Primer On Retry Queue Assertions

- [Primer On Retry Queue Assertions](./retry-queue-assertions-primer.md)

> `retryable`, `manual review`, `terminal failure`가 실제 테스트 assertion에서 어떻게 갈리는지, `실패를 다음 경로로 제대로 번역했는가`를 초심자 체크리스트로 바로 확인하게 돕는 primer

## Batch Result Testing Checklist

- [Batch Result Testing Checklist](./batch-result-testing-checklist.md)

> batch 결과 객체 테스트에서 `count만 맞추는지`, `실패 bucket과 resume 경계를 같이 잠그는지`, `어떤 fixture가 가장 싼 재현인지`를 빠르게 고르는 초심자용 follow-up checklist
...
- [Manual Review Backlog Primer](./manual-review-backlog-primer.md)

> 분류가 끝난 뒤 `manual-review` bucket을 단순 실패 보관함이 아니라, beginner-safe 필드와 ownership rule로 사람이 다음 판단을 이어 가는 backlog로 설명한 focused primer
...
- [Outbox vs Inbox 주문 예시 입문 카드](./outbox-vs-inbox-order-example-primer.md)

> advanced matrix로 바로 뛰기 전에, `outbox = 보내는 쪽에서 보낼 사실을 남김`, `inbox = 받는 쪽에서 같은 메시지 효과를 한 번만 반영`이라는 구분을 주문 하나 예시로 먼저 잡게 돕는 beginner entrypoint primer
> 특히 "`왜 outbox가 필요해요?`", "`같은 메시지가 또 오면요?`", "`producer랑 consumer 질문이 자꾸 섞여요`" 같은 retrieval query를 이 문서 하나에서 먼저 받도록 entrypoint를 분리했다.
...
> `flush()`와 commit 차이 때문에 outbox 검증 시작점이 헷갈리는 beginner query도 바로 받을 수 있게, `rollback/commit 혼동 -> outbox row -> message side effect` 이동 경로를 함께 보강했다.

## Repository Interface Contract Primer

- [Repository Interface Contract Primer](./repository-interface-contract-primer.md)

> 처음 배우는데 `Repository`를 왜 인터페이스로 두는지, `저장 계약` 질문과 `JPA/매핑 같은 persistence 세부` 질문을 어디서 갈라 읽어야 하는지 큰 그림부터 잡는 beginner entrypoint primer
> 이 README에서는 특히 "`계약은 알겠는데 test fake는 뭘 재현하죠?`"라는 바로 다음 질문을 [Repository Fake Design Guide](./repository-fake-design-guide.md)로 연결하는 persistence handoff entry로 쓴다.

## Persistence Follow-up Question Guide

- [Persistence Follow-up Question Guide](./persistence-follow-up-question-guide.md)

> `repository 다음 뭐 읽지` 질문에 먼저 답하는 beginner bridge primer. `fake / mapping / leakage` 3갈래 mini card로 30초 분기를 주고, 그다음 질문형 결정표로 계약/이름/조회 분리/매핑/누수 중 어디인지 더 좁히게 돕는다.
...
> repository fake를 in-memory JPA clone이 아니라 `repository 계약`을 메모리에서 재현하는 test adapter로 설명하고, `save()`/중복/조회 약속만 남겨 use case test와 persistence 구현 세부를 분리하게 돕는 intermediate guide
> `Repository Interface Contract Primer` 다음에 "`그 계약을 fake로 어떻게 옮기지?`"가 붙을 때 바로 여는 후속 guide다.
...
> "`서비스나 API에 `Entity`가 이미 보이는데 왜 문제지?`, `엔티티를 JSON으로 바로 내보내도 되나요?` 같은 symptom query를 받아, JPA `@Entity`가 API 응답·애플리케이션 흐름·도메인 규칙까지 침범할 때의 leakage anti-pattern을 beginner 관점으로 끊어 설명한 문서"

## Entity Leakage Review Checklist

- [Entity Leakage Review Checklist](./entity-leakage-review-checklist.md)

> beginner 코드 리뷰에서 JPA `@Entity`가 API DTO, service 계약, cross-module 호출로 새는지 "반환 타입 -> 파라미터 타입 -> 공개 API" 순서로 빠르게 찾게 돕는 짧은 체크리스트
> 특히 "`공개 API에서 entity 누수`"를 찾았다면, 바로 다음 문서는 [Module API DTO Patterns](./module-api-dto-patterns.md)다.

## Service Contract Smell Cards

- [Service Contract Smell Cards](./service-contract-smell-cards.md)

> "`구조는 맞는데 계약이 샌다`, `service 시그니처에 `ResponseEntity`, `Pageable`, `MultipartFile`, `OrderEntity`가 보여요` 같은 symptom query를 받아, 웹/프레임워크/영속 세부가 유스케이스 계약을 덮는지 바로 점검하게 돕는 compact beginner primer"
...
> "`service가 Pageable을 받아요`를 봤는데, 그럼 pagination 자체가 냄새인가요?"라는 다음 질문을 받아, service 계약 leak과 legitimate query repository/adapter pagination을 분리해 주는 intermediate bridge
...
> 특히 "`query model`이 뭐예요?", "목록 조회용 repository를 따로 둬야 해요?", "`DAO`랑 query repository는 뭐가 달라요?" 같은 첫 질문이 deep dive보다 이 primer로 먼저 떨어지도록 anchor를 보강했다.
...
> `Command`, `SupportSnapshot`, `QueryView`, `Response`가 모두 DTO처럼 보여서 이름이 섞일 때, 초심자가 suffix와 패키지 위치를 빠르게 고를 수 있게 묶은 compact checklist
...
> `@Valid` 같은 입력 형식 검증과 서비스/도메인 업무 규칙 검증을 왜 분리하는지, `400`과 `409`가 어떻게 갈리는지, `MethodArgumentNotValidException`의 필드별 `errors[]` 응답과 프로젝트마다 달라질 수 있는 `$.errors[0]` vs `$.fieldErrors[0]` JSONPath 차이를 회원 가입·주문·예약 예시로 설명한 primer
...
> `테스트 전략 기초`에서 `규칙 / 웹 계약 / JPA / 협력 흐름` 중 첫 테스트 1개를 고른 뒤, 왜 그 선택이 맞는지와 `mock/stub/fake` 차이를 한 단계 더 보고 싶을 때 여는 follow-up 문서
...
> `왜 outbox가 필요한가`, `같은 메시지가 또 오면 어쩌죠`, `rollback/commit이 헷갈려서 메시지 부작용을 못 읽겠어요` 같은 초심자 질문을 outbox/inbox 큰 그림으로 되받아 주는 follow-up bridge
...
> 초심자가 `모듈 API 계약`, `cross-module dto`, `다른 모듈에 entity를 넘겨도 되나요?`, `공개 API에서 entity 누수`처럼 검색해도 바로 붙도록 만든 beginner primer entrypoint
> `Entity Leakage Review Checklist`에서 "`공개 API에서 entity 누수`"나 "모듈 API로 entity가 샌다"까지 확인한 뒤 바로 이어 읽는 다음 문서다.
...
예시 답변으로는 `테스트 단계`를 들 수 있습니다.

이유는 구현을 마친 뒤에야 드러나는 예외 상황, 환경 차이, 통합 오류를 실제로 확인해야 하기 때문입니다.
예를 들어 결제, 자동차, 게임처럼 실패 비용이 큰 시스템은 정상 경로보다 예외 경로를 더 많이 확인해야 해서 시간과 비용이 빠르게 커집니다.
또 이 단계에서 문제가 발견되면 구현이나 설계까지 다시 돌아가야 해서 수정 비용도 함께 커집니다.
