...
|---|---|---|
| 재고 부족이면 예외가 나는지만 보고 싶다 | fake/mock repository, 순수 규칙만 확인 | 단위 테스트 |
| 주문 저장과 재고 차감이 함께 롤백되는지 보고 싶다 | 스프링 빈, 트랜잭션, 실제 저장소 협력 | `@SpringBootTest` |
| `POST /orders`가 `400`을 내는지 보고 싶다 | MVC 바인딩, 검증, 응답 포맷 | `@WebMvcTest` |
...
|---|---|---|---|
| "왜 서버 전체 테스트부터 돌리죠?" | 지금 확인할 것이 규칙인가, 협력인가 | 단위 테스트 또는 `@SpringBootTest` 중 더 싼 쪽부터 | [읽기 좋은 코드, 레이어 분리, 테스트 피드백 루프 입문](./readable-code-layering-test-feedback-loop-primer.md) |
| "Controller가 너무 많은 걸 알아요" | 입력 검증을 보는가, 업무 규칙을 보는가 | `@WebMvcTest` 또는 단위 테스트 | [계층형 아키텍처 기초](./layered-architecture-basics.md) |
| "쿼리를 바꿨는데 이 테스트가 왜 계속 초록이죠?" | 실제 JPA/SQL이 붙었는가 | `@DataJpaTest` | [Repository, DAO, Entity](./repository-dao-entity.md) |
...
|---|---|---|---|
| Controller에서 `quantity <= 0` 검사를 `@Valid`로 옮겼다 | HTTP 요청이 잘못되면 `400`이 나는가 | `@WebMvcTest` | 입력 형식과 에러 응답 계약을 가장 싸게 본다 |
| Service에서 재고 부족 규칙을 추가했다 | 주문 규칙이 맞으면 예외가 나는가 | 단위 테스트 | 비즈니스 규칙만 빠르게 잠근다 |
| 주문 저장과 재고 차감을 한 트랜잭션으로 묶었다 | 둘 중 하나가 실패하면 함께 롤백되는가 | `@SpringBootTest` | 여러 빈과 트랜잭션 경계를 실제로 붙여 봐야 한다 |
...
| JPA 쿼리 수정 (`JPA 쿼리/매핑/인덱스 관련 변경`) | 서비스 테스트에서 Mock Repository 반환값만 바꿔 쿼리 수정을 검증했다고 판단 | `@DataJpaTest`에서 테스트 데이터 저장 후 새 쿼리(`findRecentPaidOrders`) 결과 건수 + 순서를 함께 확인 |
| 주문 저장 + 재고 차감 흐름 변경 (`트랜잭션 경계/여러 컴포넌트 협력 흐름 변경`) | Repository 테스트만 1개 두고 "둘이 함께 실패하는가"를 눈으로만 확인 | `@SpringBootTest`에서 재고 차감 실패 시 주문 저장도 함께 롤백되는지 1개로 고정 |
...
|---|---|---|
| 계산식/검증 로직 변경 | 할인 10% 정책을 15%로 바꿨는데 `10,000원 -> 8,500원`이어야 할 주문이 여전히 `9,000원`으로 계산된다. | 단위 테스트 1개로 `할인율 적용 결과`만 고정한다. |
| Controller 입력 검증 변경 | `POST /orders`에 `quantity=0`을 보내 봤더니 `201`이 나와서 잘못된 요청이 통과한다. | `@WebMvcTest` 1개로 `400`과 `quantity` 에러 필드만 고정한다. |
| JPA 쿼리 정렬 변경 | 최근 결제 주문이 먼저 보여야 하는데 `findRecentPaidOrders()`가 오래된 주문부터 반환한다. | `@DataJpaTest` 1개로 테스트 데이터를 2건 저장하고 결과 건수와 순서를 함께 검증한다. |
| 주문 저장 + 재고 차감 협력 흐름 변경 | 재고 차감이 실패했는데 주문 레코드는 남아서 반쪽 성공이 생긴다. | `@SpringBootTest` 1개로 예외 발생 시 주문 저장과 재고 차감이 함께 롤백되는지 고정한다. |
