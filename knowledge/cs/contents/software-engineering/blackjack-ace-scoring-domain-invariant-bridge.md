...
|---|---|---|
| "Ace를 1로 볼지 11로 볼지 if문이 여러 곳에 있어요" | controller, service, view가 카드 합계를 각각 계산하는 코드 | 손패 점수 규칙은 `Hand`/`Score` 같은 도메인 값 경계로 모은다 |
| "bust 여부를 boolean으로 저장하고 계속 맞춰야 해요" | 카드 목록과 bust flag가 서로 다른 진실 원천이 되는 구조 | bust는 손패 합계에서 항상 파생되게 하고 별도 상태 동기화를 피한다 |
| "`hit/stand` 상태 패턴이랑 Ace 점수 규칙이 같은 문제인가요?" | 흐름 전이와 값 계산 규칙을 같은 리팩터링으로 처리하려는 상황 | 가능한 행동 전이는 상태 모델, 점수 계산은 도메인 불변식으로 분리한다 |
