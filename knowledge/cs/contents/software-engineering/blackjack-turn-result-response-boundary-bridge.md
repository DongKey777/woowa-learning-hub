...
|---|---|---|
| "`Game.hit()`가 바로 카드 문자열이나 승패 문구를 반환해요" | 도메인 메서드가 화면 문구/JSON 응답 형태를 직접 만드는 코드 | 한 턴 결과 의미와 표현 포맷을 분리한다 |
| "controller가 점수, bust, 다음 행동 가능 여부를 다시 조립해요" | 도메인 상태 조각을 여러 getter로 꺼내 응답 DTO를 만드는 구조 | `TurnResult` 같은 결과 객체로 규칙이 끝난 상태를 먼저 닫는다 |
| "stand 뒤 dealer turn 결과 문구가 여러 곳에 있어요" | service, controller, view가 승패/딜러 공개 규칙을 반복 해석하는 구현 | 결과 의미는 도메인/서비스 결과로, 문구와 JSON 필드는 바깥 매핑으로 둔다 |
