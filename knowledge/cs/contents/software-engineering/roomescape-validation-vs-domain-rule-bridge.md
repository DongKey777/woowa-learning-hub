...
|---|---|---|
| "예약 중복인데 왜 `@Valid`로 막지 말래요?" | roomescape 예약 생성 DTO 검증에 DB 조회 규칙까지 넣은 코드 | 요청 한 장으로 알 수 있는 입력 검증과 저장 상태 기반 도메인 규칙을 나눈다 |
| "roomescape에서 400이랑 409를 어디서 나눠요?" | 빈 이름, 날짜 포맷, 이미 찬 슬롯이 같은 실패 응답으로 내려가는 API | request shape 오류와 current state conflict를 다른 응답 의미로 본다 |
| "중복 예약 에러를 controller에서 처리해야 하나요?" | controller가 repository를 직접 조회해 conflict를 정하는 장면 | 충돌 판정은 service/domain rule로 올리고 controller는 HTTP 입구로 남긴다 |
