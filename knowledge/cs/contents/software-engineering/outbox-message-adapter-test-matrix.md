...
> - [Spring Mini Card: rollback-only 실패 vs checked exception인데 commit된 것처럼 보이는 놀람](../spring/spring-rollbackonly-vs-checked-exception-commit-surprise-card.md)
...
> - outbox 왜 commit 뒤에 테스트하나요
> - rollback commit 헷갈려서 outbox 테스트 못 하겠어요
> - transaction boundary confusion outbox beginner
> - message side effect test beginner
> - after commit outbox row visibility
...
## 먼저 초심자 질문을 한 줄로 번역하기

이 문서는 advanced matrix지만, 초심자가 자주 들고 오는 첫 문장은 의외로 단순하다.

- "`flush()`는 했는데 왜 outbox row가 진짜 남았는지 모르겠어요"
- "`rollback`이랑 `commit`이 헷갈려서 메시지 테스트를 어디에 둬야 할지 모르겠어요"
- "메시지가 두 번 오면 side effect가 두 번 생기나요"

이 세 문장은 각각 다른 질문이다.

| learner symptom | 먼저 붙일 mental model | 바로 이어 읽을 곳 |
|---|---|---|
| "`flush()`는 했는데 왜 outbox가 안 보여요?" | `flush`는 중간 동기화이고 `commit`은 최종 확정이다 | [`flush()` vs commit: `AFTER_COMMIT` 초심자 브리지 카드](./transactional-test-rollback-vs-commit-boundary-card.md) |
| "`UnexpectedRollbackException`이랑 outbox 누락이 같은 문제예요?" | rollback-only 놀람과 commit 뒤 side effect 질문을 섞지 않는다 | [Spring Mini Card: rollback-only 실패 vs checked exception인데 commit된 것처럼 보이는 놀람](../spring/spring-rollbackonly-vs-checked-exception-commit-surprise-card.md) |
| "왜 같은 메시지가 두 번 와도 한 번만 처리해야 하죠?" | outbox는 producer 원자성, inbox는 consumer 중복 방지 질문이다 | [Domain Events, Outbox, Inbox](./outbox-inbox-domain-events.md) |

이제 위 세 문장을 분리한 뒤에야 아래 matrix가 제대로 읽힌다.
...
- `flush()`로 row를 봤으니 outbox integration test도 끝났다고 본다. row 존재 확인은 출발점일 뿐이고, commit 뒤에도 남는지와 publish 전후 상태 전이는 별도 질문이다. 이 지점이 헷갈리면 [`flush()` vs commit: `AFTER_COMMIT` 초심자 브리지 카드](./transactional-test-rollback-vs-commit-boundary-card.md)로 한 번 돌아가면 된다.
