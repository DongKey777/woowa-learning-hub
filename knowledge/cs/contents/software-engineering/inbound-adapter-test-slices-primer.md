...
> 한 줄 요약: Hexagonal Architecture에서 controller와 message handler test는 port 경계에서 멈춰 adapter 번역 책임만 볼 때는 slice test로, 실제 트랜잭션/브로커/보안 wiring까지 실패 모드가 이어질 때는 full integration test로 나눈다.

**난이도: 🟡 Intermediate**

입력 채널 자체를 먼저 정리하고 싶다면 [Message-Driven Adapter Example](./message-driven-adapter-example.md), 전체 seam 전략을 먼저 보고 싶다면 [Hexagonal Testing Seams Primer](./hexagonal-testing-seams-primer.md)부터 읽고 이 문서는 그다음에 "inbound adapter는 어디까지 slice로 자르고, 언제 full integration으로 올릴까"만 좁혀 보면 된다.
...
- [full integration test가 필요한 순간](#full-integration-test가-필요한-순간)
...
- **full integration test**: 실제 adapter entrypoint부터 **real use case와 주요 runtime wiring**까지 이어지는 실패 모드를 검증한다

여기서 핵심은 경계를 어디에 두느냐다.
이 문맥의 full integration test는 브라우저부터 외부 시스템까지 전부 붙이는 E2E와는 다르고, 보통 **애플리케이션 entrypoint에서 real use case와 핵심 인프라 연결까지 확인하는 app integration test**에 가깝다.

| 테스트 종류 | 어디서 멈추나 | 주로 보는 실패 |
|---|---|---|
| slice test | use case port 앞에서 멈춘다 | 바인딩, 검증, 직렬화, 예외 번역, header 해석 |
| full integration test | real use case와 핵심 outbound adapter까지 통과한다 | 트랜잭션, 보안 체인, listener container, broker/DB wiring, commit-after-ack 같은 실제 runtime 결합 |

즉 slice test는 "adapter가 port를 올바르게 호출하는가"를 보고, full integration test는 "이 entrypoint가 실제 앱 안에서 끝까지 안전하게 동작하는가"를 본다.
...
## full integration test가 필요한 순간

inbound adapter라도 아래 위험이 핵심이면 full integration test로 올리는 편이 맞다.
...
기술적으로는 slice로도 충분해 보이더라도, 팀이 반복해서 아래를 놓친다면 full integration test 몇 개가 가치가 있다.
...
| 질문 | slice test가 맞는가 | full integration test가 맞는가 |
|---|---|---|
...
- **runtime 결합 책임**이면 full integration test
...
- 실제 `POST /orders`와 `payment-authorized` 소비 경로는 각각 1~2개의 full integration test로 security/transaction/ack 결합만 확인

핵심은 **많은 slice + 적은 full integration**이지, 둘 중 하나만 선택하는 것이 아니다.
...
- full integration test를 하나도 두지 않는다. 그러면 ack/transaction/security/serializer wiring 같은 실제 런타임 실패를 늦게 발견한다.

## 한 줄 정리

Hexagonal inbound adapter 테스트의 기준은 단순하다. controller/message handler가 **entry signal을 use case port 호출로 번역하는 책임**만 보면 slice test를 쓰고, 그 entrypoint의 의미가 실제 transaction, broker, security, serializer wiring까지 걸려 있으면 full integration test로 올린다.
