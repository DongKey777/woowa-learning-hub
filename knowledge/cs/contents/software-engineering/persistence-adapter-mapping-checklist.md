...
특히 `Repository Interface Contract Primer` 다음에 "`domain`과 `Entity`를 어디서 끊어야 하지?`"가 막혔다면 이 문서가 후속 선택지다.
반대로 첫 질문이 "`fake`가 이 계약을 test에서 어떻게 재현하지?`"면 [Repository Fake Design Guide](./repository-fake-design-guide.md)로, "`서비스/API에 `Entity`가 이미 보이네`"라면 [Persistence Model Leakage Anti-Patterns](./persistence-model-leakage-anti-patterns.md)로 가는 편이 맞다.
