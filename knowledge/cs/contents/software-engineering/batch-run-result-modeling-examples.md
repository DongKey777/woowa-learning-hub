...
retrieval-anchor-keywords: batch run result modeling, run summary example, chunk result example, retry candidate example, checkpoint type example, batch result type basics, batch status model beginner, 배치 결과 모델링 처음, chunk retry checkpoint intro, run summary what is, batch idempotency key boundaries, item-level idempotency key, chunk-level idempotency key, run-level idempotency key
...
3. 실패 item 중 다시 볼 대상은 `RetryCandidate`로 분리한다.
