...
이 비유는 entry용이다. 실제 구현에서는 redirect도 브라우저 정책과 상태 코드에 따라 보이는 method가 달라질 수 있고, SPA navigation도 API 호출 뒤 추가 요청을 일으킬 수 있다. 처음에는 "이동 주체"만 잡고, 세부 HTTP method 차이는 PRG 문서에서 이어서 보면 된다.
...
beginner가 여기서 많이 섞는 오해는 "`URL이 바뀌면 무조건 redirect`"다. 실제로는 아래처럼 한 칸 더 보면 덜 헷갈린다.

| 같이 확인할 단서 | redirect 쪽 | SPA navigation 쪽 |
|---|---|---|
| 서버 응답 | `3xx`와 `Location`이 보인다 | 보통 API는 `200/201`이고 라우터 코드가 뒤에서 돈다 |
| 새 문서 요청 | 브라우저가 새 HTTP 요청을 보낸다 | 문서 새로고침이 아니라면 없을 수 있다 |
| DevTools 느낌 | 요청이 두 줄 이상 이어진다 | API 성공 뒤 화면만 전환된 것처럼 보일 수 있다 |
