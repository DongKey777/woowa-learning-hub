...
| 같은 url인데 `304`가 보인다 | [HTTP 상태 코드 기초](./http-status-codes-basics.md) | 이동이 아니라 body 재사용 질문인가 |
...
- 같은 url의 body 재사용은 status/cache 질문이다.
...
- safe next step:
  [Fetch `response.redirected` vs `response.url` vs `opaqueredirect` 미니 카드](./fetch-redirected-response-url-opaqueredirect-mini-card.md) -> [Login Redirect, Hidden `JSESSIONID`, `SavedRequest` 입문](./login-redirect-hidden-jsessionid-savedrequest-primer.md)
