...
beginner가 상태 코드를 glossary처럼 외우다 막히는 지점은 "`302`도 성공처럼 보이고 `304`도 화면은 뜨는데 뭐가 다르죠?" 같은 장면이다. 이때는 숫자 이름보다 질문 순서를 먼저 고정하는 편이 안전하다.

| 먼저 던질 질문 | `예`면 먼저 읽을 뜻 | 다음 문서 |
|---|---|---|
| 다른 URL로 이동했는가 | redirect 계열일 가능성이 크다 | [Redirect vs Forward vs SPA Router Navigation 입문](./redirect-vs-forward-vs-spa-navigation-basics.md) |
| 같은 URL인데 body만 다시 안 받았는가 | cache 재사용 질문일 가능성이 크다 | [HTTP 캐싱과 조건부 요청 기초: Cache-Control, ETag, Last-Modified, 304](./http-caching-conditional-request-basics.md) |
| 그 자리에서 로그인이나 토큰 문제로 멈췄는가 | 인증 부재 질문일 가능성이 크다 | [Cookie / Session / JWT 브라우저 흐름 입문](./cookie-session-jwt-browser-flow-primer.md) |

짧게 말하면 아래처럼 읽는다.

- `302`/`303`: 목적지 이동 질문
- `304`: 같은 URL body 재사용 질문
- `401`: 인증 부재 질문

## beginner가 먼저 보는 3칸: url, method, body

상태 코드 숫자만 보면 `302`/`303`/`304`가 금방 섞인다. beginner는 먼저 "`url이 바뀌는가`", "`method가 바뀌는가`", "`body를 다시 받는가`" 세 칸만 같이 보면 훨씬 덜 헷갈린다.

| 먼저 볼 칸 | 질문 | `예`일 때 먼저 붙일 이름 |
|---|---|---|
| url | 다른 목적지로 이동했나 | `302`, `303`, redirect |
| method | `post` 결과를 `get`으로 다시 열었나 | `303`, prg |
| body | 같은 url body를 다시 쓰나 | `304`, cache revalidation |
...
그리고 "`왜 status code가 두 개 이상 보여요?`"라는 질문도 이 3칸으로 자르면 정리된다. 같은 브라우저 여정 안에서 `303 -> 200`이 보이면 url/method 변화 질문이고, 그다음 새로고침에서 `304`가 보이면 body 재사용 질문이다.
...
## status를 보고 다음 문서를 고르는 가장 짧은 분기

처음에는 코드를 많이 외우기보다 "지금 이 숫자가 어떤 follow-up 질문을 여는가"만 고정해도 충분하다.

| 먼저 보인 상태 | 바로 이어질 질문 | 다음 문서 |
|---|---|---|
| `302` | 다른 URL로 왜 보냈는가 | [Redirect vs Forward vs SPA Router Navigation 입문](./redirect-vs-forward-vs-spa-navigation-basics.md) |
| `303` | 왜 `POST` 결과를 `GET`으로 다시 열었는가 | [Post/Redirect/Get(PRG) 패턴 입문](./post-redirect-get-prg-beginner-primer.md) |
| `304` | 왜 body를 다시 안 받았는가 | [HTTP 캐싱과 조건부 요청 기초: Cache-Control, ETag, Last-Modified, 304](./http-caching-conditional-request-basics.md) |
| `401` | 왜 인증이 안 된 것으로 보였는가 | [Cookie / Session / JWT 브라우저 흐름 입문](./cookie-session-jwt-browser-flow-primer.md) |
