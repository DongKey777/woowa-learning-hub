...
|---|---|---|
| `302 Found` | 브라우저와 서버 조합에 따라 `GET`처럼 보일 수 있어 헷갈리기 쉽다 | 역사적으로 많이 보이지만 PRG 설명용 첫 선택으로는 덜 선명하다 |
...
메서드를 그대로 유지하는 다른 redirect 변형도 있지만, beginner 첫 읽기에서는 "`왜 `POST` 다음에 `GET`이 보여요?`"를 푸는 데 집중하는 편이 안전하다. 여기서의 요점은 "`302`가 항상 잘못이다"가 아니라, **입문 설명과 trace 독해에서는 `303`이 더 선명하다**는 것이다. 다른 redirect 변형은 이동 방식 문서를 읽은 뒤 follow-up으로 넘긴다.
...
즉 "`PRG를 넣었으니 중복 주문은 절대 안 난다`"처럼 절대화하면 오해다. 브라우저 새로고침 재전송을 줄여 주는 것은 맞지만, 네트워크 재시도나 서버 중복 생성 방지는 별도 설계가 필요하다.
