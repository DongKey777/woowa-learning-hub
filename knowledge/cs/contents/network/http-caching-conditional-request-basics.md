...
beginner 단계에서는 여기서 "`ETag`는 버전표다"까지만 먼저 잡으면 충분하다. strong/weak 차이처럼 validator 정밀도까지 들어가면 첫 캐시 mental model보다 범위가 커진다. 그 차이는 [Strong vs Weak ETag: validator 정밀도와 cache correctness](./strong-vs-weak-etag-validator-precision-cache-correctness.md)에서 이어서 본다.
...
- `304`는 리다이렉트가 아니다
- `304`는 캐시 재검증 결과이지, HTTP 버전 fallback/downgrade 신호가 아니다
...
### `304`와 HTTP 버전은 같은 질문이 아니다

- `304`는 cache 재검증 결과다
- `h2`/`h3` 표시는 연결 방식이다
- 둘이 같이 보여도 원인은 따로 읽는다
...
### shared cache/CDN 문제는 follow-up으로 넘긴다

브라우저 단일 캐시를 넘어서 `Vary`, compression variant, CDN key가 섞이면 beginner 범위를 넘기기 쉽다. 이 문서에서는 "`브라우저가 바로 재사용했는가`"와 "`서버에 다시 물었는가`"까지만 고정하고, shared cache는 [Compression, Cache, Vary, Accept-Encoding, Personalization](./compression-cache-vary-accept-encoding-personalization.md)로 넘긴다.
