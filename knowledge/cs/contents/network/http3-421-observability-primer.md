...
> 한 줄 요약: HTTP/3 421 Observability Primer: DevTools와 Edge Log로 Coalescing Recovery 읽기는 입문자가 먼저 잡아야 할 핵심 기준과 실무에서 헷갈리는 경계를 한 문서에서 정리한다.
...
## 먼저 분리: `421` 다음 성공 vs QUIC connect timeout 뒤 H2 fallback (계속 2)
...
## 첫 분기: discovery 이슈인가 reuse guardrail 이슈인가 (계속 2)
...
## 421 recovery와 app-level 4xx를 한 표로 구분하기 (계속 2)
...
## 한눈에 비교: `421 -> 403` vs `421 -> 200` (계속 2)
...
## Browser DevTools에서 먼저 볼 신호 (계속 2)
...
## Beginner 421 Proof 캡처 체크리스트 (계속 2)
...
## Beginner 421 Proof 캡처 체크리스트 (계속 3)
...
## Connection ID가 약한 브라우저용 20초 대체 카드 (계속 2)
...
## Safari Header Drilldown Card (계속 2)
