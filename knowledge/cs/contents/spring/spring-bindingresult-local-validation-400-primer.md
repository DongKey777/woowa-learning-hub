...
retrieval-anchor-keywords: bindingresult 400 flow, spring bindingresult beginner, bindingresult 있으면 뭐가 달라져요, 왜 methodargumentnotvalidexception 안 나요, @valid 실패인데 컨트롤러 안으로 왜 들어와요, bindingresult 붙였는데 왜 어떤 400은 여전히 컨트롤러 전에 끝나요, @requestbody bindingresult 뭘 잡아요, @requestbody bindingresult 왜 날짜 오류 못 잡아요, bindingresult empty body spring, bindingresult missing request body, json null before bindingresult, methodargumentnotvalidexception vs bindingresult, controller local validation handling, validation error local response spring, @valid bindingresult order
...
여기에 beginner가 자주 섞는 세 장면도 같이 묶어 둔다.

- body 자체를 안 보냈다
- body가 비어 있다
- JSON 본문으로 `null`만 보냈다

이 세 경우도 일반적인 `@RequestBody` JSON DTO 기준으로는 대개 `BindingResult`보다 앞단에서 갈린다. 다만 `@RequestBody(required = false)`나 nullable 파라미터 설계면 예외가 생길 수 있다.
...
여기서 handoff 용어를 같은 문장으로 고정하면 더 덜 헷갈린다.

- 로컬 `BindingResult` 처리: 컨트롤러 메서드가 validation 실패를 먼저 받아 응답 바디를 직접 정한다
- 전역 advice 번역: `MethodArgumentNotValidException`처럼 번진 실패를 `@RestControllerAdvice` / `ResponseEntityExceptionHandler`가 공통 HTTP 오류 계약으로 번역한다
- `LocalDate`/JSON parse 같은 message conversion 실패: 위 두 경로 전에 컨트롤러 진입 전 `400`으로 끝날 수 있다

즉 이 문서는 "`왜 이번 요청은 예외 처리 primer 쪽으로 갔고, 왜 이번 요청은 컨트롤러 안에서 멈췄지?`"를 가르는 입구 카드다.

## 증상과 다음 클릭

README 바인딩 follow-up 증상표와 같은 아래 네 줄이 이 문서의 직접 타깃이다.
...
## `BindingResult`를 붙이면 "예외 전에 메서드 안으로" 들어온다
...
## 메서드 시그니처로 바로 보면 더 안 헷갈린다

같은 시그니처라도 payload가 어디서 실패하느냐에 따라 결과가 달라진다.

| 실제 시그니처 | Spring이 잡는 것 | 못 잡는 것 | 보통 보이는 결과 |
|---|---|---|---|
| `create(@Valid @RequestBody dto)` | body를 DTO로 만든 뒤의 Bean Validation 실패 | JSON 문법 오류, `LocalDate` parse 실패, 숫자/enum 변환 실패를 메서드 안에서 직접 받기 | validation 실패면 `MethodArgumentNotValidException` |
| `create(@Valid @RequestBody dto, BindingResult br)` | 위와 같은 validation 실패를 `br`에 담아 로컬 처리 | JSON/body 읽기 실패, `HttpMessageNotReadableException` 계열 앞단 실패 | `br.hasErrors()`로 분기 가능 |
| `create(@Valid @RequestBody dto, String traceId, BindingResult br)` | 보장되지 않는다. `BindingResult`는 검증 대상 바로 뒤에 두는 편이 안전하다 | 잘못된 배치로 인한 기대와 다른 동작 | 예외로 바로 번지거나 의도와 다르게 읽힐 수 있음 |
| `create(@Valid @NotNull @RequestBody dto, BindingResult br)` | request body DTO 안의 필드 검증 일부 | method validation이 끼는 장면 전체 | 다른 제약까지 섞이면 `HandlerMethodValidationException`도 같이 봐야 함 |

## 요청 예시로 보면 "무엇을 못 잡는지"가 더 직접 보인다
...
#### `BindingResult`가 잡는 예시: DTO는 만들었고, 그다음 제약이 실패한 경우

```http
POST /members
Content-Type: application/json

{"name":"","age":0,"birthday":"2026-05-01"}
```

- JSON 문법은 맞다
- `age`는 숫자로 읽혔다
- `birthday`도 `LocalDate`로 변환됐다
- 그다음 `@NotBlank`, `@Positive`에서 실패한다

이 장면은 `BindingResult`가 잡을 수 있다. `bindingResult.getFieldErrors()`에 `name`, `age` 오류가 담긴다.

#### `BindingResult`가 못 잡는 예시: DTO를 만들다가 먼저 멈춘 경우

```http
POST /members
Content-Type: application/json

{"name":"neo","age":"old","birthday":"tomorrow"}
```

- `age`를 `Integer`로 못 바꾼다
- `birthday`를 `LocalDate`로 못 바꾼다
- DTO가 끝까지 만들어지지 않는다

이 장면은 `BindingResult` 차례 전에 끝난다. 보통 `HttpMessageNotReadableException` 같은 message conversion 계열 `400`을 먼저 본다.

## body 없음과 JSON `null`은 왜 또 다르게 보이나

초급자는 여기서 "`그럼 body를 안 보내거나 `null`을 보내면 field validation이 잡아 주는 거 아닌가요?`"를 바로 묻는다. 하지만 field-level validation은 **DTO 인스턴스를 만든 뒤** 돈다. 그래서 body 전체가 컨트롤러 인자로 성립하지 않으면 `BindingResult` 바구니까지 아직 못 온다.

#### `BindingResult`가 보통 못 잡는 예시: missing body / empty body / explicit JSON `null`

| 요청 장면 | 왜 앞단에서 갈리기 쉬운가 | 보통 `BindingResult`까지 오나 |
|---|---|---|
| body를 아예 안 보냄 | required body 자체가 비어 있다 | 보통 아니오 |
| body가 빈 문자열 또는 공백뿐 | 읽을 JSON 값이 없다 | 보통 아니오 |
| body가 `null` | JSON 토큰은 맞지만 객체 DTO를 기대한 흐름과 안 맞을 수 있다 | 보통 아니오 |

반대로 아래 장면은 `BindingResult`가 개입할 수 있다.
...
여기서는 DTO 생성이 끝났고, 그다음 `name`, `age` 필드 제약이 실패한다. 이 차이가 "`역직렬화 이후 field validation`"과 "`역직렬화 이전 body 문제`"의 경계다.

## 배치와 handoff

### 4. `BindingResult`는 바로 뒤에 둬야 한다
...
Spring 공식 문서 기준으로도 `Errors` 또는 `BindingResult`는 검증 대상 바로 옆에서 로컬 처리에 쓰는 패턴으로 설명된다. 반대로 다른 파라미터 제약이 같이 걸려 method validation이 필요해지면 `HandlerMethodValidationException`이 먼저 보일 수 있다.

### 5. 로컬 처리에서 전역 처리로 넘어가는 handoff
...
| 비교 질문 | 로컬 `BindingResult` 처리 | 전역 advice 번역 |
|---|---|---|
| 한 줄 정의 | 컨트롤러 메서드가 validation 실패를 먼저 받아 응답 바디를 직접 정한다 | `MethodArgumentNotValidException`처럼 번진 실패를 `@RestControllerAdvice` / `ResponseEntityExceptionHandler`가 공통 HTTP 오류 계약으로 번역한다 |
| validation 실패를 어디서 처음 잡나 | 현재 컨트롤러 메서드 | 전역 advice 계층 |
...
- "`body`가 없거나 `null`이면 `BindingResult`가 `@NotNull` field error로 받는다"
  보통은 아니다. 일반적인 `@RequestBody` DTO에서는 body 전체가 성립해야 field validation이 시작된다. missing body, empty body, explicit JSON `null`은 대개 그 앞단에서 갈린다.
...
**Q. body가 비었거나 `null`인데 왜 field error 대신 앞단 `400`이 나나요?**
`BindingResult`와 field validation은 역직렬화가 끝난 뒤에만 개입한다. 일반적인 `@RequestBody` DTO에서는 missing body, empty body, explicit JSON `null`이 "`DTO 인자를 만들 수 있나`" 단계에서 먼저 걸러진다.
