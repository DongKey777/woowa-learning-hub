...
|---|---|---|
| "`LottoService`에서 `new RandomNumberGenerator()`를 해도 되나요?" | 구매 서비스가 번호 생성 구현체 생성까지 직접 맡는 코드 | 유스케이스 조립과 협력 객체 선택 책임을 분리한다 |
| "테스트에서는 고정 번호 생성기를 넣고 싶은데 어디를 열어야 하죠?" | 랜덤 생성기가 service 내부에 숨어 테스트 seam이 닫힌 구조 | `NumberGenerator` 계약을 생성자 주입으로 받아 교체 가능하게 만든다 |
| "전략 인터페이스를 만들었는데 Spring에서는 뭘 더 해야 하나요?" | 인터페이스는 있지만 구현체 wiring이 애플리케이션에 드러나지 않는 상태 | 전략 분리와 Bean 등록/주입은 서로 다른 단계라는 점을 잡는다 |
