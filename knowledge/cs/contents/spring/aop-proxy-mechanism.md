...
## 검색어 브리지: primer 표현을 deep-dive 용어로 연결

초급 문서에서 보던 말을 여기 용어로 바꾸면 이렇게 대응된다.

| primer에서 들어온 말 | 이 문서에서 보는 정확한 말 | 먼저 볼 구간 |
|---|---|---|
| `프록시 안 지나면 annotation 안 먹음` | 프록시 기반 interception / self-invocation | [3. 왜 self-invocation이 문제인가](#3-왜-self-invocation이-문제인가) |
| `this.method transactional 안됨` | 내부 호출이 target으로 직행해서 proxy bypass 발생 | [3. 왜 self-invocation이 문제인가](#3-왜-self-invocation이-문제인가) |
| `JDK proxy`, `인터페이스 프록시` | JDK Dynamic Proxy | [2. JDK Dynamic Proxy와 CGLIB 차이](#2-jdk-dynamic-proxy와-cglib-차이) |
| `CGLIB`, `클래스 프록시`, `proxyTargetClass` | 클래스 기반 subclass proxy | [2. JDK Dynamic Proxy와 CGLIB 차이](#2-jdk-dynamic-proxy와-cglib-차이) |
| `final 메서드 프록시 왜 안 됨` | CGLIB가 상속/오버라이드 기반이라 final 제약 발생 | [시나리오 3](#시나리오-3-final-클래스메서드-때문에-cglib가-실패한다) |

한 줄 mental model:

- JDK proxy는 "인터페이스 앞에 대리인"을 세우는 느낌이다.
- CGLIB는 "원래 클래스를 상속한 대리 클래스"를 만드는 느낌이다.
- 둘 다 핵심은 같고, **호출이 프록시를 지나야 annotation 기능이 끼어든다**.
...
초급자 관점에서는 아래 한 표만 먼저 기억해도 길을 잃지 않는다.

| 지금 코드 모양 | Spring이 기대하는 프록시 방식 | 바로 떠올릴 질문 |
|---|---|---|
| 인터페이스를 통해 서비스 계약을 노출함 | JDK Dynamic Proxy 가능성이 큼 | "호출이 인터페이스 경계로 들어오나?" |
| 구현 클래스만 있고 인터페이스가 없음 | CGLIB 가능성이 큼 | "final 제약이 있나?" |
| `final class` 또는 `final method`가 보임 | CGLIB에 제약이 생김 | "상속/오버라이드 기반 프록시가 막히지 않나?" |
