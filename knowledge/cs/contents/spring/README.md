...
> 한 줄 요약: Spring을 처음 읽을 때는 MVC, DI, 트랜잭션, 요청 파이프라인 primer 4장을 먼저 보고, 운영형 deep dive는 막힌 증상이 생겼을 때만 내려가면 된다.
...
- [Network 카테고리 README: HTTP, 캐시, 세션에서 Spring으로 넘어오기](../network/README.md)
- [Network: HTTP의 무상태성과 쿠키, 세션, 캐시](../network/http-state-session-cache.md)

retrieval-anchor-keywords: spring beginner route, spring mvc primer, spring di primer, spring bean primer, spring transaction primer, spring request pipeline guide, spring 뭐부터 읽지, 처음 배우는데 spring, dispatcherservlet beginner, component scan basics, transactional beginner, spring readme return path, spring beginner safe next step, 왜 404 400 bean missing spring, spring 처음 뭐부터

<details>
<summary>Table of Contents</summary>

- beginner first: `처음 4장 루트`부터 읽고 아래 인덱스는 막힌 증상이 생겼을 때만 펼친다.
- deep dive 힌트: `전파`, `deadlock`, `timeout`, `observability`, `streaming`, `security session` 같은 단어가 보이면 심화 문서다.
...
이 `README`는 Spring primer와 deep dive를 함께 담은 **navigator 문서**다. 초급자는 인덱스를 처음부터 훑기보다 아래 4장 루트만 먼저 보고, 증상이 생겼을 때만 세부 문서로 내려가는 편이 안전하다.

- beginner first: MVC, DI, `@Transactional`, 요청 파이프라인 primer 4장을 먼저 본다.
- deep dive 신호: `propagation`, `deadlock`, `timeout`, `observability`, `condition report`, `streaming` 같은 단어가 보이면 지금 당장 다 읽지 말고 primer를 끝낸 뒤 막힌 증상에서만 내려간다.
- 증상 anchor: `404`는 MVC, `400`은 바인딩, `bean missing`은 DI/component scan, "`@Transactional`이 안 먹어요"는 프록시 경계부터 본다.
- safe next step: "`HTTP 다음에 Spring은 뭐부터 봐요?`"라면 [`Spring MVC 요청 생명주기 기초`](./spring-mvc-request-lifecycle-basics.md)부터 보고, browser 상태가 먼저 걸리면 [`Network 카테고리 README`](../network/README.md)로 잠깐 되돌아간다.
- 길을 잃으면 여기로 돌아오기: [`첫 판단 4축`](#첫-판단-4축)에서 다시 축을 고르고, HTTP/쿠키/세션 쪽이 먼저 헷갈리면 [`Network 카테고리 README`](../network/README.md)로 되돌아간다.
- beginner handoff: "`request -> controller -> database flow`가 처음"이면 [`HTTP 요청-응답 기본 흐름`](../network/http-request-response-basics-url-dns-tcp-tls-keepalive.md) -> [`Spring 요청 파이프라인과 Bean Container 기초`](./spring-request-pipeline-bean-container-foundations-primer.md) -> [`Database First-Step Bridge`](../database/database-first-step-bridge.md) 순서로만 내려간다. 반대로 `302 /login`, `cookie`, `304`가 먼저 헷갈리면 [`Network 카테고리 README`](../network/README.md)로 한 칸 되돌아간다.
...
처음에는 아래 4장만 읽어도 된다. 이 순서가 request pipeline, DI, transaction을 한 장면으로 묶어 준다.
...
| "`404`, `400`, DI 예외가 한 요청에서 다 섞여 보여요" | [`Spring 요청 파이프라인과 Bean Container 기초`](./spring-request-pipeline-bean-container-foundations-primer.md#7-1-같은-orders-요청인데도-먼저-보는-곳이-다르다) | [`Spring \`404\`/\`405\` vs Bean Wiring Error Confusion Card`](./spring-404-405-vs-bean-wiring-confusion-card.md) | 같은 주문 요청도 "길찾기 / 값 채우기 / 객체 조립 / 트랜잭션 경계"로 먼저 자르게 돕는다 |

## 처음 10분 라우트 메모

`controller`도 Bean이고 `@Transactional`도 Spring이라 헷갈리면, primer의 [`1분 비교`](./spring-request-pipeline-bean-container-foundations-primer.md#1분-비교-mvc--di--트랜잭션--요청-파이프라인) 표에서 질문 축부터 다시 자른다. 브라우저 상태가 먼저 걸리면 [`HTTP의 무상태성과 쿠키, 세션, 캐시`](../network/http-state-session-cache.md)로 잠깐 내려간다.

## 첫 10분에서 넘기기

처음 4장을 다 보고도 막힐 때만 follow-up으로 내려간다. `self-invocation`, `rollback-only`, `REQUIRES_NEW`, `condition report` 같은 단어는 "첫 10분" 범위가 아니라는 신호로 보면 된다.

1. [`Spring MVC 요청 생명주기 기초`](./spring-mvc-request-lifecycle-basics.md)
2. [`Spring Bean과 DI 기초`](./spring-bean-di-basics.md)
...
- **AOP·부트 자동구성·JPA·테스트**
  - [`AOP 기초`](./spring-aop-basics.md) — 횡단 관심사, Advice 종류, 30초 프록시 우회 체크리스트 + "`안 먹음`이면 프록시 경로 먼저" 기준으로 [`@Transactional 기초`](./spring-transactional-basics.md)와 왕복
  ...
  - [`Spring 테스트 기초`](./spring-testing-basics.md) — 처음인데 `@SpringBootTest` / `@WebMvcTest` / `@DataJpaTest` / `@JsonTest` / `@RestClientTest` 중 뭐부터 써야 할지, "`service bean not found`", "`테스트가 너무 느려요`"를 먼저 가르는 primer
...
- **처음 테스트와 트랜잭션 프록시가 같이 헷갈릴 때**
  - 컨트롤러 응답만 빨리 보거나 "`@WebMvcTest` / `@DataJpaTest` / `@SpringBootTest` 중 뭐가 맞죠?"라면 [`Spring 테스트 기초`](./spring-testing-basics.md)부터 본다.
  - "`JSON` 필드명이나 외부 API 헤더만 확인하고 싶어요"라면 [`Spring `@JsonTest` and `@RestClientTest` Slice Boundaries`](./spring-jsontest-restclienttest-slice-boundaries.md)로 가서 inbound/outbound 계약 테스트를 먼저 분리한다.
  ...
  - "`service bean not found`", "`갑자기 테스트가 너무 느려졌어요`"가 먼저 보이면 [`Spring 테스트 기초`](./spring-testing-basics.md#service-bean-not-found와-transactional-혼동-20초-분기)와 [`Spring Test Slices와 Context Caching`](./spring-test-slices-context-caching.md)에서 slice 경계와 context 재사용 문제를 먼저 가른다.
...
- 요청 한 번이 어디서 들어와 어디서 실패 응답으로 바뀌는지 한 장으로 잡고 싶다면:
  - [`Spring MVC 요청 생명주기 기초`](./spring-mvc-request-lifecycle-basics.md) — 요청 진입부터 binding, controller, 예외 응답까지 한 장으로 묶는 입문 문서
  ...
  - [`Spring @RequestParam vs @PathVariable 초급 비교 카드`](./spring-requestparam-vs-pathvariable-beginner-card.md) — path 값과 query 조건을 먼저 분리하는 primer
  - [`Spring @ModelAttribute vs @RequestBody 초급 비교 카드`](./spring-modelattribute-vs-requestbody-binding-primer.md) — 검색 query와 생성 JSON 바인딩 차이를 먼저 끊는 primer
  - [`Spring @RequestBody가 컨트롤러 전에 400 나는 이유`](./spring-requestbody-400-before-controller-primer.md) — 1단계: `400` vs `415` vs `302/403`를 어디서 먼저 가를지 설명하는 first-hit 카드
  - [`Spring @Valid는 언제 타고 언제 못 타는가: 400 첫 분기 primer`](./spring-valid-400-vs-message-conversion-400-primer.md) — 2단계: DTO 변환 실패와 validation 실패를 먼저 가르는 초급 카드
  - [`Spring BindingResult가 있으면 400 흐름이 어떻게 달라지나`](./spring-bindingresult-local-validation-400-primer.md) — 3단계: validation `400`이 전역 예외로 가는지, 컨트롤러 로컬 분기로 바뀌는지 구분하는 primer
  - [`Spring LocalDate/LocalTime JSON 파싱 400 자주 나는 형식 모음`](./spring-localdate-localtime-json-400-cheatsheet.md) — follow-up: `date`/`time` 문자열 때문에 DTO 변환 실패 `400`이 날 때 바로 찾는 cheat sheet
...
| DB 작업을 어디까지 묶나 | [`@Transactional 기초`](./spring-transactional-basics.md) | 옵션보다 "프록시를 탔는가"를 먼저 보는 습관이 중요하다 | [`@Transactional 깊이 파기`](./transactional-deep-dive.md) |
...
| "`400`이 controller 전에 나요" | [`Spring MVC 요청 생명주기 기초`](./spring-mvc-request-lifecycle-basics.md) | [`Spring @RequestBody가 컨트롤러 전에 400 나는 이유`](./spring-requestbody-400-before-controller-primer.md) | 바인딩 실패와 비즈니스 실패를 먼저 자른다 |
...
> "`@Transactional`만 문제"라는 초급자 오해를 먼저 끊고, self-invocation을 프록시 기반 annotation 공통 규칙으로 다시 잡아 고급 매트릭스 진입 전에 1분 프라이머로 연결한다. `@Async`는 같은 스레드 실행 로그 전/후 예제로 프록시 우회 여부를 바로 확인하게 보강했고, 바로 아래에서 `bean split` / `facade-worker` / `self-injection`을 한 표로 비교해 첫 수리 선택을 돕는다.
...
- [AOP 기초: 관점 지향 프로그래밍이 왜 필요한가 (`this.method()`/`private`/직접 `new` 30초 체크리스트를 질문별 개별 anchor로 바로 점프 + 문제/개선 코드 예시 옆에서 각 질문 anchor로 바로 왕복 + `self-invocation`과 `self-injection` 차이를 초반 표와 3줄 경고로 먼저 분리 + `@Transactional/@Cacheable/@Async` 비교표 옆 Bean/public/external call Yes/No 미니 진단 카드 + `@Cacheable` 내부 호출 시 DB 반복 조회 1분 확인 절차)](./spring-aop-basics.md)
- [Spring Self-Invocation 공통 오해 1페이지 카드: "`@Transactional`만 문제"가 아니다](./spring-self-invocation-transactional-only-misconception-primer.md)
- [Spring Self-Invocation Proxy Trap Matrix: `@Transactional`, `@Async`, `@Cacheable`, `@Validated`, `@PreAuthorize`](./spring-self-invocation-proxy-annotation-matrix.md)
...
> validation `400`은 request body 변환, validation, 로컬 `BindingResult`, 날짜 형식 중 어디서 끊겼는지 먼저 나누면 덜 헷갈린다.
