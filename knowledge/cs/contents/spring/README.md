...
> 한 줄 요약: Spring을 처음 읽을 때는 MVC, DI, 트랜잭션, 요청 파이프라인 primer 4장만 먼저 보고, 테스트 운영어·전파 옵션·incident 키워드는 관련 문서 링크 뒤로 미뤄 두는 편이 안전하다.
...
- [처음 막힐 때 30초 선택표: `404`/`bean missing`/`@Transactional` 첫 분기](#처음-막힐-때-30초-선택표)
- [Spring primer 되돌아가기: 질문 축 다시 고르기](#spring-primer-되돌아가기)
- [빠른 복귀 규칙: Network/Security/CS 루트로 안전하게 되돌아가기](#빠른-복귀-규칙)
- [Spring MVC 바인딩/400 -> `ProblemDetail` 4단계 주 라우트: `@RequestBody 400`부터 validation/error response, `ProblemDetail` shaping, 예외 처리 -> validation pipeline 내부로 복귀까지](#validation-400-problemdetail-route)
...
- [Spring Security Filter Chain](./spring-security-filter-chain.md)
- [Network 카테고리 README: HTTP, 캐시, 세션에서 Spring으로 넘어오는 handoff](../network/README.md#network---spring-handoff)
- [Security README: `401`/`403`/login loop를 browser/session 기준으로 다시 자르는 입구](../security/README.md#browser--session-beginner-ladder)
...
retrieval-anchor-keywords: spring beginner route, spring primer route, spring 뭐부터 읽지, spring 처음 헷갈려요, spring mvc primer, spring di primer, spring bean primer, spring transaction primer, 404 400 spring first split, validation 400 route spring, validation 400 exception handling pipeline, 요청마다 bean 만들어요, dispatcherservlet이 service 넣어줘요, mvc di transaction 차이, problemdetail 다음 validation pipeline

<details>
<summary>Table of Contents</summary>

- beginner first: `처음 4장 루트`부터 읽고 아래 인덱스는 막힌 증상이 생겼을 때만 펼친다.
- deep dive 힌트: `전파`, `timeout`, `deadlock`, `observability`, `streaming`, `security session` 같은 단어가 보이면 지금은 링크만 저장해 두고 primer부터 끝낸다.
...
- 혼합 증상 anchor: "`404`와 `bean missing`이 같이 보여요", "`controller`는 뜨는데 `service`/트랜잭션이 헷갈려요"처럼 두 축이 한 화면에 섞이면 [`Spring 요청 파이프라인과 Bean Container 기초`](./spring-request-pipeline-bean-container-foundations-primer.md)로 먼저 타임라인을 자른다.
- beginner confusion anchor: "`요청마다 Bean을 새로 만들어요?`", "`DispatcherServlet`이 `service`도 넣어줘요?`", "`@Transactional`이 Bean도 만들어 줘요?`"처럼 MVC/DI/트랜잭션 역할이 섞이면 [`Spring 요청 파이프라인과 Bean Container 기초`](./spring-request-pipeline-bean-container-foundations-primer.md)부터 본다.
...
- test bridge: "`@WebMvcTest`인데 service가 없어요"와 "`service는 있는데 트랜잭션이 안 먹어요`"는 둘 다 service 단어가 보여도 다른 질문이다. 앞은 [`Spring Test Slice Scan Boundary 오해`](./spring-test-slice-scan-boundaries.md), 뒤는 [`@Transactional 기초`](./spring-transactional-basics.md)로 바로 가른다.
- stop line for beginners: `@Import`, `@TestConfiguration`, `context cache`, `REQUIRES_NEW`, `rollback-only`가 먼저 보여도 지금은 "`slice 질문인가`, `프록시 질문인가`"만 고르고 다음 문서로 넘긴다.

## beginner 정지선

처음 읽는 단계에서는 아래 단어가 보여도 primer 본문에서 해결하려 들지 않고, **현재 질문 축만 고른 뒤 링크로 넘기는 편**이 안전하다.

| 먼저 보인 단어 | 지금 여기서 내릴 1차 판단 | 바로 넘길 문서 |
|---|---|---|
| `REQUIRES_NEW`, `rollback-only`, `self-invocation` | `@Transactional` 심화/프록시 follow-up 신호다 | [`@Transactional 기초`](./spring-transactional-basics.md), [`Spring Self-Invocation 공통 오해 1페이지 카드`](./spring-self-invocation-transactional-only-misconception-primer.md) |
| `@Import`, `@TestConfiguration`, `context cache` | 테스트 slice 확장 신호다 | [`Spring 테스트 기초`](./spring-testing-basics.md), [`Spring Test Slice Scan Boundary 오해`](./spring-test-slice-scan-boundaries.md) |
| `deadlock`, `timeout`, `observability`, `streaming` | beginner primer 범위를 넘는 운영/incident 신호다 | [`Spring Transaction Debugging Playbook`](./spring-transaction-debugging-playbook.md), [`Spring Async MVC Streaming Observability Playbook`](./spring-async-mvc-streaming-observability-playbook.md) |
| `302 /login`, `cookie`, `saved request` | Spring 내부보다 browser/security handoff 질문일 수 있다 | [`Network -> Spring handoff`](../network/README.md#network---spring-handoff), [`Security README`](../security/README.md#browser--session-beginner-ladder) |

## primer 4장 한눈에

처음엔 이 네 장을 "다른 주제 4개"가 아니라 **한 요청을 네 각도에서 자르는 지도**라고 보면 된다.

| primer | 이 문서가 답하는 첫 질문 | 읽고 바로 이어가기 좋은 다음 한 걸음 |
|---|---|---|
| [`Spring MVC 요청 생명주기 기초`](./spring-mvc-request-lifecycle-basics.md) | "`요청이 어디로 들어가고 왜 404/400이 나와요?`" | 요청과 Bean 준비가 섞여 보이면 [`Spring 요청 파이프라인과 Bean Container 기초`](./spring-request-pipeline-bean-container-foundations-primer.md) |
| [`IoC와 DI 기초`](./spring-ioc-di-basics.md) | "`왜 new 대신 주입해요?`" | "`bean`과 `component scan`이 궁금해요"까지 이어지면 [`Spring Bean과 DI 기초`](./spring-bean-di-basics.md) |
| [`@Transactional 기초`](./spring-transactional-basics.md) | "`commit/rollback은 누가 묶어요?`" | "`AOP`/프록시가 왜 나오죠?"면 [`AOP 기초`](./spring-aop-basics.md), "`service bean not found`와 헷갈려요"면 [`Spring 테스트 기초`](./spring-testing-basics.md) |
| [`Spring 요청 파이프라인과 Bean Container 기초`](./spring-request-pipeline-bean-container-foundations-primer.md) | "`MVC`, `DI`, `트랜잭션`이 한 요청에서 어떻게 만나요?`" | controller 다음 `save()`와 SQL이 궁금해지면 [`Database First-Step Bridge`](../database/database-first-step-bridge.md) |

짧게 외우면 `요청 길찾기 -> 객체 조립 이유 -> DB 작업 묶기 -> 한 요청에서 다시 합치기` 순서다.

처음 10분엔 아래 오해 두 개만 끊어도 읽기 난도가 크게 내려간다.

| 초급자가 자주 섞는 말 | 사실 먼저 나눌 질문 | 안전한 첫 문서 |
|---|---|---|
| "`@WebMvcTest`인데 service가 없어요" | "`무엇을 띄웠나?`" | [`Spring Test Slice Scan Boundary 오해`](./spring-test-slice-scan-boundaries.md) |
| "`service`는 있는데 `@Transactional`이 안 먹어요" | "`어떻게 호출했나?`" | [`@Transactional 기초`](./spring-transactional-basics.md) |
| "`AOP`가 왜 갑자기 나와요?" | "`트랜잭션 설명 도중 프록시 전달 방식을 모르는가?`" | [`AOP 기초`](./spring-aop-basics.md) |

## primer 4장 읽는 순서가 헷갈릴 때

처음에는 네 문서를 "다 읽어야 한다"보다 **지금 막힌 문장을 어느 문서에 맡길지**로 고르면 된다.

| 지금 먼저 보인 말 | 먼저 열 문서 | 여기서 바로 안 파는 것 | 다음 한 걸음 |
|---|---|---|---|
| "`왜 404/400이 나요?`", "`컨트롤러까지 안 가요`" | [`Spring MVC 요청 생명주기 기초`](./spring-mvc-request-lifecycle-basics.md) | Bean 등록, 트랜잭션 옵션 | [`Spring 요청 파이프라인과 Bean Container 기초`](./spring-request-pipeline-bean-container-foundations-primer.md) |
| "`왜 new 대신 주입해요?`", "`DI가 왜 필요해요?`" | [`IoC와 DI 기초`](./spring-ioc-di-basics.md) | component scan 세부, 프록시 경계 | [`Spring Bean과 DI 기초`](./spring-bean-di-basics.md) |
| "`bean`이 뭐예요?", "`service`는 누가 넣어요?`" | [`Spring Bean과 DI 기초`](./spring-bean-di-basics.md) | `404`/`400`, rollback-only | [`Spring 요청 파이프라인과 Bean Container 기초`](./spring-request-pipeline-bean-container-foundations-primer.md) |
| "`왜 @Transactional이 안 먹어요?`", "`commit/rollback은 누가 해요?`" | [`@Transactional 기초`](./spring-transactional-basics.md) | `404`, component scan, slice 범위 | [`AOP 기초`](./spring-aop-basics.md) |

짧게 말하면 `MVC는 요청 길찾기`, `DI는 객체 조립`, `@Transactional`은 `DB 작업 묶기`, 요청 파이프라인 primer는 `셋을 다시 한 화면에 붙이기`다.

처음 질문이 "`테스트에서 본 오류`"라면 한 줄만 더 기억하면 된다.

- "`service`가 아예 없어요"면 test slice 문장일 수 있다.
- "`service`는 있는데 동작이 이상해요"면 transaction 프록시 문장일 수 있다.
- "`controller` 전에 400이 나요"면 테스트 종류보다 request binding 문장일 수 있다.

## beginner 질문 한 줄 라우팅 카드

처음에는 용어보다 "`내 질문이 어느 축인가`"를 먼저 고르는 편이 빠르다.

| 지금 입에서 먼저 나오는 말 | 사실 먼저 자를 축 | 먼저 읽을 primer | 바로 이어 읽을 문서 |
|---|---|---|---|
| "`왜 404/400이 나요?`", "`요청이 컨트롤러까지 안 가요`" | MVC 요청 길찾기 / 바인딩 | [`Spring MVC 요청 생명주기 기초`](./spring-mvc-request-lifecycle-basics.md) | [`Spring 요청 파이프라인과 Bean Container 기초`](./spring-request-pipeline-bean-container-foundations-primer.md) |
| "`왜 new 대신 주입해요?`", "`DI가 왜 필요해요?`" | 객체 조립 이유 | [`IoC와 DI 기초`](./spring-ioc-di-basics.md) | [`Spring Bean과 DI 기초`](./spring-bean-di-basics.md) |
| "`bean`이 뭐예요?", "`controller` 안 `service`는 누가 넣어요?`" | Bean 등록 / DI 연결 | [`Spring Bean과 DI 기초`](./spring-bean-di-basics.md) | [`Spring 요청 파이프라인과 Bean Container 기초`](./spring-request-pipeline-bean-container-foundations-primer.md) |
| "`왜 @Transactional이 안 먹어요?`", "`commit/rollback은 누가 해요?`" | service 경계 / 프록시 | [`@Transactional 기초`](./spring-transactional-basics.md) | [`Spring 요청 파이프라인과 Bean Container 기초`](./spring-request-pipeline-bean-container-foundations-primer.md) |

혼합 증상도 한 줄로 자르면 된다.

- "`404`와 `bean missing`이 같이 보여요"면 한 문제로 묶지 말고 [`Spring 요청 파이프라인과 Bean Container 기초`](./spring-request-pipeline-bean-container-foundations-primer.md)에서 요청 타임라인과 Bean 준비 타임라인을 먼저 분리한다.
- "`controller` 다음 `save()`가 궁금해요"면 [`Database First-Step Bridge`](../database/database-first-step-bridge.md)로 넘긴다.
- "`302 /login`이나 `cookie`가 먼저 보여요"면 [`Network -> Spring handoff`](../network/README.md#network---spring-handoff)나 [`Security README`](../security/README.md#browser--session-beginner-ladder)로 잠깐 되돌아간다.
...
- stop line: `REQUIRES_NEW`, `context cache`, `deadlock`, `SSE`, `SavedRequest`가 먼저 보이면 이 README 위쪽 primer 구간까지만 읽고, 아래 deep dive cluster는 링크만 저장한다.
...
|---|---|---|---|
| "`DispatcherServlet`이 뭐예요?", "`404`예요" | [`Spring MVC 요청 생명주기 기초`](./spring-mvc-request-lifecycle-basics.md) | [`Spring 요청 파이프라인과 Bean Container 기초`](./spring-request-pipeline-bean-container-foundations-primer.md) | 요청 입구와 전체 그림을 한 요청 예시로 붙여 준다 |
...
| "`404`와 `bean missing`이 같이 보여요", "`요청 문제인지 주입 문제인지 모르겠어요`" | [`Spring 요청 파이프라인과 Bean Container 기초`](./spring-request-pipeline-bean-container-foundations-primer.md) | [`Spring MVC 요청 생명주기 기초`](./spring-mvc-request-lifecycle-basics.md) 또는 [`Spring Bean과 DI 기초`](./spring-bean-di-basics.md) | 한 요청 화면에 섞인 증상을 "요청 타임라인 vs Bean 준비 타임라인"으로 먼저 잘라 준다 |

## 테스트/트랜잭션 분기

service라는 단어가 보여도 test slice와 transaction 프록시는 다른 질문이다.
...
| "`@WebMvcTest`인데 service bean not found예요", "`slice 문제인지 트랜잭션 문제인지 모르겠어요`" | [`Spring Test Slice Scan Boundary 오해`](./spring-test-slice-scan-boundaries.md) | [`@Transactional 기초`](./spring-transactional-basics.md) | "무엇을 띄웠나"와 "어떻게 호출했나"를 분리해서 테스트 용어가 runtime 개념을 덮지 않게 해 준다 |
| "`@Import` 붙였더니 갑자기 bean이 많이 보여요`", "`테스트가 무거워졌어요`" | [`Spring Test Slice Scan Boundary 오해`](./spring-test-slice-scan-boundaries.md) | [`Spring Test Slice `@Import` / `@TestConfiguration` Boundary Leaks`](./spring-test-slice-import-testconfiguration-boundaries.md) | beginner primer에서는 leak 신호만 잡고, 운영형 확장은 deep dive 문서로 밀어 둔다 |

짧게 외우면 테스트/트랜잭션 축도 beginner용 첫 분기는 두 개뿐이다. "`service가 아예 없나`"면 slice, "`service는 있는데 동작이 이상하나`"면 프록시다. 전파 옵션이나 context cache는 그다음 링크로 넘긴다.

## 빠른 복귀 규칙

- deep dive 신호: `propagation`, `deadlock`, `timeout`, `observability`, `streaming`, `REQUIRES_NEW`, `context cache`, `@Import`, `@TestConfiguration` 같은 단어가 보이면 primer 뒤에만 내려간다.
...
- security handoff: `401`, `403`, `login loop`, `saved request bounce`가 먼저면 [`Security README: Browser / Session Beginner Ladder`](../security/README.md#browser--session-beginner-ladder)에서 browser/session branch를 먼저 고른 뒤 다시 Spring 문서로 돌아온다.
...
| "`@WebMvcTest`인데 service bean not found예요`", "`어느 테스트부터 써요?`" | [`Spring Test Slice Scan Boundary 오해`](./spring-test-slice-scan-boundaries.md) | test slice와 프록시 문제를 다른 축으로 분리해 준다 |
...
| `deadlock`, `lock wait`, `timeout`, `observability`, `streaming` | primer 범위를 넘는 운영/incident형 신호다 | README 아래쪽 deep dive cluster 또는 관련 문서 링크만 저장 |
...
| "`DispatcherServlet`이 `service`도 주입해요?", "요청마다 Bean을 새로 만들어요?" | 요청 길찾기와 객체 조립 타임라인을 같은 순서로 보는 습관 | [`Spring 요청 파이프라인과 Bean Container 기초`](./spring-request-pipeline-bean-container-foundations-primer.md) |
...
| "`404`, `400`, DI 예외가 한 요청에서 다 섞여 보여요" | [`Spring 요청 파이프라인과 Bean Container 기초`](./spring-request-pipeline-bean-container-foundations-primer.md) | [`Spring 요청 파이프라인 기초의 \`404\`/\`400\`/\`bean missing\` 혼동 분리표`](./spring-request-pipeline-bean-container-foundations-primer.md#mvc-di-transaction-confusion-card) | 같은 주문 요청도 "길찾기 / 값 채우기 / 객체 조립 / 트랜잭션 경계"로 먼저 자르게 돕는다 |
...
처음 4장을 다 보고도 막힐 때만 follow-up으로 내려간다. `self-invocation`, `rollback-only`, `REQUIRES_NEW`, `condition report`, `deadlock`, `timeout`, `observability` 같은 단어는 "첫 10분" 범위가 아니라는 신호다. 처음에는 deep dive 여러 장을 연달아 열지 말고, **primer 1장 + follow-up 1장**까지만 이동하면 된다.
...
follow-up을 붙일 때도 아래처럼 한 장씩만 고르면 된다.

| primer에서 남은 질문 | 붙일 follow-up 1장 |
|---|---|
| "`415`, `Accept`, `produces`, `consumes`가 더 궁금해요" | [`Spring `@RequestBody 415 Unsupported Media Type` 초급 primer`](./spring-requestbody-415-unsupported-media-type-primer.md) |
| "`service`는 있는데 `@Transactional`이 안 먹어요" | [`Spring Self-Invocation 공통 오해 1페이지 카드`](./spring-self-invocation-transactional-only-misconception-primer.md) |
| "`service bean not found`가 test에서만 보여요" | [`Spring Test Slice Scan Boundary 오해`](./spring-test-slice-scan-boundaries.md) |
| "`controller` 다음 SQL/lock/flush`가 궁금해요" | [`Database First-Step Bridge`](../database/database-first-step-bridge.md) |
...
- **Spring 5단계 시작점**
  ...
  - [`@Transactional 기초`](./spring-transactional-basics.md) — "요청 하나"와 "트랜잭션 하나"를 분리하고, `this.method()`/`private`/직접 `new`를 **프록시 정문 3문항**으로 먼저 거르는 primer
...
- **AOP·부트 자동구성·JPA·테스트**
  - [`AOP 기초`](./spring-aop-basics.md) — 처음에는 "annotation이 붙는 문은 프록시"라는 감각과 `this.method()`/`private`/직접 `new`를 묶은 **프록시 정문 3문항**만 잡고, 자세한 프록시 메커니즘은 관련 문서로 넘기는 입문 primer
...
- 요청 한 번이 어디서 들어와 어디서 실패 응답으로 바뀌는지 한 장으로 잡고 싶다면:
  ...
  - [`Spring @RequestBody가 컨트롤러 전에 400 나는 이유`](./spring-requestbody-400-before-controller-primer.md) — 1단계: `컨트롤러 로그 안 찍힘`, `@RequestBody인데 컨트롤러 전에 400이 나요`, `JSON parse error가 보여요`처럼 body 파싱/DTO 변환 실패를 먼저 의심하는 first-hit 카드
  - [`Spring @RequestBody 415 Unsupported Media Type 초급 primer`](./spring-requestbody-415-unsupported-media-type-primer.md) — 1.5단계 보조 분기: `JSON인데 415 Unsupported Media Type가 떠요`, `Content-Type: application/json 안 붙였는데 415예요`, `Content-Type 때문에 막힌 것 같아요`처럼 검색어가 직접 `415`/헤더를 말할 때 바로 꽂히는 entrypoint
...
| `컨트롤러 로그 안 찍힘`, `@RequestBody인데 컨트롤러 전에 400이 나요`, `JSON parse error가 보여요` | [`Spring @RequestBody가 컨트롤러 전에 400 나는 이유`](./spring-requestbody-400-before-controller-primer.md) | `Content-Type`보다 먼저 body 값 해석/DTO 변환 실패 후보를 보게 한다 |
| `JSON인데 415 Unsupported Media Type가 떠요`, `Content-Type: application/json 안 붙였는데 415예요`, `Content-Type 때문에 막힌 것 같아요` | [`Spring @RequestBody 415 Unsupported Media Type 초급 primer`](./spring-requestbody-415-unsupported-media-type-primer.md) | body 값보다 먼저 `Content-Type` 헤더/계약 문제를 가장 짧게 찾게 한다 |
...
## 바인딩 primer follow-up
  ...
  - [`Spring LocalDate/LocalTime JSON 파싱 400 자주 나는 형식 모음`](./spring-localdate-localtime-json-400-cheatsheet.md) — 2.5단계 형식 오류 follow-up: `date`/`time` 형식 오류처럼 validation 전에 멈춘 변환 실패를 빠르게 찾는다
  - [`Spring BindingResult가 있으면 400 흐름이 어떻게 달라지나`](./spring-bindingresult-local-validation-400-primer.md) — 3단계: "`BindingResult` 있으면 뭐가 달라져요?`, `왜 MethodArgumentNotValidException 안 나요?`"처럼 전역 예외와 컨트롤러 로컬 분기를 가르는 증상을 바로 받는다
...
- [`Spring 커스텀 Error DTO에서 `ProblemDetail`로 넘어가는 초급 handoff primer`](./spring-custom-error-dto-to-problemdetail-handoff-primer.md) — 4단계 주 라우트 마감: validation `400` 응답 바디 계약과 `ProblemDetail` shaping을 팀 DTO 대비로 정리하고, 다음 return path를 예외 처리 -> validation pipeline 내부 복기로 이어 준다
...
|---|---|---|
| `왜 @Valid 안 타요?`, `같은 400인데 뭐가 다른가요?`, `@NotBlank가 왜 안 먹어요?` | [`Spring @Valid는 언제 타고 언제 못 타는가: 400 첫 분기 primer`](./spring-valid-400-vs-message-conversion-400-primer.md) | [`Spring @RequestBody가 컨트롤러 전에 400 나는 이유`](./spring-requestbody-400-before-controller-primer.md) |
| "`BindingResult` 있으면 뭐가 달라져요?", "`@Valid` 실패인데 왜 컨트롤러 안으로 들어와요?" | [`Spring BindingResult가 있으면 400 흐름이 어떻게 달라지나`](./spring-bindingresult-local-validation-400-primer.md) | [`Spring 예외 처리 기초`](./spring-exception-handling-basics.md) |

## validation/응답 바디 follow-up 증상표
...
| "`2026/05/01` 보냈는데 `400`이 나요?", "`18시`, `오후 6시`, `6pm` 보내면 왜 안 되죠?" | [`Spring LocalDate/LocalTime JSON 파싱 400 자주 나는 형식 모음`](./spring-localdate-localtime-json-400-cheatsheet.md) | [`Spring @Valid는 언제 타고 언제 못 타는가: 400 첫 분기 primer`](./spring-valid-400-vs-message-conversion-400-primer.md) |
| "`validation 400` 응답 바디를 어떤 계약으로 보여 줘야 해요?", "`ProblemDetail`로 가야 하나요?" | [`Spring 커스텀 Error DTO에서 `ProblemDetail`로 넘어가는 초급 handoff primer`](./spring-custom-error-dto-to-problemdetail-handoff-primer.md) | [`Spring 예외 처리 기초`](./spring-exception-handling-basics.md) |
...
| "`404`, DI 예외, `400`, 트랜잭션 문제가 한꺼번에 섞여요" | [`Spring 요청 파이프라인과 Bean Container 기초`](./spring-request-pipeline-bean-container-foundations-primer.md#mvc-di-transaction-confusion-card) | [`Spring MVC -> JDBC/트랜잭션 -> DI/AOP 전환 오해 체크리스트`](./spring-mvc-jdbc-transaction-di-aop-transition-checklist.md) | 같은 요청 장면에서도 "길찾기 / 객체 조립 / 값 채우기 / 트랜잭션 경계"를 먼저 분리하게 돕는다 |
...
- 트랜잭션 / persistence cluster로 바로 들어가려면:
  - beginner-safe ladder는 `무엇을 같이 commit/rollback하나` -> `Spring에서 그 경계가 어디서 적용되나` -> `옵션/동시성 follow-up 1장` 순서다. 안전한 첫 경로는 [트랜잭션 기초](../database/transaction-basics.md) -> [`@Transactional 기초`](./spring-transactional-basics.md) -> [`Spring Transaction Propagation Beginner Primer: `REQUIRED`, `REQUIRES_NEW`, rollback-only`](./spring-transaction-propagation-required-requires-new-rollbackonly-primer.md) 또는 [트랜잭션 격리 수준 기초](../database/transaction-isolation-basics.md)다.
  - `DB Lock Wait / Deadlock vs Spring Proxy / Rollback`
  - `[playbook] Spring Transaction Debugging Playbook`
  - `REQUIRES_NEW`, `rollbackFor`, `readOnly`부터 먼저 열고 있으면 [Spring Self-Invocation 공통 오해 1페이지 카드](./spring-self-invocation-transactional-only-misconception-primer.md)로 내려가 `this.method()`/`private`/직접 `new` 30초 체크를 먼저 끝낸다.
  - `Spring Persistence Context Flush / Clear / Detach`
  - `dirty read`, `write skew`, `phantom`, `왜 @Transactional이 안 먹지`, `왜 안 롤백되지`가 같이 섞이면 처음부터 `REQUIRES_NEW`, rollback-only, readOnly, routing-datasource까지 한 번에 열지 말고 beginner ladder를 [트랜잭션 기초](../database/transaction-basics.md) -> [`@Transactional 기초`](./spring-transactional-basics.md) -> [트랜잭션 격리 수준 기초](../database/transaction-isolation-basics.md) 또는 [`Spring Transaction Propagation Beginner Primer: `REQUIRED`, `REQUIRES_NEW`, rollback-only`](./spring-transaction-propagation-required-requires-new-rollbackonly-primer.md) 순으로 먼저 맞춘다. `트랜잭션 격리수준과 락`, `@Transactional 깊이 파기`, `Spring Service-Layer Transaction Boundary Patterns`는 그다음 deep dive core ladder다.
...
| 지금 먼저 보인 말 | 먼저 열 문서 | 이유 |
|---|---|---|
| "`왜 new 대신 주입해요?`" | [IoC와 DI 기초](./spring-ioc-di-basics.md) | 객체 조립 책임을 왜 컨테이너로 넘기는지부터 잡는다 |
| "`bean`이 뭐예요?", "`service`는 누가 연결해요?`" | [Spring Bean과 DI 기초](./spring-bean-di-basics.md) | Bean 후보 등록과 wiring 감각이 바로 필요하다 |
| "`404`랑 `bean missing`이 같이 보여요" | [Spring 요청 파이프라인과 Bean Container 기초](./spring-request-pipeline-bean-container-foundations-primer.md) | 요청 길찾기와 객체 조립 타임라인을 먼저 분리해야 한다 |
| "`service`는 보이는데 `@Transactional`이 안 먹어요" | [@Transactional 기초](./spring-transactional-basics.md) | Bean 등록보다 프록시 경계 질문일 가능성이 크다 |
...
- [@Transactional 기초: 요청 하나와 트랜잭션 하나를 분리하고, `this.method()`/`private`/직접 `new`를 묶은 `프록시 정문 3문항`부터 보는 프록시 우선 입문](./spring-transactional-basics.md)
...
- [AOP 기초: 관점 지향 프로그래밍이 왜 필요한가 (`프록시 정문 3문항`의 `this.method()`/`private`/직접 `new` anchor로 바로 점프 + 문제/개선 코드 예시 옆에서 같은 anchor로 바로 왕복 + 초반 표와 3줄 경고로 `self-invocation`과 `self-injection` 차이를 먼저 분리 + `@Transactional/@Cacheable/@Async` 공통 프록시 규칙을 한 표로 다시 묶는 입문 primer)](./spring-aop-basics.md)
- [Spring Self-Invocation 공통 오해 1페이지 카드: "`@Transactional`만 문제"가 아니다](./spring-self-invocation-transactional-only-misconception-primer.md)
- [Spring Self-Invocation Proxy Trap Matrix: `@Transactional`, `@Async`, `@Cacheable`, `@Validated`, `@PreAuthorize`](./spring-self-invocation-proxy-annotation-matrix.md)
...
> validation `400` 입문은 4단계 주 라우트를 `@RequestBody 400` -> `@Valid` 분기 -> `BindingResult` 로컬 처리 -> `ProblemDetail` handoff로 잡고, `415`는 1.5단계 보조 분기, `LocalDate/LocalTime`는 2.5단계 형식 오류 follow-up으로 끼워 넣으면 가장 덜 헷갈린다.
...
> 주 라우트는 1 `@RequestBody 400` -> 2 `@Valid` 분기 -> 3 `BindingResult` -> 4 `ProblemDetail`이고, 여기서 4단계는 "`validation `400`을 어떤 body shape로 번역할까`"를 정한 뒤 4.5단계에서 `Spring 예외 처리 기초`를 거쳐 다시 validation pipeline 내부(binding vs validation vs `BindingResult`)로 복귀하는 return step까지 포함한다. `415`는 1.5단계 보조 분기, `406 Not Acceptable`은 "body 형식은 읽혔는데 응답 형식 협상이 막힌다"는 1.6단계 handoff, `LocalDate/LocalTime` 형식 오류는 2.5단계 follow-up으로 둔다. 특히 "`4단계 ProblemDetail`", "`validation 400 ProblemDetail`", "`custom error dto vs ProblemDetail`", "`problemdetail 다음 validation pipeline`"처럼 primer 제목 축으로 검색해도 같은 번호 체계로 바로 내려오게 맞춘다.

## Spring MVC 바인딩/400 주 라우트 목록
...
- [1단계 `@RequestBody`가 컨트롤러 전에 `400` 나는 이유: JSON, 타입, `Content-Type` 첫 분리](./spring-requestbody-400-before-controller-primer.md)
- [1.5단계 보조 분기 `@RequestBody 415` 초급 primer](./spring-requestbody-415-unsupported-media-type-primer.md)
- [1.6단계 handoff `406 Not Acceptable`, `json 대신 html 와요`: `Accept`/`produces` 응답 협상은 Spring Content Negotiation 브리지로 넘기기](./spring-content-negotiation-pitfalls.md)
...
- [3.5단계 `MethodArgumentNotValidException` vs `HandlerMethodValidationException` 초급 브리지: `@Valid` request body와 method validation `400`를 한 표로 잇기](./spring-methodargumentnotvalidexception-vs-handlermethodvalidationexception-beginner-bridge.md)
- [4단계 `ProblemDetail` handoff primer: validation `400`, custom error DTO vs `ProblemDetail` 응답 바디 계약 정리](./spring-custom-error-dto-to-problemdetail-handoff-primer.md)
- [4.5단계 return path `ProblemDetail shaping` -> `Spring 예외 처리 기초` -> `Spring Validation and Binding Error Pipeline`: 전역 `400` 번역을 다시 잡고 곧바로 validation pipeline 내부(binding / validation / `BindingResult`)로 복귀](./spring-exception-handling-basics.md)
...
`BindingResult`는 DTO 생성 후 validation 실패에서만 갈라지므로, 4단계는 그다음 질문인 "`그 validation 400 응답 바디를 어떤 계약으로 보여 줄까?`"가 생겼을 때 들어가는 경계다.

## Spring MVC 바인딩/400 증상표

| symptom query | first hit | second hit |
|---|---|---|
| `requestbody 400 왜 바로 나요`, `컨트롤러 로그 안 찍힘`, `@RequestBody인데 컨트롤러 전에 400이 나요`, `JSON parse error가 보여요` | [`@RequestBody` 400 첫 분리 primer](./spring-requestbody-400-before-controller-primer.md) | [`@Valid` 400 첫 분기 primer](./spring-valid-400-vs-message-conversion-400-primer.md) |
| `JSON인데 415 Unsupported Media Type가 떠요`, `Content-Type: application/json 안 붙였는데 415예요`, `Content-Type 때문에 막힌 것 같아요` | [`1.5단계 보조 분기 @RequestBody 415` 초급 primer](./spring-requestbody-415-unsupported-media-type-primer.md) | [Spring Content Negotiation Pitfalls](./spring-content-negotiation-pitfalls.md) |
| `406이 왜 나와요`, `406 not acceptable`, `json 대신 html 와요` | [`1.6단계 handoff Spring Content Negotiation Pitfalls`](./spring-content-negotiation-pitfalls.md) | [Browser DevTools `Accept` vs Response `Content-Type` 미니 카드](../network/browser-devtools-accept-vs-content-type-mini-card.md) |
| `왜 @Valid 안 타요?`, `같은 400인데 뭐가 다른가요?`, `@NotBlank가 왜 안 먹어요?` | [`@Valid` 400 첫 분기 primer](./spring-valid-400-vs-message-conversion-400-primer.md) | [`BindingResult` 로컬 처리 카드](./spring-bindingresult-local-validation-400-primer.md) |
| `4단계 problemdetail이 뭐예요`, `validation 400 problemdetail`, `custom error dto vs problemdetail`, `에러 응답 body 뭐예요` | [`4단계 ProblemDetail` handoff primer](./spring-custom-error-dto-to-problemdetail-handoff-primer.md) | [Spring 예외 처리 기초](./spring-exception-handling-basics.md) |

## Spring Content Negotiation 브리지

> `406`, `accept/content-type 헷갈려요`, `json 대신 html`처럼 응답 형식과 헤더 계약이 섞이면 이 브리지에서 먼저 "지금 HTML이 negotiation 결과인가, 아니면 security login redirect HTML인가"를 가른 뒤 `Accept`, request `Content-Type`, response `Content-Type`, `produces`/`consumes`를 분리한다.
...
## Spring Content Negotiation 증상표

| symptom query | first hit | second hit |
|---|---|---|
| `406이 왜 나와요`, `json 대신 html 와요` | [Spring Content Negotiation Pitfalls](./spring-content-negotiation-pitfalls.md) | [Browser DevTools `Accept` vs Response `Content-Type` 미니 카드](../network/browser-devtools-accept-vs-content-type-mini-card.md) |
| `api인데 로그인 html 와요`, `fetch가 401 대신 login html 받아요`, `api인데 302 /login 보여요` | [Spring API는 `401` JSON인데 브라우저 페이지는 `302 /login`인 이유: 초급 브리지](./spring-api-401-vs-browser-302-beginner-bridge.md) | [Spring `SecurityFilterChain`을 둘로 나눠 `/admin/**`은 `302 /login`, `/api/**`는 `401` JSON으로 안전하게 다루는 입문 primer](./spring-securityfilterchain-multiple-entrypoints-primer.md) |
| `accept/content-type 헷갈려요`, `Accept랑 Content-Type이 왜 달라요?` | [Spring Content Negotiation Pitfalls](./spring-content-negotiation-pitfalls.md) | [HTTP 요청·응답 헤더 기초](../network/http-request-response-headers-basics.md) |
| `JSON인데 415 Unsupported Media Type가 떠요`, `Content-Type 때문에 막힌 것 같아요` | [`@RequestBody 415` 초급 primer](./spring-requestbody-415-unsupported-media-type-primer.md) | [Spring Content Negotiation Pitfalls](./spring-content-negotiation-pitfalls.md) |

- `response Content-Type: text/html`만 보고 곧바로 `406`으로 읽지 않는다. `Location: /login`, 최종 body의 login form, `fetch`가 redirect를 따라간 흔적이 먼저 보이면 negotiation보다 [Spring API는 `401` JSON인데 브라우저 페이지는 `302 /login`인 이유: 초급 브리지](./spring-api-401-vs-browser-302-beginner-bridge.md)로 들어가는 편이 안전하다.

## Spring MVC 바인딩/400 follow-up

> return path: `ProblemDetail`에서 응답 바디 계약과 shaping을 잡았으면 [Spring 예외 처리 기초](./spring-exception-handling-basics.md)로 전역 `400` 번역을 다시 묶고, 바로 [Spring Validation and Binding Error Pipeline](./spring-validation-binding-error-pipeline.md)으로 돌아가 "`이 `400`이 binding인지 validation인지, `BindingResult` 로컬 처리였는지`"를 validation pipeline 내부 단계 기준으로 다시 분해하면 된다.

| symptom query | first hit | second hit |
|---|---|---|
| "`BindingResult` 있으면 뭐가 달라져요?", "왜 `MethodArgumentNotValidException` 안 나요?", "`@Valid` 실패인데 컨트롤러 안으로 왜 들어와요?", "`BindingResult` 붙였는데 왜 어떤 `400`은 여전히 컨트롤러 전에 끝나요?" | [`BindingResult` 로컬 처리 카드](./spring-bindingresult-local-validation-400-primer.md) | [Spring 예외 처리 기초](./spring-exception-handling-basics.md) |
| "`2026/05/01` 보냈는데 `400`이 나요?", "`18시`, `오후 6시`, `6pm` 보내면 왜 안 되죠?", "`LocalDate`인데 `2026-05-01T18:00:00` 보내도 되나요?" | [`2.5단계 형식 오류 follow-up LocalDate`/`LocalTime` JSON `400` 형식 모음](./spring-localdate-localtime-json-400-cheatsheet.md) | [`@Valid` 400 첫 분기 primer](./spring-valid-400-vs-message-conversion-400-primer.md) |
| "`validation 400` 응답 바디를 어떤 계약으로 보여 줘야 해요?", "`팀 `ErrorResponse`로 충분한가요, `ProblemDetail`로 가야 하나요?`" | [`4단계 ProblemDetail` handoff primer](./spring-custom-error-dto-to-problemdetail-handoff-primer.md) | [Spring 예외 처리 기초](./spring-exception-handling-basics.md) |
| "`전역 400 번역은 알겠는데 어떤 validation 갈림길로 여기까지 왔는지 아직 헷갈려요`", "`problemdetail 다음에 bindingresult / message conversion 순서를 다시 보고 싶어요`" | [Spring 예외 처리 기초](./spring-exception-handling-basics.md) | [Spring Validation and Binding Error Pipeline](./spring-validation-binding-error-pipeline.md) |
