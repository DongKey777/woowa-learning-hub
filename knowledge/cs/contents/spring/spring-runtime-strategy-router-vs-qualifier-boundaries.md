...
retrieval-anchor-keywords: runtime strategy selection, router pattern, Map<String, Bean> router, Map<String, PaymentClient>, Map<String, NotificationSender>, strategy registry, strategy router, runtime dispatch vs qualifier, injection-time qualifier, qualifier boundary, collection injection router, payment router, notification router, bean map injection, bean name map key, custom qualifier vs router, @Qualifier vs runtime selection, per request strategy selection
