...
retrieval-anchor-keywords: spring component scan failure, component scan boundary, component scan 뭐예요, component scan 과 di 차이 헷갈려요, springbootapplication package, scanbasepackages, multi module component scan, nosuchbeandefinitionexception, service bean not found, controller bean missing, bean 안 떠요 왜, 처음 배우는데 bean 안 떠요, bean 못 찾는 이유 뭐예요, @springbootapplication package 왜 중요해요, webmvctest service bean not found
...
"`component scan`이 뭐예요?"처럼 개념 자체가 먼저 막히면 [Spring Bean과 DI 기초: Component Scan, Configuration, Proxy 감각 잡기](./spring-bean-di-basics.md)를 먼저 보고, "`bean`이 왜 안 떠요?", "`service bean not found`가 왜 나요?"처럼 **누락 증상**이 보일 때 이 문서로 내려오면 검색 첫 타격이 더 정확해진다.
