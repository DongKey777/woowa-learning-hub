...
초급자 검색 문장으로는 "`savedrequest 처음`", "`로그인 메모 뭐예요`", "`로그인 후 원래 페이지를 왜 기억해요`" 같은 질문에 대응한다.
검색에서 바로 들어와 "관리자 URL이 `302 /login`인데 이게 왜 `403`이랑 다른가요?"를 먼저 묻는 독자라면 deep dive보다 [Spring 관리자 요청이 `302 /login`이 될 때와 `403`이 될 때: 초급 브리지](./spring-admin-302-login-vs-403-beginner-bridge.md)로 먼저 내려보내는 편이 안전하다.
또 "로그인 후 원래 `/admin`으로 복귀했는데 왜 마지막엔 `403`인가요?"가 첫 질문이면 deep dive보다 [Spring 로그인 성공 후 원래 관리자 URL로 돌아왔는데도 마지막에 `403`이 나는 이유: `SavedRequest`와 역할 매핑 초급 primer](./spring-admin-login-success-but-final-403-savedrequest-role-mapping-primer.md)를 먼저 거쳐 `복귀`와 `최종 권한 실패`를 분리하는 편이 이탈을 줄인다.
...
advanced 문서로 바로 들어왔는데 "`RequestCache`가 뭐예요?", "`SavedRequest`가 세션이랑 뭐가 달라요?", "`왜 `/login` 갔다가 다시 원래 주소로 와요?`"가 먼저 떠오르면 아래 순서로 즉시 되돌아간다.

| 지금 막히는 질문 | 먼저 돌아갈 primer | 여기로 다시 복귀하는 조건 |
|---|---|---|
| `쿠키`, `세션`, `SecurityContext` 자체가 아직 흐리다 | [Spring 관리자 인증에서 쿠키와 세션이 어떻게 이어지는가: 초급 primer](./spring-admin-session-cookie-flow-primer.md) | "쿠키는 표식, 세션은 보관함, `SecurityContext`는 로그인 메모"가 한 문장으로 설명된다 |
| `302 /login`과 `403`이 왜 다른지 헷갈린다 | [Spring 관리자 요청이 `302 /login`이 될 때와 `403`이 될 때: 초급 브리지](./spring-admin-302-login-vs-403-beginner-bridge.md) | "`302`는 인증 전 이동, `403`은 인증 후 권한 실패"가 분리된다 |
| 로그인 후 원래 `/admin`으로 돌아왔는데 마지막 `403`이 섞여 보인다 | [Spring 로그인 성공 후 원래 관리자 URL로 돌아왔는데도 마지막에 `403`이 나는 이유: `SavedRequest`와 역할 매핑 초급 primer](./spring-admin-login-success-but-final-403-savedrequest-role-mapping-primer.md) | "`복귀`와 `인가 실패`를 따로 본다"가 고정된다 |

짧게 말하면 복귀 경로는 `쿠키/세션 primer -> SavedRequest/최종 403 primer -> 이 deep dive`다.
이 문서에서 길을 잃었을 때도 같은 순서를 반복하면 된다. 먼저 **로그인 상태 보관**을 분리하고, 그다음 **원래 URL 복귀**를 분리한 뒤, 마지막에만 `RequestCache` 구현/경계를 본다.
