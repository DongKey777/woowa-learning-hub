...
retrieval-anchor-keywords: modelattribute vs requestbody beginner, @modelattribute 뭐예요, @requestbody 뭐예요, modelattribute requestbody 헷갈려요, json body랑 query 차이 뭐예요, get post dto 차이 처음, spring 400 원인 헷갈려요, get은 되는데 post는 400 spring, 폼 바인딩 json 바인딩 차이, get admin reservations query vs post admin reservations json, roomescape admin binding primer, 컨트롤러 전에 415 unsupported media type, json인데 415 unsupported media type가 떠요, content-type: application/json 안 붙였는데 415예요, content-type 때문에 막힌 것 같아요
...
## `컨트롤러 전에 415 Unsupported Media Type` / `JSON인데 415 Unsupported Media Type가 떠요` 빠른 분기
...
- "`JSON인데 415 Unsupported Media Type가 떠요`처럼 `415` 증상으로 들어왔다"면 [Spring Content Negotiation Pitfalls](./spring-content-negotiation-pitfalls.md)에서 `Content-Type`과 `consumes` 축을 바로 확인한다.
