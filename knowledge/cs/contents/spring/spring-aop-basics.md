...
contextual_chunk_prefix: |
  이 문서는 Spring AOP beginner primer로, 로깅/트랜잭션/보안 같은 cross-cutting concern,
  aspect/advice/proxy, @Transactional과 AOP 관계, annotation이 안 먹는 증상을 설명한다.
  this.method(), private method, 직접 new Foo(), Bean + public + external call 같은 프록시 경로 문제를 먼저 분기한다.
---
...
## 먼저 `@Transactional`과의 관계만 짧게 잡는다

초급자에게 AOP는 독립된 큰 새 단원보다 "`왜 `@Transactional` 설명에 프록시가 끼어드는가`"를 풀어 주는 연결 문서에 가깝다.

| 지금 보인 말 | 이 문서에서의 초급자용 뜻 | 바로 다음 문서 |
|---|---|---|
| "`AOP`가 뭐예요?" | 공통 코드를 메서드 앞뒤에 붙이는 방식 | [`Spring @Transactional 기초`](./spring-transactional-basics.md) |
| "`프록시`가 왜 필요해요?" | 진짜 메서드 앞에서 규칙을 대신 적용하는 문지기 | [`Spring @Transactional 기초`](./spring-transactional-basics.md) |
| "`annotation이 안 먹어요`" | 프록시 정문을 우회했는지 보는 질문 | 이 문서의 `Bean + public + external call` 체크 |

짧게 외우면 "`@Transactional`은 규칙이고, AOP는 그 규칙을 붙이는 방법"이다.
...
비유는 "문지기" 정도까지만 쓰는 편이 안전하다. 실제로는 프록시가 메서드 호출 경계를 가로채는 것이지, 클래스 안 모든 줄마다 자동으로 마법을 거는 것은 아니다.
...
이 beginner primer에서는 Spring AOP를 먼저 **Spring Bean의 `public` 메서드 외부 호출** 기준으로 이해하는 편이 안전하다. 같은 클래스 내부 메서드 호출(self-invocation)에는 프록시가 끼어들지 못한다. 이것이 `@Transactional` 내부 호출 함정과 같은 출발점이다.
