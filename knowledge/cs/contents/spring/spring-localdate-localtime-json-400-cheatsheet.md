---
schema_version: 3
title: Spring `LocalDate`/`LocalTime` JSON 파싱 `400` 자주 나는 형식 모음
concept_id: spring/localdate-localtime-json-400-cheatsheet
canonical: false
category: spring
difficulty: beginner
doc_role: playbook
...
review_feedback_tags:
  - requestbody-date-time-parse-error
  - localdate-localtime-format-mismatch
  - validation-before-controller-confusion
aliases:
  - localdate 400 spring
  - localtime 400 spring
  - requestbody date parse error
  - requestbody time parse error
  - localdate json format beginner
  - localtime json format beginner
  - 2026/05/01 400 spring
  - 18시 400 spring
  - cannot deserialize localdate
  - cannot deserialize localtime
symptoms:
  - "`LocalDate` 필드에 `2026/05/01`이나 datetime 문자열을 보냈더니 컨트롤러 전에 `400`이 나요"
  - "`LocalTime` 필드에 `18시`, `오후 6시`, `6pm`을 넣으면 왜 거절되는지 모르겠어요"
  - "`@Valid`를 붙였는데 검증 메시지보다 message conversion 단계에서 먼저 실패해요"
intents:
  - symptom
  - troubleshooting
  - definition
...
forbidden_neighbors: []
expected_queries:
  - Spring `LocalDate` 필드에 `2026/05/01` 보내면 왜 `400`이 나요?
  - "`LocalTime`에 `18시`나 `6pm`을 보내면 왜 컨트롤러 전에 막히나요?"
  - date/time 형식 오류와 `@Valid` 검증 실패를 처음에 어떻게 구분해요?
  - "`LocalDate` 칸에 `2026-05-01T18:00:00`을 보내면 왜 안 되나요?"
contextual_chunk_prefix: |
  이 문서는 Spring 초급자가 `@RequestBody` DTO의 `LocalDate`/`LocalTime`
  필드에서 자주 만나는 JSON 형식 mismatch를 빠르게 분류하도록 돕는
  playbook이다. `2026/05/01`, `18시`, `6pm`,
  `2026-05-01T18:00:00`처럼 사람 눈에는 그럴듯하지만 타입이 기대한
  문자열 모양과 달라 컨트롤러 전에 `400`이 나는 장면을 다룬다.
---
...
이 문서는 Spring `@RequestBody` 초급 라우트에서 "`LocalDate`에 `2026/05/01`을 보내면 왜 `400`이 나요?", "`LocalTime`에 `18시`나 `6pm`을 보내면 왜 안 돼요?", "`@Valid` 전에 왜 끊겨요?"처럼 **문자열 모양과 DTO 타입 mismatch**를 먼저 확인해야 하는 질문에 바로 답하려는 cheatsheet다.
...
| "`LocalDate`인데 `2026-05-01T18:00:00`이나 `2026-05-01 18:00` 보내도 되나요?" | 날짜 칸에 datetime 전체 문자열을 넣은 건가? | 타입보다 더 긴 datetime 입력 |
...
| `LocalDate`에 datetime 문자열 전송 | 없음 | `"2026-05-01T18:00:00"`, `"2026-05-01 18:00"` | 날짜만 받는 칸에 시간까지 같이 들어왔다 |
| `LocalTime`에 date 문자열 전송 | 없음 | `"2026-05-01"` | 시간만 받는 칸에 날짜가 들어왔다 |

짧게 외우면 이렇다.

- `LocalDate` 기본값: `yyyy-MM-dd`
- `LocalTime` 기본값: 초급자 첫 입력은 `HH:mm`로 고정해 두는 편이 안전
- 날짜와 시간을 한 문자열로 보내고 싶으면 애초에 필드 타입이 다른지 같이 의심한다

여기서 중요한 caveat도 하나 있다.

- 이 문서의 `yyyy-MM-dd`, `HH:mm`는 "처음 맞춰 보기 가장 쉬운 안전 형식"이다.
- 프로젝트가 `@JsonFormat`, 커스텀 `ObjectMapper`, 별도 변환 규칙을 둔 경우에는 허용 형식이 더 넓어질 수 있다.
...
### 3. "`LocalDate`인데 `2026-05-01T18:00:00`이나 `2026-05-01 18:00` 보내면 더 자세해서 좋은 것 아닌가요?"
...
| `"2026-05-01 18:00"` | `LocalDate` | 자주 `400` |
...
- "`18:00:00`도 시간인데 왜 문서에는 `HH:mm`만 써 있지?"
  이 문서는 초급자용 안전 입력을 먼저 고정한 cheatsheet다. 프로젝트 기본 설정이나 `@JsonFormat`에 따라 seconds까지 허용될 수 있으니, `HH:mm`만 절대적으로 유일한 형식이라고 외우면 안 된다.
...
5. 그래도 아니면 그다음에 `@JsonFormat`, 커스텀 `ObjectMapper`, 팀 규칙처럼 허용 형식을 넓힌 설정이 있는지 본다.
...
`LocalDate`는 `yyyy-MM-dd`, `LocalTime`는 `HH:mm` 기준으로 먼저 맞추는 편이 가장 안전하다. 다만 그 형식만 "유일하게 허용"된다고 단정하지 말고, 이후에는 `@JsonFormat`이나 팀 직렬화 설정도 같이 확인해야 한다.
