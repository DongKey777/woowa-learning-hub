...
retrieval-anchor-keywords: strategy pattern basics, strategy pattern beginner, 전략 패턴 기초, 결제 전략, 할인 전략, if-else 대신 전략, caller chooses strategy, 호출자가 전략 선택, 처음 배우는데 전략 패턴, 전략 패턴 큰 그림, 전략 패턴 언제 쓰는지, 호출자가 구현을 고른다, 호출자가 구현을 넣어준다, 주입으로 구현 고르기, 부모가 흐름을 쥔다 vs 호출자가 전략을 고른다
