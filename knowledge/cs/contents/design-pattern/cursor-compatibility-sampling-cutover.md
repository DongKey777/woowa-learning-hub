---
schema_version: 3
title: Cursor Compatibility Sampling for Cutover
concept_id: design-pattern/cursor-compatibility-sampling-cutover
canonical: true
...
aliases:
- cursor compatibility sampling
- projection cutover pagination canary
- legacy cursor reuse canary
- legacy cursor reuse allowlist
- cursor version canary matrix
- requested emitted cursor version
- cursor reissue canary sampling
- two page continuity sampling
- cursor cutover sample floor
- legacy cursor acceptance drift
symptoms:
- fresh query sample만 보고 legacy cursor reuse traffic이 실제로 안전한지 검증하지 않는다
- REISSUE와 REJECT를 같은 restart bucket으로 뭉개 requested/emitted cursor version matrix drift를 놓친다
- ACCEPT bucket에서 page1은 맞지만 page2 duplicate/gap이 발생하는데 canary가 low-rate mismatch로만 집계한다
intents:
- troubleshooting
- design
- deep_dive
...
forbidden_neighbors: []
expected_queries:
- Cursor compatibility sampling은 cutover canary에서 legacy cursor reuse와 requested/emitted version을 어떻게 증명해?
- Fresh query sample이 많아도 legacy cursor compatibility가 증명되지 않는 이유가 뭐야?
- ACCEPT allowlist bucket은 page1뿐 아니라 page2 continuity PASS까지 있어야 green인 이유가 뭐야?
- REISSUE bucket은 requested cursor version과 emitted cursor version matrix가 고정됐는지 봐야 하는 이유가 뭐야?
- allowlist 밖 ACCEPT나 ACCEPT인데 DUPLICATE_OR_GAP이 나오는 경우 pagination canary를 즉시 freeze해야 하는 이유가 뭐야?
contextual_chunk_prefix: |
  이 문서는 Cursor Compatibility Sampling for Cutover playbook으로, projection cutover canary에서
  fingerprint bucket, requested cursor version, emitted cursor version, ACCEPT/REISSUE/REJECT verdict,
  legacy reuse allowlist, page1->page2 continuity sample floor를 따로 수집해 restart-only 전환과
  진짜 compatibility를 구분하는 방법을 설명한다.
---
...
샘플링의 핵심은 "얼마나 많이 봤는가"가 아니라 "어느 cursor family와 page-size family를 실제로 page chain까지 따라갔는가"다.
