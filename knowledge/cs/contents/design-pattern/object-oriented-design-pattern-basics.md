...
이 문서는 `"디자인 패턴 처음 뭐부터 봐요?"`, `"전략/템플릿 메소드/팩토리/빌더가 왜 다른가요?"`, `"왜 상속보다 조합부터 보라고 해요?"` 같은 첫 질문이 들어왔을 때 가장 먼저 연결되는 entrypoint primer다.

즉 검색어가 `"디자인 패턴 뭐예요"`, `"처음 디자인 패턴 배우는데 큰 그림"`, `"패턴 이름이 너무 많아요"`에 가까우면 이 문서가 첫 착지점이어야 한다.
...
처음에는 패턴 이름보다 "지금 바꾸려는 것"을 먼저 적는 편이 안전하다.

| 지금 바뀌는 것 | 먼저 떠올릴 패턴 | 한 줄 이유 | 바로 버려도 되는 경우 |
|---|---|---|---|
| 할인/결제/정렬처럼 규칙 자체 | `전략` | 같은 목적의 실행 방법을 갈아끼운다 | 규칙이 거의 안 바뀌고 분기도 1~2개뿐일 때 |
| 읽기 -> 검증 -> 저장처럼 순서 뼈대 | `템플릿 메소드` | 부모가 전체 순서를 고정한다 | 런타임 교체가 더 중요하고 상속이 부담일 때 |
| 어떤 구현을 만들지, 생성 책임을 어디 둘지 | `팩토리` | `new`와 생성 규칙을 호출부에서 숨긴다 | 생성 규칙이 단순하고 한 곳에서만 쓸 때 |
| 한 객체의 필드/옵션 조합 | `빌더` | 필수값과 선택값을 읽기 좋게 조립한다 | 필드 수가 적고 생성자가 더 단순할 때 |
| 상태 변화 뒤에 붙는 후속 반응 여러 개 | `옵저버` | 알림 발행자와 반응자를 느슨하게 연결한다 | 핵심 성공 경로를 반드시 함께 처리해야 할 때 |

짧게 외우면 이렇다.

- `무엇을 할지 바꾼다` -> 전략
- `어떤 순서로 할지는 고정한다` -> 템플릿 메소드
- `무엇을 만들지 숨긴다` -> 팩토리
- `한 객체를 읽기 좋게 채운다` -> 빌더
- `무언가 바뀌었음을 알린다` -> 옵저버
...
- "`if-else`가 있으면 무조건 전략으로 바꿔야 한다" -> 아니다. 분기가 작고 잘 안 바뀌면 그대로 두는 편이 더 읽기 쉽다.
- "추상 클래스가 보이면 템플릿 메소드다" -> 아니다. 부모가 `전체 순서`를 쥐는지가 먼저다. 추상 메서드만 모아 둔 경우는 전략/일반 추상화일 수 있다.
- "팩토리는 `new`를 전부 없애는 패턴이다" -> 아니다. 생성 규칙이 복잡하거나 구현 선택이 있을 때만 가치가 크다.
- "빌더는 setter 체이닝이면 다 된다" -> 아니다. mutable 객체를 계속 바꾸는 fluent API와, 마지막에 완성 객체를 만드는 builder는 의도가 다르다.
- "옵저버는 비동기 이벤트 시스템이다" -> 보통 아니다. 기본 옵저버는 같은 프로세스 안의 알림 구조다. 브로커, 재시도, 적재가 나오면 Pub/Sub 질문으로 넘어간다.

비유를 하나만 쓰면, 다섯 패턴은 "도구 상자"처럼 볼 수 있다. 다만 모든 망치를 못에 쓰는 것처럼 패턴 이름에 문제를 끼워 맞추면 오히려 설계가 무거워진다. 먼저 문제 축을 고르고, 그다음 패턴 이름을 붙이는 순서가 안전하다.

---

## 실무에서 쓰는 모습

하나의 주문 흐름으로 묶어 보면 다섯 패턴의 자리가 분리된다.

| 주문 흐름 장면 | 잘 맞는 패턴 | 이유 |
|---|---|---|
| 회원 등급에 따라 할인 계산이 달라진다 | `전략` | 할인 규칙을 갈아끼우는 문제다 |
| `validate -> pay -> save -> notify` 순서는 항상 지켜야 한다 | `템플릿 메소드` | 공통 순서를 부모가 고정하기 좋다 |
| PG사별 `PaymentClient` 생성 규칙이 다르다 | `팩토리` | 어떤 구현을 만들지 호출부에서 숨긴다 |
| `OrderRequest`에 옵션이 많아 생성자가 읽기 어렵다 | `빌더` | 필수값/선택값을 이름과 함께 조립한다 |
| 주문 완료 후 이메일, Slack, 로그가 따라붙는다 | `옵저버` | 후속 반응을 느슨하게 추가할 수 있다 |

중요한 점은 한 기능에 패턴이 하나만 쓰이는 것은 아니라는 점이다. 보통은 생성, 실행 규칙, 후속 반응이 다른 축이기 때문에 한 흐름 안에서 같이 등장해도 이상하지 않다.

---

## 더 깊이 가려면

- 전략 판단을 더 좁히고 싶으면 [전략 패턴 기초](./strategy-pattern-basics.md)와 [템플릿 메소드 vs 전략](./template-method-vs-strategy.md)
- 템플릿 메소드의 `hook method`, `abstract step`이 헷갈리면 [템플릿 메소드 패턴 기초](./template-method-basics.md)
- 생성 책임과 조립 책임을 더 또렷하게 가르려면 [팩토리 패턴 기초](./factory-basics.md)와 [빌더 패턴 기초](./builder-pattern-basics.md#builder-quick-entry)
- 알림, direct call, Pub/Sub 경계를 자르고 싶으면 [옵저버 패턴 기초](./observer-basics.md)와 [옵저버, Pub/Sub, ApplicationEvent](./observer-pubsub-application-events.md)
- 패턴 이름보다 상속/조합 감각이 더 필요하면 [상속보다 조합 기초](./composition-over-inheritance-basics.md)와 [OOP 설계 원칙 기초](../software-engineering/oop-design-basics.md)

---

## 면접/시니어 질문 미리보기

> Q: 전략과 템플릿 메소드는 둘 다 중복을 줄이는데, 언제 경계가 갈리나?
> 의도: "교체 규칙"과 "고정 흐름"을 분리해서 설명하는지 본다.
> 핵심: 런타임 교체가 중요하면 전략, 부모가 순서를 강제해야 하면 템플릿 메소드다.

> Q: 팩토리와 빌더는 둘 다 생성 패턴인데 왜 나눠 부르나?
> 의도: 구현 선택 문제와 객체 조립 문제를 구분하는지 본다.
> 핵심: 팩토리는 "무엇을 만들지", 빌더는 "하나를 어떻게 읽기 좋게 만들지"에 초점이 있다.

> Q: 옵저버를 직접 호출 대신 쓰면 항상 더 좋은가?
> 의도: 느슨한 결합의 이점과 추적 어려움의 비용을 함께 보는지 확인한다.
> 핵심: 후속 반응 분리에는 좋지만, 핵심 정합성이나 실패 전파가 중요한 경로는 direct call이 더 안전할 수 있다.
