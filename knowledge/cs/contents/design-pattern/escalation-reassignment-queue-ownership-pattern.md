---
schema_version: 3
title: Escalation, Reassignment, and Queue Ownership Pattern
concept_id: design-pattern/escalation-reassignment-queue-ownership-pattern
canonical: true
...
- manual-review-operator-contract
aliases:
...
- reassignment policy
symptoms:
- manual review에서 queue owner, reviewer, approver 권한을 섞어 담당자 변경이 승인 결정처럼 해석된다
- 개인에게 배정한 review item을 lease가 아니라 영구 소유처럼 다뤄 부재, 교대, overload 때 방치된다
- SLA 위반 시 queue escalation, decision escalation, override authorization을 한 이벤트로 뭉개 audit reason이 사라진다
intents:
- troubleshooting
- design
- deep_dive
...
forbidden_neighbors: []
expected_queries:
- manual review에서 queue owner와 approver 권한을 분리해야 하는 이유가 뭐야?
- review assignment를 영구 배정이 아니라 lease claim으로 모델링하면 어떤 장애를 줄일 수 있어?
- SLA breach가 났을 때 queue escalation과 decision escalation, override authorization은 어떻게 달라?
- reviewer 부재나 shift handoff를 ad hoc 운영이 아니라 reassignment control loop로 설계하려면 무엇을 남겨야 해?
- UNDER_REVIEW 상태와 queue metadata를 분리해야 audit와 business truth가 덜 섞이는 이유가 뭐야?
contextual_chunk_prefix: |
  이 문서는 Escalation, Reassignment, and Queue Ownership Pattern playbook으로,
  manual review workflow에서 workflow owner, queue owner, reviewer, approver의 권한을 분리하고
  review item을 lease claim으로 다루며 reassignment, SLA breach, queue escalation,
  decision escalation, override audit를 operator contract로 모델링하는 방법을 설명한다.
---
...
특히 workflow state는 business truth를, queue metadata는 사람과 운영 활동을 표현한다. 둘을 나누면 "검토 중"이라는 도메인 상태를 유지하면서도 누가 점유했고 왜 재할당됐는지 설명할 수 있다.
