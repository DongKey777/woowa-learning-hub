...
> 학습 순서 라벨: `basics -> framework examples -> deep dive` (`deep dive` 단계)

관련 문서:

- [템플릿 메소드 패턴 기초](./template-method-basics.md) - 처음 배우는데는 **"부모가 흐름을 쥔다"**, **"필수 빈칸 vs 선택 빈칸"**, **"언제 쓰는지"**부터 먼저 잡는 primer
- [Java 상속과 오버라이딩 기초](../language/java/java-inheritance-overriding-basics.md) - `extends`와 override 감각이 흐릴 때 먼저 돌아가는 기초 링크
...
처음에는 "고정 순서 + 빈칸 채우기"로 보면 된다.

```text
부모: validate -> convert -> write -> afterExport
자식:           (필수)      (필요하면 선택)
```
...
|---|---|
| 누가 순서를 고정하나 | 부모 클래스 |
| 자식이 반드시 채워야 하나 | `abstract step`만 반드시 |
| 자식이 선택적으로 덧붙이나 | `hook method`에서만 |

즉 이 패턴의 중심은 "상속 재사용"보다 **순서 통제**다.

### 30초 선택표: 템플릿 메소드 vs 전략

| 질문 | 템플릿 메소드 | 전략 |
|---|---|---|
| 누가 실행 순서를 쥐나 | 부모 클래스가 고정한다 | 호출자/컨텍스트가 고른다 |
| 교체 시점 | 상속 구조를 바꿀 때(컴파일 타임 중심) | 요청별/런타임 교체 가능 |
| 초보자 한 줄 기준 | "순서 보호"가 핵심일 때 | "규칙 교체"가 핵심일 때 |

처음 읽을 때는 아래 세 질문만 먼저 판정해도 충분하다.

1. 부모 클래스가 반드시 순서를 고정해야 하는가?
2. 필수 단계(`abstract step`)와 선택 확장(`hook`)을 분리할 수 있는가?
3. 요청마다 구현 교체가 필요하다면 전략으로 넘길 수 있는가?

---

## 핵심 개념

`hook method`와 `abstract step`을 처음 배우는 중이면 먼저 [템플릿 메소드 패턴 기초](./template-method-basics.md)에서 **"부모가 흐름을 쥔다"**, **"필수 빈칸 vs 선택 빈칸"**, **"언제 쓰는지"**부터 먼저 잡고 온다.
이 문서는 그다음에 왜 `final`과 순서 통제가 중요한지, 상속을 어디까지 허용할지까지 깊게 본다.
...
- **"추상 클래스면 다 템플릿 메소드인가요?"**: 아니다. 부모가 전체 순서를 실제로 고정할 때만 해당한다.
- **"`hook`을 많이 열수록 유연한가요?"**: 유연해 보이지만, 대부분은 상위 클래스 계약을 흐리게 만든다.
- **"`final`이 너무 강한 제약 아닌가요?"**: 템플릿 메소드는 순서 보호가 목적이라 템플릿 메서드 자체를 고정하는 편이 안전하다.
- **"런타임 정책 교체도 템플릿으로 되지 않나요?"**: 가능은 해도 클래스 폭발이 쉽게 온다. 그 축은 전략 패턴이 더 자연스럽다.
- **"프레임워크 콜백이면 다 템플릿 메소드인가요?"**: 아니다. 책임 연쇄/인터셉터처럼 체인 참가자 모델도 있으니, "부모가 순서를 고정했는지"부터 먼저 확인한다.
...
- `hook method`/`abstract step`이 아직 낯설면: [템플릿 메소드 패턴 기초](./template-method-basics.md)
- 상속 vs 객체 주입 선택이 헷갈리면: [템플릿 메소드 vs 전략](./template-method-vs-strategy.md)
- 프레임워크 API에서 어디가 템플릿인지 찾고 싶으면: [프레임워크 안의 템플릿 메소드](./template-method-framework-lifecycle-examples.md)
- hook가 과하게 늘어나는 신호를 점검하려면: [Template Hook Smells](./template-hook-smells.md)
