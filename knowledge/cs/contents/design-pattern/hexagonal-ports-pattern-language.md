---
schema_version: 3
title: Hexagonal Ports Pattern Language
concept_id: design-pattern/hexagonal-ports-pattern-language
canonical: true
...
- 포트와 어댑터
symptoms:
- Controller, repository, external client를 모두 adapter라고 부르지만 inbound와 outbound 책임이 구분되지 않는다
- port 이름이 기술명으로 잡혀 유스케이스 의도보다 프레임워크 세부가 먼저 드러난다
- GoF Adapter의 interface translation과 architecture-level port boundary를 같은 층위로 설명한다
intents:
- deep_dive
- design
- comparison
...
forbidden_neighbors: []
expected_queries:
- Hexagonal architecture에서 inbound port와 outbound port는 호출 방향과 책임이 어떻게 달라?
- ports and adapters에서 adapter라는 말이 GoF Adapter pattern과 왜 다른 층위야?
- 컨트롤러와 메시지 컨슈머가 같은 use case port를 호출하게 만들면 어떤 장점이 있어?
- PaymentPort 같은 outbound port 이름을 기술명보다 유스케이스 의도로 잡아야 하는 이유가 뭐야?
- hexagonal boundary를 Spring controller service repository 구조에 적용할 때 어디까지 분리해야 해?
contextual_chunk_prefix: |
  이 문서는 Hexagonal Ports deep dive로, inbound port, outbound port, driving adapter,
  driven adapter를 use case boundary와 dependency inversion 관점에서 구분하고
  GoF Adapter의 object-level interface translation과 혼동하지 않도록 설명한다.
---
...
### 4. port 이름은 프레임워크보다 요구를 말해야 한다

나쁜 port 이름은 기술을 먼저 드러낸다.

- `JpaOrderRepository`
- `RestPaymentClient`
- `KafkaEventPublisher`

이 이름들은 구현체에는 괜찮지만, 애플리케이션 core가 기대하는 계약 이름으로는 약하다. port에는 다음처럼 "무엇을 요구하는가"가 드러나는 편이 낫다.

- `OrderStore`
- `PaymentRequester`
- `OrderEventPublisher`

이 차이가 작아 보여도 테스트와 교체 시점에 크게 드러난다. 유스케이스가 `RestPaymentClient`를 알면 이미 REST라는 기술 선택이 core 안으로 들어온 것이다.
