...
|---|---|---|
| "예약 상태 enum은 있는데 service switch가 계속 길어져요" | `CONFIRMED`, `CANCELED`, `FINISHED`별 분기가 여러 메서드에 반복되는 코드 | 상태 이름보다 현재 상태가 허용하는 행동을 객체가 설명하게 한다 |
| "취소된 예약도 변경 메서드를 타는 걸 어디서 막아야 하죠?" | controller, service, repository가 각각 상태 전이 조건을 조금씩 아는 구조 | 취소/변경 가능 여부와 실행을 같은 전이 규칙으로 묶는다 |
| "Strategy로 검증기를 나눴는데 상태 혼란은 그대로예요" | 예약 검증 알고리즘과 예약 lifecycle 전이가 같은 문제처럼 다뤄지는 설계 | Strategy는 행동 선택, State는 현재 단계의 허용 행동 질문으로 분리한다 |
