...
로드맵 4단계에서 처음 디자인 패턴을 공부하는 분이라면 아래 5편을 먼저 읽는다.

- [싱글톤 패턴 기초](singleton-basics.md) — 인스턴스 하나 공유의 이점과 함정, Spring DI와의 차이
- [전략 패턴 기초](strategy-pattern-basics.md) — if-else 대신 구현을 교체하는 구조, Context·Strategy 역할 분리
...
- [상속보다 조합 기초](composition-over-inheritance-basics.md) — is-a vs has-a, 조합으로 유연성 얻기
...
- [템플릿 메소드 패턴 기초](template-method-basics.md) — 알고리즘 뼈대 고정, 추상 단계 위임, 전략 패턴과 차이
...
- [전략 (Strategy)](strategy-pattern.md) - runtime에 구현을 고르는 주체가 호출자/설정인지, `Context`는 실행만 맡는지, 작은 함수·단순 분기·전략 폭발 경계를 어디서 그을지 정리한 primer
...
- [템플릿 메소드 vs 전략](template-method-vs-strategy.md) - runtime 교체 vs fixed flow chooser, abstract step / hook method / strategy 경계를 beginner-first wording으로 정리
...
- [템플릿 메소드 (Template Method)](template-method.md) - beginner-first deep dive, abstract step vs hook method, 언제 쓰지 말아야 하는지까지 포함
...
- [프레임워크 안의 템플릿 메소드: Servlet, Filter, Test Lifecycle](template-method-framework-lifecycle-examples.md) - basics를 다시 풀지 않고 `HttpServlet`, `OncePerRequestFilter`, classic xUnit fixture에서 skeleton이 어디 있는지 짚고, safe hook boundary를 smell 문서로 이어 주는 companion 문서
