...
- 고전 패턴 primer부터 읽고 싶다면:
  - `객체지향 디자인 패턴 기초`
  - `상속보다 조합 기초` -> `템플릿 메소드 vs 전략` -> `템플릿 메소드`  ← "상속으로 뼈대를 잡을까, 객체를 주입할까?"를 가장 빨리 자르는 beginner route
  - `전략`
...
- [템플릿 메소드 패턴 기초](template-method-basics.md) — 알고리즘 뼈대 고정, 상속이 허용되는 조건, 전략/조합과의 첫 분기
...
- [템플릿 메소드 vs 전략](template-method-vs-strategy.md) - runtime 교체 vs fixed flow chooser, 상속 skeleton vs 조합 strategy를 beginner-first wording으로 정리
...
- [템플릿 메소드 (Template Method)](template-method.md) - beginner-first deep dive, abstract step vs hook method, 상속을 써도 되는 좁은 경우와 언제 쓰지 말아야 하는지 포함
