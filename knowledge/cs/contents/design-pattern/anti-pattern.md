...
### 디자인 패턴 과사용
디자인 패턴 자체는 문제를 구조적으로 해결하는 도구지만, **패턴을 적용하는 것 자체가 목적이 되면** 안티 패턴이 된다. 예를 들어 아주 단순한 분기 로직에 템플릿 메소드와 전략 패턴을 동시에 도입하거나, 외부 요청이 한 단계만 통과하는데도 프록시와 퍼사드를 함께 얹는 경우가 있다. 이런 설계는 "확장 가능해 보이는 코드"를 만들지만 실제로는 이해 비용만 높인다.

#### 디자인 패턴 과사용의 신호
1. 패턴 이름은 멋진데 문제 설명이 더 복잡해진다.
2. 실제 변화 지점보다 추상화 계층이 더 많다.
3. 새 기능이 추가되기 전에 먼저 패턴 클래스를 만드는 습관이 있다.
4. 팀원이 패턴 구조를 이해하기보다 클래스 관계를 외우는 데 시간을 쓴다.

#### 디자인 패턴 과사용의 원인
1. "좋은 설계"를 패턴 개수로 착각하는 경우
2. 면접이나 발표를 의식해 과한 추상화를 넣는 경우
3. 재사용 가능성을 검증하기 전에 일반화부터 하는 경우

#### 디자인 패턴 과사용을 피하는 기준
1. 바뀌는 지점이 한 군데인지 먼저 본다.
2. 분기문이나 작은 함수로 충분하면 먼저 단순하게 둔다.
3. 패턴은 "지금의 복잡도"를 줄이는가, "미래의 가정"만 늘리는가를 따진다.
4. 추상화가 코드 읽기 비용보다 실제 유지보수 비용을 얼마나 줄이는지 확인한다.
