---
schema_version: 3
title: Cursor Pagination and Sort Stability Pattern
concept_id: design-pattern/cursor-pagination-sort-stability-pattern
canonical: true
...
- mutable sort key drift
- cutover cursor invalidation
- pagination parity sampling
- two page continuity check
symptoms:
- createdAt DESC만으로 cursor pagination을 만들고 같은 timestamp row에서 page 중복이나 누락이 발생한다
- cursor를 last row position으로만 보고 normalized query, sort direction, tie-breaker, cursor version과 묶지 않는다
- read model cutover에서 first page만 비교하고 next cursor와 second page continuity를 검증하지 않는다
intents:
- troubleshooting
- design
- deep_dive
...
forbidden_neighbors: []
expected_queries:
- Cursor pagination에서 stable ordering과 tie breaker가 없으면 page 중복이나 누락이 생기는 이유가 뭐야?
- cursor는 lastSeenId만이 아니라 normalized query, sort direction, page size, cursor schema version과 묶여야 하는 이유가 뭐야?
- createdAt DESC 같은 정렬에는 id DESC 같은 unique tie breaker를 붙여야 하는 이유가 뭐야?
- mutable sort key인 updatedAt이나 score로 cursor pagination을 할 때 snapshot cutoff나 requery policy가 필요한 이유가 뭐야?
- read model cutover에서 page1만 비교하지 말고 next cursor와 page2 continuity까지 검증해야 하는 이유가 뭐야?
contextual_chunk_prefix: |
  이 문서는 Cursor Pagination and Sort Stability Pattern playbook으로, cursor pagination을
  안정적으로 만들기 위해 normalized query contract, stable sort family, unique tie-breaker,
  cursor schema version, mutable sort drift guard, page1->page2 continuity check를 함께
  설계하는 방법을 설명한다.
---
...
즉 cursor pagination의 안전성은 token encoding보다 정렬 계약의 안정성에서 나온다. token을 잘 숨겨도 정렬 세계가 흔들리면 page boundary는 흔들린다.
