---
schema_version: 3
title: Spring EventListener vs TransactionalEventListener Timing
concept_id: design-pattern/spring-eventlistener-vs-transactionaleventlistener-timing
canonical: true
...
- spring-eventlistener-timing
- transactional-event-listener
- transaction-phase-boundary
aliases:
- spring eventlistener vs transactionaleventlistener
- before commit after commit after rollback
- spring listener ordering
- rollback semantics
- fallbackexecution
- application event transaction phase
- after commit requires new transaction
- event listener commit boundary
- spring eventlistener timing
- transactional event listener phase
symptoms:
- "@Order" 값만 보고 EventListener와 TransactionalEventListener 전체 실행 순서가 정해진다고 오해한다
- AFTER_COMMIT listener가 실패하면 이미 commit된 주문까지 되돌릴 수 있다고 착각한다
- "@EventListener"에서 외부 side effect를 실행한 뒤 이후 검증 실패로 DB만 rollback되어 외부 세계와 불일치가 생긴다
intents:
- comparison
- troubleshooting
- design
...
- spring/transactionaleventlistener-fallbackexecution-no-transaction-boundaries
...
forbidden_neighbors: []
expected_queries:
- Spring @EventListener와 @TransactionalEventListener는 이벤트 수신보다 timing과 rollback contract가 어떻게 달라?
- AFTER_COMMIT listener는 왜 원 transaction을 되돌릴 수 없고 retry나 outbox가 필요한가?
- @Order는 phase가 같은 listener 안에서만 의미가 강하고 BEFORE_COMMIT과 AFTER_COMMIT 전체 순서를 뒤집지 못하는 이유가 뭐야?
- @EventListener에서 외부 webhook이나 email을 보내면 DB rollback보다 side effect가 먼저 나갈 수 있는 이유가 뭐야?
- fallbackExecution=true는 no transaction 경로에서 after commit 의미를 유지하지 못하고 즉시 실행으로 내려오는 이유가 뭐야?
contextual_chunk_prefix: |
  이 문서는 Spring EventListener vs TransactionalEventListener Timing chooser로,
  @EventListener의 same-call dispatch와 @TransactionalEventListener의 transaction phase
  지연 실행을 BEFORE_COMMIT, AFTER_COMMIT, AFTER_ROLLBACK, fallbackExecution, @Order,
  rollback side effect 기준으로 구분한다.
---
...
이 셋이 정해지지 않은 상태에서 annotation만 고르면, listener가 실행은 됐지만 DB는 rollback되거나, commit 후 listener 실패를 원 transaction 실패처럼 착각하는 문제가 생긴다.
