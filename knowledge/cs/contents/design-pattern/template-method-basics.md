...
retrieval-anchor-keywords: template method basics, 템플릿 메소드 패턴 기초, 템플릿 메소드 패턴이 뭔가요, template method beginner, abstract class template, 추상 메서드 패턴 입문, hook method beginner, 뼈대 알고리즘 패턴, 상속 기반 흐름 고정, 공통 흐름 재사용, beginner template method, 템플릿 메소드 예시
