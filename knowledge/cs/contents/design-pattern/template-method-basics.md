...
- 부모 클래스가 공통 순서를 끝까지 고정해야 한다: 템플릿 메소드
- 부모 클래스보다 객체 주입이 더 자연스럽다: 전략 + 조합
...
- **"전략 패턴과 같다"** — 템플릿 메소드는 상속으로 흐름을 재사용하고, 전략 패턴은 인터페이스 구현체를 주입해 행동을 교체한다. 흐름이 고정인지 교체 가능한지가 핵심 차이다.
