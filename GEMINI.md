...
- raw prompt의 override/tool-only/coach 판단은 유지하되, domain/depth/definition/study-intent 감지는 reformulated query도 함께 본다. 라우터는 `signal_rules.py`와 corpus frontmatter의 corpus-owned vocabulary도 domain bridge로 사용하므로 수동 lexicon에 없는 세부 용어도 corpus vocabulary가 있으면 Tier 1+로 올라갈 수 있다.
...
- `response_hints.must_offer_next_action`이 있으면 답변 끝에 자연스럽게 surface (예: "다음 턴에 DI drill 한 번 풀어볼까?"). 이 값은 현재 질문과 추천 concept이 겹칠 때만 채워진다.
