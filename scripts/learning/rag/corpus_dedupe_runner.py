...
Loads the production index (``state/cs_rag/index.sqlite3`` +
``dense.npz``) once, runs ``dedupe.find_near_duplicate_pairs``, and
returns a list of ``LintViolation`` entries the lint module can mix
with its other checks.

Lazy-imports numpy + sqlite3 so the lint module stays importable on
environments without the dense index (CI, fresh clone before
First-Run Protocol).
"""
...
def load_index_paths_and_embeddings(index_root: Path) -> tuple[list[str], object]:
    """Load (paths_per_chunk, embeddings_matrix) from a production index.
