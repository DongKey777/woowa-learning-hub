...
>
> 1. 이 저장소를 자기 노트북 원하는 위치에 클론 (예: `~/IdeaProjects/`)
>    ```bash
>    git clone https://github.com/DongKey777/woowa-learning-hub.git
>    cd woowa-learning-hub
>    ```
> 2. 저장소 루트에서 AI 세션 한 번 열기 (`claude` / `codex` / `gemini` 중 하나)
> 3. 한국어로 의도 한 줄: *"이 저장소로 학습 시작하자. spring-core-1부터 가고 싶어."*
> 4. AI가 자동 처리: 의존성 설치 → HuggingFace 모델 warm-up → CS 인덱스 빌드
>    → **`missions/` 안에 학습 테스트/미션 저장소 자동 클론** → 첫 학습 가이드
> 5. 학습자는 답변 따라 한국어로 학습 진행
>
> AI 세션이 시작 파일(`CLAUDE.md` / `AGENTS.md` / `GEMINI.md`)의 First-Run Protocol을
> 따라 순서대로 셋업한다. 학습자가 직접 `bin/...`이나 `git clone`을 외울 일은 없다.

---

## 학습자 노트북 배치 — 어떤 저장소를 어디에 받는가

학습자가 자기 노트북에 다루는 저장소는 세 종류. **이 저장소(woowa-learning-hub)만 직접 클론**
하고, 나머지(학습 테스트 / 미션 저장소)는 AI가 첫 세션에서 자동으로 `missions/` 아래에 받아온다.

```
woowa-learning-hub/                    ← (1) 이 저장소. 학습자가 직접 클론
├─ bin/, scripts/, knowledge/, docs/   ← AI 코칭 워크벤치 도구
├─ state/                              ← AI가 자동 생성 (gitignored, 노트북에만 존재)
│   ├─ learner/                          학습 이벤트 stream + profile
│   ├─ repos/<repo>/                     PR 아카이브 / 코칭 컨텍스트
│   └─ cs_rag/                           CS RAG 인덱스 (87초, 첫 세션 자동 빌드)
└─ missions/                           ← AI가 자동 클론 (gitignored, 노트북에만)
   ├─ spring-learning-test/              (2) 학습 테스트 — Spring 개념 검증용
   ├─ spring-roomescape-admin/           (3) 학습자 자기 미션 fork — PR 보낼 곳
   └─ java-janggi/                       동시에 여러 미션 진행 가능
```
...
| 종류 | 무엇 | 어디에 | 누가 클론 |
|---|---|---|---|
| **(1) woowa-learning-hub** | AI 코칭 워크벤치 | 자기 노트북 원하는 곳 | 학습자가 직접 (1회) |
| **(2) spring-learning-test** | 우테코 학습 테스트 (`woowacourse/spring-learning-test`) | `missions/spring-learning-test/` | AI 자동 |
| **(3) 미션 저장소** (java-janggi, spring-roomescape-admin 등) | **학습자의 자기 fork** | `missions/<repo-name>/` | AI 자동 |

### Fork / Upstream 관계 (PR 코칭 흐름)

자기 미션 저장소는 **자기 fork를 클론**해야 한다 — 학습자가 PR을 자기 fork에 push해서
upstream `woowacourse/<repo>`로 보낼 거니까. AI는 학습자가 한국어로 알려주는 것만으로 자동
처리:

> 학습자: *"내 미션 저장소를 코칭해줘. 저장소는 `https://github.com/내계정/java-janggi`이고,
> upstream은 `woowacourse/java-janggi`, 사이클2를 진행 중이야."*
>
> AI 자동: 자기 fork를 `missions/java-janggi/`에 클론 → upstream을
> `woowacourse/java-janggi`로 등록 → peer PR 수집 시 upstream 기준으로 가져옴

### 이미 다른 곳에 미션 저장소가 있다면

학습자가 이미 자기 노트북 다른 위치에 미션 저장소를 클론해뒀어도 OK — 한국어로 위치만
알려주면 AI가 그 경로로 등록.

> 학습자: *"내 미션 저장소는 `~/code/java-janggi`에 이미 있어."*
>
> AI 자동: `bin/onboard-repo --path ~/code/java-janggi --upstream woowacourse/java-janggi ...`

`missions/` 안에 있는 게 권장 (gitignore 자동 적용 + 다른 미션과 한곳 모음). 그러나 이미
배치된 곳이 있으면 그대로 사용 가능.

### 하나의 저장소에서 여러 미션 동시 진행

`missions/` 아래에 여러 미션 저장소를 동시에 둘 수 있고, 각자 별도 학습 데이터로 추적됨.
학습자 프로필(`state/learner/profile.json`)은 미션 경계를 넘어 cross-mission 패턴까지 인식.

```
missions/
├─ spring-learning-test/        ← 개념 검증
├─ spring-roomescape-admin/     ← 미션 1 (학습 중)
└─ java-janggi/                 ← 미션 2 (이전에 진행)
```
...
이 저장소를 자기 노트북에 한 번 클론. 위치는 자유 (홈 디렉토리, `~/IdeaProjects/` 등). 다만
한 번 클론한 위치를 계속 사용:
...
이후 디렉터리 생성, 환경 점검, 학습 테스트 / 미션 저장소 클론 (`missions/` 아래), PR 수집은
모두 첫 AI 세션에서 자동으로 수행합니다. 학습자가 직접 `git clone`이나 `mkdir`을 추가로
실행할 일은 없습니다.
